...
로프 진자 운동을 핵심 조작으로 삼는 Canvas 기반 2D 횡스크롤 게임이다. 하나의 붕괴 도시 월드에 이어진 저작 진행 영역을 돌파하며 로프 숙련과 Foundation 선택으로 빌드를 강화한다.
...
세계관·게임 목표·조작 예시는 브라우저에서 [게임 도움말](./help.html)을 열어 한 페이지에서 확인할 수 있다.
...
- 모바일 이동·점프·로프: 하단 `← · 점프 · →`와 기본 선택된 `로프`를 사용하고 월드를 눌러 조준
- 모바일 마법: 네 슬롯 중 하나를 누른 뒤 월드 위치를 터치해 한 번 시전하며, 시전 뒤 기본 `로프` 선택으로 복귀
