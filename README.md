...
현재 프로토타입은 48단계의 하나의 큰 절차 생성 월드, 모든 지형 표면 부착, 고정 길이 진자 운동, 접선 드래그 충격, 해제 관성,
...
배포 URL에 `?metrics=1`을 붙여 활성 시간·처치·피해·로프 절단·첫 보상 시간을 확인할 수 있다. 다음 단계는 실제 초반 플레이테스트와 서로 다른 기기의 2인 장시간 검증이다.
