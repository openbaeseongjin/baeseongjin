...
배포 URL에 `?metrics=1`을 붙여 활성 시간·처치·피해·로프 절단·첫 Foundation 선택 시간을 확인하고 **진단 복사**로 기록할 수 있다. seed와 world revision은 멀티 참가자가 같은 저작 월드와 결정적 표현을 재현하기 위한 동기화 식별자로 유지한다.
