...
- 도트 캐릭터의 원본 프레임 기준은 24×24픽셀, 기본 월드 출력 크기는 30×30픽셀로 둔다. 스프라이트 구조는 원본 프레임과 출력 크기의 가로·세로 값을 명시적으로 받고 에셋별로 교체할 수 있어야 한다.
...
- 플레이어 표현 상태의 우선순위는 `respawn > hit > rope > jump/fall > run > idle`이다. `hit`은 0.24초, `respawn`은 0.45초 재생한 뒤 현재 지속 상태로 돌아가고, 새 피격은 `hit`을 처음부터 재생하되 `respawn` 중 일반 피격은 무시한다. 이는 표현 전이일 뿐 물리와 입력을 멈추지 않는다.
...
- 실제 렌더 경로 검증을 위해 최소한의 24×24 개발용 mock 스프라이트를 함께 제공하되, 완성형 캐릭터 디자인·색감·고급 픽셀 아트에는 공수를 쓰지 않는다. mock 고유 정보는 게임 상태 계약에 넣지 않고 동일 규격 PNG 교체로 정식 에셋을 적용할 수 있어야 한다.
...
- mock player asset은 `assets/sprites/modern-rpg-guy.png`, 출처·라이선스 기록은 `assets/sprites/README.md`에 있다. 정식 아트로 교체할 때 source/destination 크기와 anchor를 명시하고 collider 설정은 별도로 유지한다.
