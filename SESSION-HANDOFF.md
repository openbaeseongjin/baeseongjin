...
- 브랜치: `main`
- 현재 단계: 이동·절차 월드·기본 전투·생명·모바일·PWA 기반 완료, 로그라이크 런 순환 구현 전
...
아티팩트, 정상 도달 목표, 런 성공, 장거리 통과 가능성 검사와 실제 멀티플레이는 아직 구현하지 않았다.

## 다음 작업

`docs/implementation-roadmap.md`의 P0을 순서대로 진행한다.

1. 정상 목표와 `run-completed` 상태
2. 첫 1~2분 안의 아티팩트 선택 보상
3. 런 한정 아티팩트 인벤토리와 효과 적용
4. 성공·사망 후 새 시드와 빈 아티팩트로 재시작

구현 전에 다음 네 항목을 인터뷰로 확정한다.

1. 정상 도달 후 즉시 종료 또는 다음 구간 진행
2. 첫 보상 트리거: 처치 수, 고도, 시간
3. 아티팩트 선택지 2개 또는 3개
4. 로프 연계 강화의 속도 판정 시점
...
- 사망하면 런 아티팩트를 잃고 새 월드에서 다시 시작한다.
