...
- 도트 캐릭터의 원본 프레임 기준은 24×24픽셀이며 현재 개발용 player definition의 월드 출력 크기는 48×48픽셀이다. 스프라이트 구조는 atlas·원본 프레임·출력 크기의 가로·세로와 anchor·offset을 명시적으로 받고 에셋별로 교체할 수 있어야 한다.
...
- mock player asset은 저장소에서 직접 만든 `assets/sprites/player-action-mock.svg`이며 atlas layout과 출처 기록은 `assets/sprites/README.md`에 있다. `PlayerSpriteDefinition`이 source·atlas/frame/destination 크기·anchor/offset·상태별 clip과 명시적 fallback을 검증하고, renderer는 정의가 반환한 presentation만 소비한다. 정식 아트로 교체할 때도 collider 설정은 별도로 유지한다.
