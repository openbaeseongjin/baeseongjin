...
- 멀티 조작 판정은 사용자 체감을 우선한다. 자기 이동·점프·로프·자기 공격 표시는 입력 프레임에 클라이언트가 먼저 판정·예측하고 서버는 같은 공용 규칙으로 검증·확정한다. 자기 캐릭터는 현재 예측 시간축, 동료는 지연된 두 스냅샷 사이 보간 시간축을 사용한다. 현재의 명령 제출 시점 예측과 동료 데드 레코닝은 이 구조로 대체하며 상세 순서는 `docs/client-first-network-feel.md`에 둔다.
