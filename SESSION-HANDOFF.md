...
- 0.29.0 기본 런은 Sector 01·02·03의 24개 legacy Stage 정의를 수정하지 않고, 각 Sector local 좌표 안에서 `1 → 8` 순서의 세로 등반 spine으로 compile한다. landmark 양옆에는 실제로 밟고 로프로 오갈 수 있는 city wing을 붙여 Sector 폭은 4,800px로 유지한다. `1-1 → 3-8`은 presentation·migration alias이며 intra-Sector 진행은 objective로 열린 physical connector를 직접 통과한다. Stage별 Gate·exit panel·포탈·문 visual, per-Area Checkpoint와 Stage boundary collision은 기본 world output에 없다. future Boss room은 Sector transition slot에 넣어 downstream Sector의 local 좌표·콘텐츠 ID를 다시 쓰지 않는다.
