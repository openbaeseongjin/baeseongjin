...
- 0.42.1 `seamless-sector-runtime-v7`의 Access module topology는 Sector마다 정확히 3개이며 해당 Sector의 다음 경계 해제 요구량도 3개다. Sector 01은 1-3·1-6·1-7, Sector 02는 2-2·2-5·2-7, Sector 03은 3-2·3-5·3-7의 기존 경비 slot을 Carrier로 사용해 적 수·좌표·activation을 늘리지 않는다. 현재 content-end Sector에 아직 다음 Runtime Sector가 없으면 새 전이를 임의로 만들지 않되 module topology는 같은 수치로 저작한다. 요구량을 채우기 전에는 Sector 경계 장치가 실제 통과를 막고 HUD는 `ACCESS x/3 · NEED n`만 보여준다. 위치를 `RIGHT/LEFT` 같은 글자로 설명하거나 `accessHint`를 snapshot에 넣지 않는다. 다음 미수집 Carrier가 화면 밖이면 safe-area 외곽의 무문자 Amber 화살표 하나로 방향을 표시하고, 화면 안으로 들어오면 화살표를 숨기고 Carrier 위의 무문자 다이아몬드 표식으로 전환한다. 이 guide는 공통 HUD 토글을 따르며 모바일 조작·좌상단 HUD와 겹치지 않는다. `3/3` 달성 시 모든 Player에게 재사용 가능한 1.2초 camera unlock scene을 즉시 동시 재생하고 `ACCESS READY`로 전환한다. scene은 카메라 표현만 바꾸며 공용 월드·적·투사체·Player 입력을 멈추거나 Player를 무적으로 만들지 않는다. skip, 화면 밖 피격 보호, 재접속 replay와 복수 연출 queue 정책은 후속 보완으로 남긴다. 개인·전원 사망 뒤에도 모듈·encounter·objective는 보존한다.
