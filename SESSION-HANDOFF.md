...
- 이 체크포인트는 해당 Player의 부활 위치만 소유한다. objective·route·enemy·증강·공용 진행을 되돌리거나 보상을 열지 않는다. Timer·Purge와 전멸 규칙이 미정인 현재는 한 명 또는 전원이 사망해도 `sector-reset`을 만들지 않고 각자 마지막 세이브 포인트에서 즉시 부활한다. 향후 공용 reset은 Timer/Purge 결정에서 명시적인 별도 사건으로 정의하며 사망자 수에서 추론하지 않는다.
- 24개 기본 Runtime Stage는 각각 진입 지점에 명시적인 `STAGE SAVE` 구조물을 가진다. 구조물의 최외곽 로컬 bounds가 gameplay trigger와 renderer의 단일 계약이며 캐릭터 collider 겹침으로만 활성화한다. 활성 anchor가 바뀌면 구조물의 열린 core, 화면 피드백과 cue로 저장 완료를 즉시 알리고, 사망·낙사 안내도 `Sector entry`가 아니라 활성 Stage 세이브 포인트 복귀로 표현한다. 구조물은 부활 좌표와 같은 `respawnAnchor`를 직접 렌더하며 별도 장식 좌표를 만들지 않는다.
- 진행 중 바닥이 생기는 회귀를 막기 위해 seamless Runtime은 objective·save·논리 route 변화 전후 동일한 surface ID 집합을 사용한다. authored 출구·입구 deck 사이 필요한 `sector-seam`은 Run 시작부터 존재하며 `requiredRouteId`로 surface를 추가·제거하지 않는다.

### [L2] Access 경계와 Module 안내는 실제 3-of-3 진행을 숨기지 않는다

- 0.43.2 `seamless-sector-runtime-v8`의 Access module topology는 Sector마다 정확히 3개이며 해당 Sector의 다음 경계 해제 요구량도 3개다. Sector 01은 1-3·1-6·1-7, Sector 02는 2-2·2-5·2-7, Sector 03은 3-2·3-5·3-7의 기존 경비 slot을 Carrier로 사용해 적 수·좌표·activation을 늘리지 않는다. 현재 content-end Sector에 아직 다음 Runtime Sector가 없으면 새 전이를 임의로 만들지 않되 module topology는 같은 수치로 저작한다. 요구량을 채우기 전에는 Sector 경계 장치의 blocker가 중앙 통로뿐 아니라 기본 Grapple budget 안에서 도달 가능한 좌우 우회 경로까지 실제로 막아야 하며, `3/3` 전에는 어떤 이동·로프 경로로도 다음 Sector에 진입할 수 없어야 한다. HUD는 `ACCESS x/3 · NEED n`만 보여주고 위치를 `RIGHT/LEFT` 같은 글자나 `accessHint` snapshot으로 설명하지 않는다. 현재 Sector의 미수집 Module은 최대 3개를 거리순으로 모두 동시에 무문자 guide로 표시하고, Player와 가까운 Module일수록 guide가 더 크게 보여 우선순위를 읽게 한다. 화면 안 Module은 Carrier 위 다이아몬드, 화면 밖 Module은 safe-area 외곽 Amber 방향 표시를 같은 대상·scale resolver로 사용하며 공통 HUD 토글을 따르고 모바일 조작·좌상단 HUD와 겹치지 않는다. 같은 edge의 표식은 방향을 유지한 채 분산한다. `3/3` 달성 시 모든 Player에게 재사용 가능한 1.2초 camera unlock scene을 즉시 동시 재생하고 `ACCESS READY`로 전환한다. scene은 카메라 표현만 바꾸며 공용 월드·적·투사체·Player 입력을 멈추거나 Player를 무적으로 만들지 않는다. skip, 화면 밖 피격 보호, 재접속 replay와 복수 연출 queue 정책은 후속 보완으로 남긴다. 개인·전원 사망 뒤에도 모듈·encounter·objective는 보존한다. v8 회귀 검증은 중앙·좌우 collider sweep, 3→2→1→0 guide 수량과 데스크톱/모바일 safe inset을 함께 고정한다.
- `seamless-sector-runtime-v7`은 Sector 01~03에 Stage-local enemy slot을 `16 → 18 → 22`로 배치한다. 1-1·1-2는 적 0이고 후반으로 갈수록 Pursuit·Shield·Artillery·Support·Swarm 조합을 누적한다. 모든 slot은 매 Run 존재하고 위치·activation·수량은 authored 권위이며, 적 종류만 `slotId + runSeed + worldRevision` pool로 결정한다. 각 intra-Sector Stage 경계의 city wing과 authored core 사이에는 양쪽 64px 하강 개구부를 남긴다. source/target deck·`sector-seam`·wing을 합친 충돌이 전체 폭을 봉인하지 않으므로 1-4까지 먼저 올라가도 1-3 Carrier로 되돌아갈 수 있다. 상세 적 기준은 `docs/enemy-density-composition.md`다.
- 증강 선택 Confirm은 W/위쪽/모바일 점프의 sampled 상태 변화가 아니라 `InputSampler`가 실제 새 누름마다 증가시키는 `interactSequence`를 사용한다. 선택창이 열린 sequence와 같은 held 입력, key repeat, 중간 neutral command는 소비하지 않고 release 뒤의 새 누름만 확정한다. Player command v5가 이 값을 싱글 replay와 멀티 command batch에 보존한다.
...
- 현재 Runtime은 체력, 적 본체 피해·넉백·무적 시간, 사망·낙사 시 Player별 활성 Stage 세이브 포인트 즉시 부활을 사용한다. 한 명 또는 전원 사망 모두 공용 진행을 보존하며 Timer/Purge 결정 전에는 current Sector를 reset하지 않는다. 착지 직전 하강 속도 `800px/s`까지는 안전하고 `1400px/s`에서 최대 체력만큼 피해가 되도록 선형 낙하 피해를 적용한다.
- 0.26.0은 과거 Foundation 3종을 generic 증강 v1로 교체했다. offer Catalog는 Rope 6·교체 Action 6·Signature 6·범용 modifier 4의 22장이고 Player별 최대 6장을 갖는다. 모든 Player는 offer와 별개로 built-in `default-punch` Action을 시작 loadout으로 가지며, 주먹과 여섯 교체 Action이 같은 `ActionAugmentState`의 input edge·activation sequence·charge·recharge·snapshot/restore를 사용한다. 별도 `punchCooldownRemaining`은 제거하고 과거 snapshot migration에만 읽는다. 각 offer는 `runSeed + stablePlayerId + selectionIndex`로 호환 카드 3장을 결정하며 reroll·rarity·동일 Player 중복 선택이 없다. PC 우클릭과 모바일 Action Aim, 감전 로프·충돌 폭발, owner-first `augment-impact`·known tombstone no-op이 구현됐다. 모든 유효 Action 시작은 방향·종류를 포함한 공용 presentation event를 만들고 로컬 주먹 잔상과 `gameplay-action-swing` mock cue로 즉시 읽힌다. 기본 주먹은 적이 없어도 charge를 소비하고 입력 피드백을 만들지만 피해는 기존 사거리·대상 판정만 따른다. 0.28.0 획득 source는 `1-4 Maintenance Node → 2-3 Residential Service Node → 3-5 Commercial Service Node`의 명시적 stable object/objective이며 Sector 04~~06 Node, Timer +10 trigger, Purge origin/rejoin은 계속 HOLD다. 상세 기준은 `docs/augment-v1.md`다.
...
- authored world의 mock은 디버그 도형이 아니라 레벨 피드백이 가능한 공간 blockout이어야 한다. 현재 Runtime의 환경·Stage UI 기준은 로컬 Player 좌표가 포함되거나 수직으로 가장 가까운 `landmark.bounds`다. 각 4,800px Sector는 세로 등반 spine과 city wing으로 읽혀야 하며 Stage Gate·exit panel·portal·route lock visual을 기본 화면에 만들지 않는다. Anchor와 Stage savepoint는 역할이 읽히는 설비 실루엣으로 표시한다.
- 공용 `PlayerMessagePresentation`은 gameplay/Story와 분리된 local message queue·priority·causal dedupe·lifetime·타이핑 진행만 소유한다. message envelope는 `channel/audience/speakerId/text/revealCharactersPerSecond/causalId`를 가져 `player-bark/local-player`와 향후 `party-chat/party`를 같은 core로 수용한다. Stage Bark의 문구·trigger·timing은 별도 message catalog에 중복하지 않고 Direction authoring data가 소유하며 renderer는 해당 speaker 머리 위에서 글자를 순서대로 표시한다. 현재 Bark는 자기 화면에만 표시하고 network snapshot/event에 넣지 않으며 party transport·입력 UI·moderation은 후속 결정 전 구현하지 않는다.
- Stage별 연출을 더 추가하기 전에 Camera·System Text·Player Bark·Audio·Lighting·비언어 표현뿐 아니라 Player 제어·무적, Enemy 생성, Collision·Objective·Gate 같은 gameplay/world 변화까지 시퀀싱 가능한 공용 연출 시스템 구조를 먼저 확정한다. 최상위 목표는 시스템 준수가 아니라 좋은 게임이며 기획 의도를 우선한다. 공용 `DirectionDefinition` schema가 시스템 계약을 소유하고, Stage 기획 데이터는 이 계약에 맞는 실행 가능 저작 원본이 된다. 단, 기획 표현을 개발 Runtime 형식에 직접 종속시키지 않고 기획 친화 입력을 normalize/compile해 immutable Runtime definition으로 변환하는 adapter 경계를 둔다. 변환은 의미 보존이 기본이며 표현 불가능한 기획은 자동 근사하지 않는다. 새 track/authority가 필요하거나 구현 비용·성능·멀티 동기화·asset 제약 때문에 그대로 구현하기 어렵다고 판단되면 `review-required`로 개발자 검토를 요청하고 원본 의도·차단 근거·대안·추천안을 함께 제시한다. 개발자는 의도를 보존하는 시스템 확장·최적화는 직접 결정할 수 있지만, 효과를 축소·변형하는 `fallbackPolicy`는 기획자 승인이 필요하다. 필수 track 미해결은 Stage release를 차단하며 기획에서 `optional: true`로 선언한 track만 손실을 표시하고 release할 수 있다. `DirectionRuntime`은 Beat trigger·순서·재생 수명주기만 소유하고 각 track을 `domain/action/scope/payload/causalId` typed command로 기존 도메인 owner에 위임한다. Camera·Audio·Lighting·말풍선은 local presentation, Player 상태는 소유 클라이언트, Enemy·공용 Collision·Gate·Objective는 서버/월드 권위를 사용하며 compiler가 scope/authority 불일치를 거부한다. 첫 배포는 전체 도메인 command 공개 계약·compiler·validator·runtime을 만들되 실제 production adapter와 기획 migration은 1-1/1-2의 Camera·System Text·Bark·Audio·Lighting·비언어 track까지만 완결하고, 현재 기획에 없는 강제 이동·Enemy 연출을 임의로 추가하지 않는다. `DESIGN LOCKED`는 사람이 소유하고 `unsupported/compile-failed/review-required/unbound/implemented/verified` 구현 상태는 compiler·adapter coverage·검증에서 산출한다. 기준 구조는 `docs/architecture.md`의 **Stage direction authoring target**과 `.omx/specs/deep-interview-stage-direction-system.md`를 따른다.
- 0.45.0 Direction Runtime v1은 위 계약의 첫 배포를 구현했다. 1-1/1-2 `DIRECTION-SPEC.json`을 실행 원본으로 compile하고 Camera·System Text·Bark·Audio·Lighting·Character local adapter를 실제 연결했으며 Player·Enemy·Collision·Objective·Gate는 owner/server typed command와 fake-adapter 회귀를 제공한다. `validate:direction-specs`가 migrated track 33개의 coverage와 release gate를 검사하고 Story/Bark/Area `storyTriggers`의 1-1/1-2 중복 hardcode를 제거했다. Direction Audio 7종은 `default-mock` procedural asset이므로 정식 제작물 교체가 남아 있으며 다른 Stage는 migration 전까지 legacy Story 경로를 유지한다. 기준 구조는 `docs/architecture.md`의 **Stage direction authoring**과 `.omx/specs/deep-interview-stage-direction-system.md`를 따른다.
- Sector 01 `1-1`~`1-8`은 `assets/runtime/environments/sector-01-maintenance/`의 공용 `far/mid/near` 비충돌 배경 package를 사용한다. 세 레이어는 플레이어의 Sector 전체 상승 진행률과 각기 다른 낮은 parallax로 이동하며, 하단에서 시작해 Containment 전환까지 같은 시설로 이어진다. Runtime은 `1024×1536` 원본을 Collision이나 Gameplay Geometry로 사용하지 않고 배경 레이어로만 그리며, package 실패 시 backdrop만 기존 authored fallback으로 전환한다.
- Sector 01 Gameplay surface는 기존 `kind`와 `presentationId`만 사용해 `platform` 32px catwalk, 어두운 `recovery`, 64px 보강 `safe-deck`, X-brace `overhang`, 폐쇄형 `cover/solid`, shutter형 `sealed-door`, 1-1 P0 `ground-foundation`으로 외형을 구분한다. 본체 skin은 기존 collision polygon 안에서만 그리고, one-way 발판은 실제 `oneWayEdgeEnd` chain을 따르는 `2px` 저채도 청회색 분절선, 32px 반복 grate aperture와 작은 상향 절개로 전달한다. 색을 보지 않아도 비어 있는 얇은 catwalk와 꽉 찬 Solid 구조의 실루엣이 달라야 하며 Anchor·Rope Cyan보다 먼저 튀지 않게 한다. P0 시작 바닥은 aperture 없이 같은 저채도 선을 연속으로 유지한다. 이 표현 변경은 1-1 P0의 명시된 전폭 바닥을 제외한 surface 좌표·폭·collision·Anchor·Gate·Portal 위치를 수정하지 않으며 상세 상태는 `assets/runtime/environments/sector-01-maintenance/README.md`를 따른다.
- Sector 01 `grapple-landmark`는 공용 사각 mock 대신 어두운 팔각 정비 하우징, 양쪽 잠금 jaw, 중앙 원형 coupling socket, 작은 Orange service marker와 짧은 상부 bracket으로 표시한다. 강한 Cyan은 전체 외곽선이 아니라 실제 결합 지점인 중앙 core와 socket ring에 집중한다. Runtime object의 `areaId`로 Area `sectorId`를 찾아 표현만 선택하며 Anchor 좌표·label·24×24 비충돌 `grapple-target`·로프 사거리와 물리는 그대로 유지한다. 상세 상태는 `assets/runtime/environments/sector-01-maintenance/README.md`를 따른다.
- Sector 01 좌우 Authored Area 경계벽은 기존 Area bounds에서 파생한 `44px` collision wall을 유지한다. 외형은 64px 반복 장갑 panel bay, 이중 세로 rail, 어두운 pipe channel, 희소한 저강도 Cyan service slot, Orange maintenance fastener와 짧은 내부 brace로 구성한다. Gate Panel 위의 개구부 없는 전폭 층간 bulkhead는 기존 `58px` 높이 안에서 이중 수평 하중빔·반복 X형 철골·중앙 체결판·양끝 두꺼운 하중 post를 사용해, Panel 조작 뒤 별도 Gate aperture로만 다음 영역에 갈 수 있다는 인상을 준다. 새 장식은 이동 공간에 충돌이나 고체 실루엣을 추가하지 않으며 상세 상태는 `assets/runtime/environments/sector-01-maintenance/README.md`를 따른다.
- Sector 01 출구 Gate는 기존 `52×62` bottom-center 문 bounds를 유지한 채 잠김 상태를 장갑 shutter·중앙 maintenance lock·희소한 Orange 봉쇄등으로, 개방 상태를 비어 있는 aperture·측면 jamb·작은 Cyan 통과등으로 표시한다. `44×45` bottom-center Gate Panel bounds도 유지하며 차단은 닫힌 보호 slat, 조작 가능은 Orange cross handle, 개방 완료는 분리된 Cyan 상태창으로 색 외의 형태 차이를 준다. Gate barrier·Portal trigger·interaction radius·좌표는 변경하지 않으며 상세 상태는 `assets/runtime/environments/sector-01-maintenance/README.md`를 따른다.
- Checkpoint 비콘은 모든 Sector가 공유하는 중립 산업 장치로 유지한다. 특정 Sector의 대표색·배관 문법을 넣지 않고 흑연색·철색 본체를 사용하며, 비활성은 닫힌 중앙 shutter, 활성은 양옆으로 열린 shutter와 세로 core, 이미 지난 지점은 닫힌 형태의 낮은 대비로 구분한다. 좌표·활성 반경·진행 저장·부활 판정은 변경하지 않으며 반복 기준은 `docs/graphics-asset-guide.md`의 **장애물과 오브젝트**를 따른다.
- 모든 `wind-source`는 Sector 01의 산업 정비 설비 문법을 적용한 `128×128` 공용 환풍구로 표시한다. 어두운 팔각 장갑 하우징·청회색 보강 프레임·매입 fan well·십자 보호대·벽체 쪽 duct collar를 사용하고 Orange는 service fastener, Cyan은 작동 pressure core와 3칸 출력 meter에만 제한한다. 기존 Wind Zone 상태가 날개와 meter를 함께 구동하며 좌표·radius·bounds·방향·세기·falloff·cycle·Wind Shadow·충돌·Force는 변경하지 않는다. 상세 상태는 `assets/runtime/environments/sector-01-maintenance/README.md`를 따른다.
- 공용 Runtime package의 원본은 `assets/artwork/environments/sector-01-maintenance-vertical-backdrop/export/layers-v2/`의 `far`, `mid-connected-v3`, `near`다. 중경의 큰 기계는 가까운 측벽·후면 격자에 브래킷·트러스·덕트·세로 레일·배관으로 지지되며, 중앙 이동 여백을 가로지르는 Gameplay 발판처럼 보이지 않게 한다. `dark-v3`와 `mid-supported-v2`는 이전 저작 비교 후보로만 남기고 현재 Runtime 입력으로 사용하지 않는다.
- Sector 01 중경 배관은 빈 공간에서 raw cut처럼 끝내지 않고 가까운 설비·서비스 레일·manifold로 연결하거나, 연결이 불필요한 배기구는 flange·guard cap 또는 명확한 외부 파손 흔적으로 의도적인 끝임을 보여준다. 현재 Runtime `backdrop-mid.png`는 `mid-connected-v3`와 같은 파일이며 상단 허브↔팬 보조 배관, 하단 제어함의 capped exhaust와 폐쇄 return loop를 반영한다. 반복 기준은 `docs/graphics-asset-guide.md`의 **배경과 지형**, 상세 제작 상태는 `assets/artwork/environments/sector-01-maintenance-vertical-backdrop/README.md`와 `assets/runtime/environments/sector-01-maintenance/README.md`를 따른다.
- Sector 01의 최종 환경 아트는 배경만이 아니라 현재 Gameplay 발판·벽·Gate·이동 Portal·Gate 패널·Anchor·Checkpoint·Terminal 등 화면에 보이는 이동·상호작용 설비 전부를 `industrial-maintenance` 섹터 컨셉으로 통일한다. 납품 파일은 `far/mid/near backdrop`, collision-aligned `terrain skin`, Gate·Portal·상호작용 object와 상태별 표현으로 분리하며, 기존 Gameplay 구조물의 월드 좌표와 위치 관계는 바꾸지 않는다.
...
- 현재 사용자 검토에서 납품 승인된 `player-main` 모션은 `idle`, `run`, `jump`, `fall`, `death` 다섯 가지다. `rope`, `hit`, `respawn`은 런타임 임시 연결 상태를 유지할 수 있지만 재작업 전에는 최종 승인·납품 자산으로 취급하지 않는다. `death`는 8프레임·총 0.70초이며 사망 직전 위치에서 전부 재생한다.
...
- 클라이언트 우선 수렴 우선순위는 `오차 무시 → 필요 시 비대칭 표현 보정 → 소유권 분리 → 원격 grace → 공격자 관측 존중`이다. 현재 서버는 소유 플레이어 물리를 독립 재실행하지 않으므로 정상 snapshot에는 소유자 보정 자체를 적용하지 않는다. 100ms 보간·120ms 제한 외삽은 동료와 적 복제본에만 적용하고, 공격 claim은 서버 현재 위치·궤적으로 재판정하지 않는다. 장기 수렴 불가의 마지막 안전장치는 소유자를 과거 서버 복제값으로 hard sync하는 것이 아니라 연결을 종료하고 재접속 welcome에서 현재 공유 상태로 새 세션을 시작하는 것이다.
...
- 기본 Sector Runtime에서 멀티 소유 클라이언트는 자기 savepoint 충돌과 chooser를 먼저 적용하고 서버가 검증한다. objective 완료와 공용 보상 source 소비는 서버 `GameSimulation`에서 수렴하지만 Stage cursor·landmark entry·물리 route unlock은 사용하지 않는다. legacy Area revision의 exit panel·Gate portal은 compatibility 경로에만 남는다.
- 증강 선택 UI는 owner client가 명시적 `augment-node` 근처에서 결정적 3장 offer를 즉시 열고 서버가 membership·source 소비를 검증한다. Player 전원의 source 소비 뒤 공유 objective를 완료하되 route geometry를 잠그지 않는다. 사망·재접속은 같은 offer·selection index·consumed source를 보존한다.
- impact의 최종 체감 결과는 피해 클라이언트가 소유한다. 적 탄환과 착지 피해의 정상 claim은 사건 자료와 도메인 상태 지문만 보내며 서버가 같은 전이를 적용해 일치하면 바로 확정한다. 지문이 다를 때는 `impact-claim-receipt`의 `accepted: true`, `resolution: recovery-required`와 impact ID·피해자 연결에 묶인 일회용 `recoveryId`로 복구 자료를 요청한다. 피해 클라이언트는 응답 시점의 최신 소유자 상태·`stateTick`·새 지문을 한 번 보내 서버·동료를 자기 결과로 수렴시킨다. challenge 없는 전체 상태는 거부하고, 승인 복구는 상태와 `ownerMotionTick`을 원자적으로 갱신하며 로컬 HP·로프·부활은 복구하지 않는다. 상세 기준은 `docs/multiplayer-synchronization.md`의 **impact claim과 최종 수렴**을 따른다.
...
섹터 1 Runtime은 하나의 4,800px 연속 공간이다. `1-1`~`1-8` landmark는 geometry·object·objective 저작 단위지만 물리 Stage 진입 권위가 아니다. 모든 surface와 seam은 시작부터 정적이고 objective는 자기 trigger/prerequisite로 판정한다. savepoint 충돌만 Player checkpoint를 갱신하며 전원 사망도 공용 진행을 초기화하지 않는다. Boss·Timer·Purge 전환 제약은 HOLD다.
...
Sector 01-2의 현재 기준은 `docs/bsh/scenario/1/1-2/README.md`의 `DOUBLE ANCHOR SHAFT` REV 3.1, `PRODUCTION-ALIGNMENT.md`다. 이 구간은 증강 선택 전 기본 Rope 학습 구간이며 과거 Foundation 세 Profile 전용 telemetry를 새 Runtime 계약으로 사용하지 않는다.
...
Sector 01-4 `maintenance-node`, 2-3 `specialization-node`, 3-5 `service-calibration-frame`은 Player별 generic Augment offer source다. source 소비와 선택은 사망·재접속·전원 사망 뒤 유지된다.
...
시나리오 mock 연결 뒤 최근 멀티 변경을 실제 두 기기에서 확인한다. 두 플레이어가 겹치지 않고 서로를 지지하는지, 모바일 HP가 항상 보이는지, 로프 몸체 충돌의 즉시 피드백과 서버 확정이 중복 표시되지 않는지, 착지 피해 HP가 피해자·서버·동료에서 수렴하는지를 한 채널에서 함께 검증한다. 실패 시 채널 번호·월드 시드·디버그 패널의 **진단 복사** 결과와 재현 순서를 남긴다.
...
5. 서로 다른 실제 기기에서 로프 절단·사망·개별 Stage 세이브 포인트 부활·전원 사망 공용 진행 보존·content boundary 도달을 한 세션으로 검증
...
- 장기 목표는 맵 저작 데이터의 단일 권위를 세우고 `README → AREA-SPEC-REV8-DESIGN.json → JS Catalog → PRODUCTION-ALIGNMENT` 사이의 수동 중복과 drift를 줄이는 것이다. 이를 위해 기존 Stage 데이터의 명시적인 migration을 설계 범위에 포함한다.
...
- 완료 조건은 Gate를 직접 자동 개방하지 않고 문 옆 패널을 활성화한다. 모든 층은 활성 패널 조작으로만 문을 열며, 층간 경계는 개구부 없는 전폭 고정 충돌 격벽으로 막아 로프 탄성 우회를 허용하지 않는다.
...
- `CommandRecorder`가 dt와 PlayerCommand 복제본을 기록하고, 재생 결과의 위치·전투·체크포인트·generic Augment·지표 다이제스트로 결정성을 비교한다.
- `PlayerCommandBatch` 프로토콜 v5가 목표 틱·플레이어 ID·입력 `sequence`, 협동 `interact` 의도와 실제 새 상호작용 누름을 나타내는 `interactSequence`를 정규 정렬하고 JSON 왕복 시 동일한 불변 계약을 유지한다.
...
- 협동 동기화는 클라이언트 트리거·서버 검증형으로 설계하며, 1/120초 공용 규칙·60Hz 입력·20Hz 공유 스냅샷을 사용한다. generic Augment loadout은 플레이어별 빌드로 유지하며 `FoundationAugmentState`와 관련 wire 필드명은 호환 경계다. 상세 계약은 `docs/multiplayer-synchronization.md`를 따른다.
- 투사체·낙하물처럼 예측 가능한 객체는 전체 위치를 반복 전송하지 않는다. 명시적으로 자동 무기를 활성화한 경우 플레이어 발사는 소유 클라이언트가 즉시 예측한 뒤 `projectile-spawn-claim`으로 보내며, 서버는 비활성 claim을 거부하고 활성 claim의 소유권·`predictionId` 형식·tick 범위와 소유자 clientTick 간격(발사 주기 - 2틱 유예)을 검증해 같은 projectile ID receipt와 spawn 사건을 멱등 확정한다. 서버 시계의 쿨다운·서버 상태의 최근접 대상 재선택·재계산한 발사 위치 오차는 거부 조건이 아니다. 멀티 서버 fixed tick은 플레이어 탄환을 따로 생성하지 않고 승인된 탄환의 복제 궤적·수명만 진행하며, 거부 receipt는 로컬 예측 탄환을 취소한다. 몹·적 투사체 같은 중립 객체의 생성·궤적·수명은 서버가 담당한다. 양쪽 모두 생성 틱과 초기 상태를 공유해 각 클라이언트가 재생하며, 플레이어와 만나는 피격·로프 절단은 피해 클라이언트 claim으로 확정한다. 공격 클라이언트의 탄환과 피해 클라이언트가 충돌 처리한 적 탄환은 첫 로컬 적중에서 소비되며 receipt 거부로 되살아나지 않는다. 중간 입장 welcome은 활성 객체의 원래 spawn 이벤트를 같은 ID로 다시 제공한다. 상세 계약은 `docs/multiplayer-synchronization.md`를 따른다.
...
- `WorldSnapshotEnvelope` 프로토콜 v11은 top-level `respawnAnchorId`·`partyWipeBaseline`을 두지 않고 각 `players[]` state의 개인 `respawnAnchorId`와 non-null Action state를 전송한다. 기본 주먹도 `default-punch` charge/recharge로 동료 HUD까지 수렴한다. authored enemy의 `displayName`·`activation`·`rules` 같은 정적 정의는 `worldRevision + objectId`로 클라이언트가 복원하고, 20Hz에는 위치·HP·행동·공격·넉백 같은 동적 상태만 전송한다. Player command v5, owner-motion v5, player-impact v9, augment-impact v1, augment-offer v1을 함께 사용한다.
- 0.43.4 멀티 예측은 20Hz snapshot마다 56개 Enemy runtime 그래프를 재생성하지 않는다. `id/objectId/enemyType`가 같은 Enemy는 기존 runtime·collider·Patrol·attack/behavior FSM을 in-place restore하고 실제 spawn/despawn 또는 정적 identity 변경만 생성·제거한다. `RemoteWorldStateBuffer`는 snapshot push 때 collection별 ID index를 만들고 sample의 반복 배열 `find()`를 제거하며, `MultiplayerGameApp.update()` 한 fixed step은 remote sample 하나를 chooser·collision·audio·camera에서 재사용한다. 이 최적화는 120Hz prediction, 20Hz snapshot, owner-first 권위와 100ms 보간/120ms 외삽 계약을 바꾸지 않는다.
...
- 이벤트의 즉시 체감과 지속 상태 수렴은 별도 경로다. 플레이어·로프·HP·사망·Player별 세이브 부활은 소유·피해 클라이언트 결과를 서버와 동료가 검증해 따른다. 몹과 공용 objective는 서버 스냅샷이 수렴 원점이며 party-wipe baseline은 사용하지 않는다.
- 기본 멀티는 `WorldSnapshot` protocol v11의 `progressKind: sector`, Player별 `respawnAnchorId`·Action state와 동적 enemy state를 사용한다. 개인 치명 피격·낙사는 피해 클라이언트가 자기 활성 Stage 세이브 포인트 부활을 먼저 적용하고 서버·동료가 owner-motion v5·player-impact v9로 수렴하며 legacy checkpoint claim을 보내지 않는다. 한 명 또는 전원 사망으로 `sector-reset`을 만들지 않는다. 체크포인트 claim·rollback은 이전 Area world revision compatibility에만 남고 기본 `MultiplayerGameApp`은 Area progress에서만 해당 경로를 호출한다.
- 현재 기본 저작 시나리오의 진행은 위치 기반 objective·명시적 prerequisite와 마지막 objective의 content boundary를 사용한다. Stage cursor·physical route lock·동적 connector는 없으며 legacy Gate/portal과 절차 월드 summit claim은 compatibility test에만 남는다.
...
- 특정 플레이어에게 귀속되는 이동·로프·공격·피격·절단 같은 사건은 소유자 또는 피해자 클라이언트가 즉시 로컬 적용하고 claim을 보낸다. 서버는 체감 경로를 먼저 시작하지 않는다. impact는 인증·형식·중복과 결과 지문을 확인하고, 공격 claim은 소유권·ID·중복·공식 효과와 대상 생존/tombstone만 검증한다. 클라이언트가 본 지연 표본과 서버 현재 위치·궤적·겹침이 다르다는 이유로 정상 공격을 거부하지 않는다.
- 명시적으로 활성화한 플레이어 자동 무기 투사체는 발사한 클라이언트가 적중을 먼저 판정한다. 멀티 서버 fixed tick은 검증용 궤적·대상 소실·8초 수명만 진행하고 적 충돌이나 HP 감소를 자동 시작하지 않는다. 공격 클라이언트는 첫 적중에서 총알을 소비하고 VFX와 hit claim을 한 번만 만든다. 거부 receipt가 총알을 같은 겹침에 복구하지 않으며 승인 뒤에는 서버 적 HP 스냅샷과 resolve 사건으로 모두 수렴한다.
- 멀티 서버 fixed tick은 체력 0을 스캔해 사망·부활을 시작하거나 보상 선택 입력을 직접 확정하지 않는다. 플레이어 사망·부활은 피해 `player-impact` claim, generic Augment 선택은 호환 이름의 `foundation-selection` claim에서만 검증·확정한다. 싱글은 같은 `GameSimulation`의 기본 자동 복구와 로컬 보상 입력을 유지한다.
...
- 클라이언트의 자기 캐릭터 체감을 최우선으로 한다. 멀티 HUD는 owner HP·로프·선택 증강·Action charge를 표시한다. Player 피격과 Player-source 적 피해 모두 로컬 반응을 먼저 적용하고 정상 승인 뒤 서버 snapshot으로 owner 반응을 되감지 않는다.
- impact receipt는 피해 클라이언트가 이미 인식한 HP·부활·로프 상태를 되감지 않는다. 서버가 같은 전이를 적용한 지문이 다르면 같은 `impact-claim-receipt`의 `recovery-required` 성공 결과로 복구 자료를 요청하고, 그때만 피해 클라이언트가 최신 소유자 상태를 한 번 보내 서버 복제본과 동료를 수렴시킨다. 정상 impact마다 전체 상태를 보내거나 소비한 적 탄환을 다시 표시하지 않는다.
- 클라이언트 피드백은 공용 월드 효과와 개인 상태 효과를 capability/mixin으로 분리한다. 타격·로프 절단 위치의 링·파티클과 원격 Player 사망 animation은 모든 클라이언트가 같은 사건으로 재생하지만, 화면 흔들림·피해 강조·로프 절단 경고와 `Stage 세이브 포인트 부활` 같은 상태 문구는 사건의 `playerId/sourcePlayerId/targetId`가 현재 viewer와 일치할 때만 각 사용자 화면에서 재생한다. `player-respawned`는 `statusType`과 원인을 공유하고 각 클라이언트의 personal status capability가 자기 문구와 2.2초 수명을 로컬 소유하며, 전역 `eventFlash`나 앱별 viewer 분기에 의존하지 않는다. 서버는 효과 수명이나 파티클을 전송하지 않는다. 상세 계약은 `docs/architecture.md`와 `docs/multiplayer-synchronization.md`를 따른다.
...
- `OwnerPredictionRuntime`은 소유 `InputDrivenObject` 집합의 입력 이력·예측 tick·claim 수명·별도 복구 계약이 있는 사건 전이·표시 보정만 담당한다. `owner-motion` receipt로 물리 상태를 복원하지 않으며 impact receipt도 로컬 피해 상태를 복구하지 않고 `recovery-required`일 때 최신 소유자 상태를 서버에 전달한다. 실제 이동·로프·전투 규칙은 capability 믹스인과 시뮬레이션 단계에 둔다.
...
- 자기 플레이어의 물리 예측과 Canvas 표시 상태를 분리한다. 정상 동기화 중에는 서버 위치·HP·로프·생명 상태로 스냅하지 않는다. `owner-motion`은 상태 크기 때문에 거부하거나 receipt로 표시 보정을 시작하지 않으며 impact `recovery-required`는 서버·동료 쪽을 피해자의 최신 상태로 수렴시킨다.
...
- `game-kit`은 순수 기반만 소유하고 로프·월드·적·generic Augment·HUD는 `src/game/`과 렌더링 계층이 소유한다.
...
- `owner-motion` v4와 `player-impact` v8은 owner-owned Action cooldown·guard·shield mirror와 피해 결과를 수렴시키되 정상 승인 때 서버 snapshot으로 owner 반응을 되감지 않는다. `augment-impact`는 공격 owner가 만든 damage+movement 사건만 검증·공유하며 known tombstone의 late event를 atomic no-op 처리한다.
- 자동 무기 발사점은 몸 중심이 아니라 조합된 `Collider.outsidePointToward()`가 대상 방향으로 계산한 형상 바깥 점이다. 발사체 반경과 8px 여유를 포함하며 소유자는 최신 `owner-motion` 다음 총구 위치 claim을 보낸다. 서버는 소유권·prediction ID·tick·발사 간격을 검증하고 claim 위치를 그대로 spawn 사건에 사용하며, 지연된 서버 위치에서 총구를 재계산해 거부하지 않는다. 구현 경계는 `docs/architecture.md`, 전송 계약은 `docs/multiplayer-synchronization.md`를 따른다.
...
- 애니메이션 상태를 renderer 내부 조건문으로 흩뜨리지 않는다. 현재 상태·경과 시간·허용 전이만 소유하는 순수 `StateMachine`, 남은 시간 예산을 조합하는 `TimedStateController`, actor별 snapshot을 표현 상태로 바꾸는 resolver를 분리한다. 플레이어는 기존 전용 resolver를, 적은 `EnemyPresentationState`를 사용하며 의미와 수명이 다른 gameplay·투사체 상태를 하나의 FSM으로 합치지 않는다.
...
- 2026-08-20 사용자 최종 검수로 Sector 01의 `sentry-t1`과 `pursuit-drone-t1` 이미지·애니메이션을 완료 처리한다. Sentry는 후보 `06` 기반 고정 베이스와 upright 회전 머리·단일 포신, Pursuit는 후보 `09` 기반 기본 이미지와 `seek/windup/dash/recover/knockback` 각 4프레임이 승인 범위다. 현재 Runtime 기준과 검증 기록은 `assets/runtime/characters/sector-01-enemies/README.md`가 소유하며, 명시적인 새 사용자 결정 전에는 두 자산을 재생성·교체 대상으로 취급하지 않는다. Shield·Artillery 및 다른 몬스터는 이번 최종 승인에 포함하지 않는다.
