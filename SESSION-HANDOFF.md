...
- Foundation 선택 UI는 클라이언트가 즉시 연다(`openFoundationChooserCandidate`: 예측 위치가 augment-node의 interactionRadius 안 + interact 입력). 열리면 예측 로프를 해제하고 명령을 zeroing하며, 확정된 선택만 서버에 claim으로 보내 서버가 거리·선택 충돌·tick 창을 검증한 뒤 receipt로 답한다(서버 reward 선개방 불필요, `requireOpenReward: false`). 거부 receipt는 UI를 닫고 재시도할 수 있게 한다. interact-choice 개방 평가는 공용 frontier 영역에 묶지 않고 모든 영역의 노드를 검사해 늦게 도착한 플레이어도 선택할 수 있다.
