...
- impact claim 대기 중에는 projectile ID별 피격 직전 전체 소유자 상태와 tick을 보존한다. 승인 receipt는 복구 정보를 제거하고 권위 HP·부활·로프 상태로 수렴한다. 거부 receipt는 적 탄환을 다시 활성화하는 동시에 피격 직전 상태를 복원하고 그 뒤 실제 입력만 공용 1/120초 시뮬레이션으로 재실행해, 거절된 넉백·무적·로프 절단·재부착 금지가 남지 않게 한다.
