...
- `WorldSnapshotEnvelope` 프로토콜 v9는 v8 Sector 진행 계약을 유지하면서 authored enemy의 `displayName`·`activation`·`rules` 같은 정적 정의를 `worldRevision + objectId`로 클라이언트가 복원하고, 20Hz에는 위치·HP·행동·공격·넉백 같은 동적 상태만 전송한다. Player command v4, owner-motion v4, player-impact v8, augment-impact v1, augment-offer v1을 함께 사용한다.
