...
- 투사체·낙하물처럼 예측 가능한 객체는 전체 위치를 반복 전송하지 않는다. 플레이어 자동 무기 발사는 소유 클라이언트가 즉시 예측한 뒤 `projectile-spawn-claim`으로 보내며, 서버는 소유권·tick·쿨다운·최근접 대상·발사 위치를 검증해 같은 projectile ID receipt와 spawn 사건을 멱등 확정한다. 멀티 서버 fixed tick은 플레이어 탄환을 따로 생성하지 않고 쿨다운·확정 탄환의 검증용 궤적·수명만 진행하며, 거부 receipt는 로컬 예측 탄환을 취소한다. 몹·적 투사체 같은 중립 객체의 생성·궤적·수명은 서버가 담당한다. 양쪽 모두 생성 틱과 초기 상태를 공유해 각 클라이언트가 재생하며, 플레이어와 만나는 피격·로프 절단은 피해 클라이언트 claim으로 확정한다. claim 대기 탄환은 숨긴 채 진행하고 거부 receipt에서 복원하며, 중간 입장 welcome은 활성 객체의 원래 spawn 이벤트를 같은 ID로 다시 제공한다. 상세 계약은 `docs/multiplayer-synchronization.md`를 따른다.
- 플레이어 자동 발사 예측은 탄환뿐 아니라 발사 직전 무기 쿨다운도 prediction ID별로 보존한다. 승인 receipt는 예측 기록을 정리하고 이후 권위 스냅샷의 쿨다운으로 수렴하며, 거부 receipt는 예측 탄환 취소와 함께 경과 tick을 뺀 발사 이전 쿨다운으로 즉시 복구한다. claim 대기 중 도착한 이전 스냅샷은 로컬 발사 반응을 지우지 않는다.
- 로프 공명 강화는 소유 클라이언트가 스윙 프레임에 즉시 적용하고 `owner-motion → rope-swing-claim → projectile-spawn-claim` 순서로 보낸다. 서버는 소유권·tick·위치·부착 anchor·아티팩트를 검증해 강화 타이머와 `rope-swing` 사건을 멱등 확정한다. 거부 receipt는 해당 로컬 예측 강화 이전 값으로 복구하고, 승인 뒤 소유자·서버·동료는 스냅샷의 강화 시간과 무기 피해량으로 최종 수렴한다.
...
- impact claim 대기 중에는 projectile ID별 피격 직전 전체 소유자 상태·tick·발생 순서를 보존한다. 승인 receipt는 복구 정보를 제거하고 권위 HP·부활·로프 상태로 수렴한다. 거부 receipt는 적 탄환을 다시 활성화하는 동시에 피격 직전 상태를 복원하고 그 뒤 실제 입력과 아직 대기 중인 후속 impact를 원래 tick·순서대로 공용 1/120초 시뮬레이션에 재실행한다. 재적용한 impact의 직전 상태도 새 시간축으로 갱신해, 앞 claim 거절이 뒤 피격을 지우거나 뒤 claim 거절이 오래된 영향을 되살리지 않게 한다.
