...
- 기본 런은 Sector 01·02·03의 24개 legacy Stage 정의를 수정하지 않고 각 Sector local 좌표 안에서 세로 등반 spine으로 compile한다. landmark 양옆에는 실제 city wing을 붙여 Sector 폭은 4,800px로 유지한다. Stage별 Gate·exit panel·포탈·문 visual과 물리 route lock은 없으며 모든 authored surface와 `sector-seam`은 Run 시작부터 collision/renderer에 같은 정적 집합으로 존재한다. future Boss room 제약은 Boss 구현 시 별도 계약으로 추가한다.
...
- Sector 01 access vertical slice는 1-3·1-6·1-7 Carrier 중 아무 2개를 처치해 공용 모듈을 얻는 objective를 유지하지만 Sector 01→02 물리 connector를 잠그지 않는다. HUD는 좌표 기반 현재 Sector에서 수집 수와 남은 signal을 표시하고 개인·전원 사망 모두 모듈·encounter·objective를 보존한다.
