...
- 자기 탄환의 로컬 충돌은 타격 VFX를 즉시 재생하고 공격 클라이언트가 `predictionId`·대상·clientTick·적중 위치만 주장한다. 서버는 연결 소유권, 탄환·대상, 허용 tick과 위치 오차, 중복을 검증하고 서버가 보유한 대미지로 HP·사망을 최종 확정한다. 적 탄환의 플레이어 피격은 서버 확정을 유지한다.
