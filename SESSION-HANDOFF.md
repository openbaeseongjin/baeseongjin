...
- 서버는 소유 클라이언트가 만든 플레이어 상태·사건을 검증해 다른 클라이언트에 공유하는 허브다. 정상 승인 중인 소유자의 HP·피격 무적·생명·로프·쿨다운·시간 제한 강화는 서버 스냅샷으로 다시 쓰지 않는다. 서버가 상태를 직접 진행하는 범위는 몹·중립 투사체·공용 월드·세션 수명주기이며, 소유자 전체 복원은 최초 입장·재접속과 명시적 claim 거부 복구로 제한한다. 상세 계약은 `docs/multiplayer-synchronization.md`를 따른다.
...
- 이벤트의 즉시 체감과 지속 상태 수렴은 별도 경로다. 플레이어·로프·HP·사망·개별 체크포인트 부활의 수렴 원점은 서버 스냅샷이 아니라 소유·피해 클라이언트의 로컬 `GameSimulation` 결과이며, 서버 복제본과 동료가 검증된 claim·`owner-motion`을 통해 이를 따라간다. 몹·공용 월드의 수렴 원점만 서버 스냅샷이다. 서버가 claim을 명시적으로 거부하면 클라이언트는 전이 전 상태를 복원하고 이후 미확정 입력과 사건을 공용 1/120초 시뮬레이션으로 재실행한다. 동료와 적은 지연된 두 스냅샷 사이 보간과 제한 외삽으로 각 권한 원점에 수렴한다. 상세 계약은 `docs/multiplayer-synchronization.md`를 따른다.
...
- 클라이언트의 자기 캐릭터 체감을 최우선으로 한다. 자기 탄환 충돌뿐 아니라 자신과 교차한 적 탄환의 본체 피격·로프 절단도 피해 클라이언트가 즉시 판정해 피드백·넉백·HP·치명 시 체크포인트 부활과 아티팩트 손실까지 같은 로컬 `GameSimulation` 전이로 적용하고 claim을 보낸다. 멀티 HUD도 마지막 서버 플레이어가 아니라 소유 클라이언트의 HP·로프 비활성·아티팩트를 표시한다. 피해 경로는 피격 직전 `owner-motion`을 캡처하고 로컬 반응을 먼저 적용한 뒤, 전송선에서는 캡처한 motion 다음 impact claim 순서를 지킨다. 서버는 클라이언트 대미지를 신뢰하지 않고 연결 소유권, 허용 tick·자신이 가진 탄환의 궤적·대미지·위치 오차와 중복을 검증해 서버 복제본과 동료에게 공유한다. 정상 승인 뒤 서버 HP·부활·로프 상태를 소유 클라이언트에 다시 적용하지 않는다.
- impact claim 대기 중에는 projectile ID별 피격 직전 전체 소유자 상태·tick·발생 순서를 보존한다. 승인 receipt는 로컬 결과가 검증됐다는 뜻이며, 해당 projectile의 resolve 사건까지 이전 공유 snapshot의 덮어쓰기를 막는다. resolve 사건은 복구 정보를 정리하지만 소유자의 HP·부활·로프를 서버 값으로 교체하지 않는다. 거부 receipt만 적 탄환을 다시 표시하고 피격 직전 상태를 복원한 뒤 실제 입력과 아직 대기 중인 후속 impact를 원래 tick·순서대로 공용 1/120초 시뮬레이션에 재실행한다. 탄환 자체는 같은 겹침에서 즉시 재충돌하지 않고 대상과 분리된 뒤 다시 진입해야 재무장된다. 재적용한 impact의 직전 상태도 새 시간축으로 갱신해, 앞 claim 거절이 뒤 피격을 지우거나 뒤 claim 거절이 오래된 영향을 되살리지 않게 한다.
- 정상 승인 중인 소유자의 로프·HP·생명·위치가 서버 복제 스냅샷과 달라도 소유자 상태를 스냅하지 않는다. 최초 입장·재접속과 명시적 claim 거부 복구에서만 공유 기준 상태를 사용하며, 이때 작은 위치 오차는 짧게 감쇠하고 큰 오차는 즉시 복원한다.
...
- `OwnerPredictionRuntime`은 소유 `InputDrivenObject` 집합의 입력 이력·예측 tick·claim 수명·명시적 거부 복구·표시 보정만 담당한다. 실제 이동·로프·전투 규칙은 capability 믹스인과 시뮬레이션 단계에 두며 이 런타임에 종류별 분기를 추가하지 않는다.
- 소유 `InputDrivenObject`는 앱의 120Hz 고정 스텝마다 capability 입력과 피해 사건을 즉시 적용하고 제한된 이력을 보존한다. 60Hz submit, 승인 receipt와 정상 스냅샷은 소유자 상태를 되감지 않는다. 최초 입장·재접속과 명시적 거부 전이만 공유 상태를 복구 기준으로 사용한다.
- 자기 플레이어의 물리 예측과 Canvas 표시 상태를 분리한다. 정상 승인 중에는 서버 위치·HP·로프·생명 상태로 스냅하지 않는다. 명시적 거부 복구에서만 작은 표시 오차를 감쇠하고 큰 오차를 즉시 복원한다.
