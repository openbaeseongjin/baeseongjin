...
- 최근 Sector/Stage 통합 뒤 진행 중 갑자기 생기는 바닥은 별도 증상 보정으로 숨기지 않는다. `sector-connector`는 두 Stage의 authored 출구·입구 deck 사이 실제 빈 구간만 같은 deck 높이로 채우며, anchor 중심 전체를 덮는 큰 platform을 생성하지 않는다. 실제 surface ID·종류·`requiredRouteId`·collision/renderer 상태를 같은 frame에서 확인해 compiler 또는 공용 progress predicate의 근본 원인을 수정한다.
...
- 0.31.3부터 실제 traversal이 landmark entry 판정점을 비껴 `currentLandmarkId`가 뒤처져도, 현재 Sector에서 실제 도달한 명시적 `augment-node`의 `interact-choice`는 PC W/점프·모바일 점프 근접 상호작용으로 chooser를 연다. 다른 Sector·거리 밖 Node와 일반 objective는 기존 current-landmark 경계를 유지한다. 디버그 이동에서만 선택되던 회귀를 막는 기준은 `SectorProgressController`와 `docs/architecture.md`가 소유한다.
...
- authored world의 mock은 디버그 도형이 아니라 레벨 피드백이 가능한 공간 blockout이어야 한다. 현재 Runtime의 렌더 기준은 공용 진행이 아니라 로컬 플레이어의 X/Y가 포함되거나 수직으로 가장 가까운 `landmark.bounds`이며, 그 landmark의 `sectorId`로 환경 표현을 고른다. 진행이 전진한 뒤 플레이어가 이전 섹터 높이로 떨어져도 자기 위치의 섹터 표현으로 즉시 돌아간다. 각 4,800px Sector는 기존 세로 등반 spine과 양옆 city wing으로 공간 단위가 읽혀야 하며, 가로 폭은 진행을 좌우로 꺾는 기준이 아니다. Stage 경계벽·층간 격벽·Stage Gate·exit panel·portal visual을 기본 화면에 다시 만들지 않는다. Anchor·route lock·Sector-entry beacon은 역할이 읽히는 설비 실루엣으로 표시하고 Recovery point 같은 설계용 위치와 절차 생성 `default-mock` decoration은 기본 플레이 화면에 노출하지 않는다. 사용권을 확인하지 않은 시나리오 레퍼런스 이미지는 runtime asset으로 사용하지 않으며 반복 기준은 `docs/graphics-asset-guide.md`의 **mock도 공간 피드백을 전달한다**를 따른다.
...
- 기본 Sector Runtime에서 멀티 소유 클라이언트 예측은 공용 world progress(route objective·interact-choice 보상·landmark entry)를 로컬에서 확정하지 않는다. 이 시스템 상호작용은 서버 `GameSimulation`만 진행하고 클라이언트는 `worldProgress` snapshot과 복제 사건으로 수렴한다. 보상이 서버에서 열리면 `synchronizePredictionProgress`가 예측 로프를 해제해 서버 `beginFoundationReward`와 맞춘다. legacy Area revision의 exit panel·Gate portal은 compatibility 경로에만 남는다.
