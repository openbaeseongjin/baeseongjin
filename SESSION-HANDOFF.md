...
- `RemoteWorldStateBuffer`가 최대 8개 권위 스냅샷을 보관하고 동료·적 위치를 100ms 지연된 서버 tick의 두 표본 사이에서 보간한다. 미래 표본이 없을 때만 최대 120ms 외삽하며 HP·체크포인트 부활·로프·진행 상태는 최신 권위 값을 즉시 사용하고 사건을 한 번만 전달한다.
