...
- 최신 로프 해제와 낙사 전이는 연속 운동 봉투가 거부돼도 별도 단조 tick과 도메인 전이로 확정
...
자동 멀티 시나리오는 OS 네트워크 설정을 바꾸지 않고 테스트 WebSocket 경계에서 왕복 지연 0/50/100/200ms와 송신 명령 손실 0/2/5%의 모든 조합을 재현한다. 최대 프로필에서도 늦은 명령 거부 원칙을 유지하도록 입력은 30틱(250ms) 앞에 예약하며, 모든 프로필에서 자기 입력의 즉시 예측과 서버 권위 이동 지속을 검증한다.
...
- `RunMetrics`가 활성 플레이 시간·체크포인트·처치·피해·로프 절단·패배·첫 보상 시간을 권위 시뮬레이션에서 수집하며, 멀티는 이 값을 서버 snapshot으로 전달해 HUD와 진단 복사에 사용한다.
...
- 협동 동기화는 클라이언트 트리거·서버 검증형으로 설계하며, 1/120초 공용 규칙·60Hz 입력·20Hz 공유 스냅샷을 초기값으로 사용한다. 아티팩트는 플레이어별 빌드이며 다운 중에는 유지되고, 팀 전멸 복귀 때 서버가 각자 일부 손실을 확정한다. 상세 계약은 `docs/multiplayer-synchronization.md`를 따른다.
- 투사체·낙하물처럼 예측 가능한 객체는 전체 위치를 반복 전송하지 않는다. 담당 클라이언트가 생성 틱과 초기 상태를 `spawn` claim으로 트리거하고 서버가 검증·공유하며, 피격·로프 절단·만료·제거도 담당 클라이언트 claim을 `resolve` 이벤트로 확정한다.
...
- 멀티 조작 판정은 사용자 체감을 최우선으로 한다. 자기 이동·점프·로프·피격 반응은 클라이언트 운동 상태를 기준으로 하며, 일상적인 서버 스냅샷은 소유 플레이어의 위치·속도·로프를 되감지 않는다. 클라이언트가 운동 상태를 서버에 보내면 서버는 tick·속도·이동 거리 봉투를 검증해 공용 월드에 반영한다. HP·다운·부활·체크포인트 같은 결과는 서버가 최종 확정한다. 동료는 지연된 두 스냅샷 사이 보간 시간축을 사용한다.
...
- 모든 게임플레이 이벤트의 최초 트리거 주체는 클라이언트다. 이동·로프·공격·피격·절단·획득·체크포인트·다운·부활처럼 플레이어가 인식하는 사건은 담당 클라이언트가 즉시 로컬 적용하고 claim을 보낸다. 서버는 새 사건을 체감 경로에서 먼저 시작하지 않고 claim의 소유권·tick·위치·중복을 검증해 공용 결과를 확정·배포한다. 서버가 먼저 보낸 결과를 기다린 뒤 모바일 반응을 시작하는 구현은 금지한다.
- 트리거 담당은 자기 행동은 소유 클라이언트, 충돌·피격은 피해 클라이언트, 대상 없는 공용 월드 사건은 안정적인 플레이어 ID 순서의 대표 클라이언트로 정한다. 여러 클라이언트가 같은 사건을 감지할 수 있으면 결정적 event key로 서버가 한 번만 승인한다. 연결·퇴장·방 제거 같은 세션 수명주기만 서버 시작 이벤트로 허용한다.
- 이 규칙은 신규 기능과 수정 기능에 즉시 적용한다. 기존 서버 시작형 게임플레이 사건은 전환 대상으로 취급하며, 완료되지 않은 경로를 클라이언트 트리거 완료로 문서화하지 않는다.
- 클라이언트의 자기 캐릭터 체감을 최우선으로 한다. 자기 탄환 충돌뿐 아니라 자신과 교차한 적 탄환의 본체 피격·로프 절단도 피해 클라이언트가 즉시 예측해 피드백과 이동 반응을 적용하고 claim을 보낸다. 서버는 연결 소유권, 허용 tick·궤적·위치 오차와 중복을 검증하며 HP·다운·최종 로프 상태를 확정한다.
...
- 체크포인트 아티팩트 선택 중에도 공용 월드 시간·적·투사체·동료는 계속 진행한다. 선택 중인 플레이어의 좌우·확정 입력만 보상 UI가 소비해 이동·점프·로프로 중복 적용하지 않으며, 먼저 선택한 동료는 즉시 다시 움직일 수 있다.
...
- `LocalPlayerPredictor`는 초기 입장과 생명 상태 전환만 서버 상태로 복원한다. 정상 플레이 중 위치·속도·로프는 클라이언트 시뮬레이션을 유지하고 `owner-motion`으로 서버 검증을 받으며, 일반 스냅샷으로 운동 결과를 되감지 않는다.
