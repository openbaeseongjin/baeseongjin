...
- `PlayerCommandBatch` 프로토콜 v4가 목표 틱·플레이어 ID·입력 `sequence`와 협동 `interact` 의도를 정규 정렬하고 JSON 왕복 시 동일한 불변 계약을 유지한다.
