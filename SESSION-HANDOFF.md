...
- 공용 `PlayerMessagePresentation`은 gameplay/Story와 분리된 local message queue·priority·causal dedupe·lifetime·타이핑 진행을 소유한다. message envelope는 `channel/audience/speakerId/text/revealCharactersPerSecond/causalId`를 가져 첫 rollout의 `player-bark/local-player`와 향후 `party-chat/party`를 같은 core로 수용한다. renderer는 해당 speaker의 머리 위 말풍선에서 글자를 순서대로 표시하며, 현재 Bark는 자기 화면에만 표시하고 network snapshot/event에 넣지 않는다. 1-1의 `뭐야…?`·`…일단 위로.`, 1-2의 `…리프트도?`가 각 System Story 뒤 처음 적용되며 문구·trigger·duration·표시 속도·priority는 catalog에서 빠르게 수정한다. party transport·입력 UI·moderation은 후속 결정 전 구현하지 않는다.
...
- **UI lane 검증 ledger:** candidate `7ff38863cd195c6612796ac516780bfd62bef1ce` (base `5c4db080ee015914672aec6bb8a1c4b0ea69cbd8`, Node 24.14.0 / npm ci 환경)에서 `node tests/areaAuthoringV2.mjs`, `node tests/areaEditorDraft.mjs`, `node tests/mapEditorAuthoringServer.mjs`, `node tests/areaPreviewGameApp.mjs`, generated-output check, `npm test`, `npm run format:check`, `git diff --check`가 PASS했다. `npm run check`의 syntax와 AREA-SPEC 18개 검사는 PASS했지만, foundation lane이 이미 만든 scenario integration checkpoint stale(시나리오·authored-area hash)에서 실패한다. UI lane은 Stage content·Sector facade·live Runtime 연결을 바꾸지 않았으므로 marker hash를 갱신하지 않는다. Codex in-app browser가 loopback navigation을 `net::ERR_BLOCKED_BY_CLIENT`로 차단해 실제 Canvas 화면 조작 PASS는 아직 없으며, 정상 로컬 브라우저에서 Draft → Validate → Apply → Preview를 한 번 실행해 확인해야 한다.
