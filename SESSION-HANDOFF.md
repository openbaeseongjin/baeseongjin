...
- 서버는 소유 클라이언트가 만든 플레이어 상태·사건을 검증해 다른 클라이언트에 공유하는 허브다. 소유자의 HP·피격 무적·생명·로프·쿨다운·시간 제한 강화는 서버 snapshot이나 impact receipt로 다시 쓰지 않는다. 서버가 상태를 직접 진행하는 범위는 몹·중립 투사체·공용 월드·세션 수명주기이며, 소유자 전체 복원은 최초 입장·재접속으로 제한한다. 상세 계약은 `docs/multiplayer-synchronization.md`를 따른다.
...
- 이벤트의 즉시 체감과 지속 상태 수렴은 별도 경로다. 플레이어·로프·HP·사망·개별 체크포인트 부활의 수렴 원점은 서버 스냅샷이 아니라 소유·피해 클라이언트의 로컬 `GameSimulation` 결과이며, 서버 복제본과 동료가 검증된 claim·`owner-motion`을 통해 이를 따라간다. 몹·공용 월드의 수렴 원점만 서버 스냅샷이다. 거부 처리는 claim별 계약을 따른다. `owner-motion`·체크포인트처럼 복구 가능한 전이는 기준 상태와 미확정 입력으로 재실행하지만, impact 거부는 이미 인식한 HP·부활·아티팩트·로프와 소비한 적 탄환을 되감지 않는다. 동료와 적은 지연된 두 스냅샷 사이 보간과 제한 외삽으로 각 권한 원점에 수렴한다. 상세 계약은 `docs/multiplayer-synchronization.md`를 따른다.
...
- impact receipt는 피해 클라이언트가 이미 인식한 HP·부활·아티팩트·로프 상태를 되감지 않는다. 거부 receipt도 소비한 적 탄환을 다시 표시하지 않는다. 서버는 인증된 피해자, 서버 탄환 ID·대상과 중복을 검증하되 지연된 서버 위치·궤적 오차로 로컬 충돌을 거절하지 않고, 서버 복제본과 동료가 피해 클라이언트 결과로 수렴하게 공유한다.
...
- 정상 승인 중인 소유자의 로프·HP·생명·위치가 서버 복제 스냅샷과 달라도 소유자 상태를 스냅하지 않는다. 최초 입장·재접속과 `owner-motion` 같은 복구 가능한 명령의 명시적 거부에서만 공유 기준 상태를 사용하며, impact 거부는 HP·부활·로프를 복구하지 않는다.
...
- `OwnerPredictionRuntime`은 소유 `InputDrivenObject` 집합의 입력 이력·예측 tick·claim 수명·복구 가능한 명령의 명시적 거부·표시 보정만 담당한다. impact receipt는 로컬 피해 상태를 복구하지 않는다. 실제 이동·로프·전투 규칙은 capability 믹스인과 시뮬레이션 단계에 두며 이 런타임에 종류별 분기를 추가하지 않는다.
...
- 자기 플레이어의 물리 예측과 Canvas 표시 상태를 분리한다. 정상 승인 중에는 서버 위치·HP·로프·생명 상태로 스냅하지 않는다. `owner-motion` 거부 복구에서만 작은 표시 오차를 감쇠하고 큰 오차를 즉시 복원하며 impact 거부는 표시와 게임 상태를 되감지 않는다.
...
- 현재 폴리곤 표현은 기본 렌더러로 유지하되 시뮬레이션과 분리된 렌더 프로필로 제공한다.
- 기획과 아트 방향이 확정되면 도트 스프라이트·프레임 애니메이션 렌더러를 추가하며, 같은 게임 상태를 폴리곤과 도트 렌더러 중 선택해 그릴 수 있어야 한다.
