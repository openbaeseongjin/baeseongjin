...
- `GameSimulation.addPlayer()`가 첫 플레이어와 협동 플레이어를 같은 팩토리·월드·`players` 배열에 등록하며, 첫 플레이어는 중복 객체 별칭이 아니라 비공개 ID로 식별한다.
- 로프 조준·부착 후보·포인터 전이·입력 버퍼·스윙 드래그·연계 강화 시간은 각 플레이어 엔티티가 독립 소유하고, 기존 GameSimulation 필드는 첫 플레이어 별칭이다.
- `GameSimulation.updatePlayer(playerEntity, command, dt)`는 플레이어별 이동·로프·타이머·아티팩트 무기 효과·자동 발사의 내부 공통 경로다. 싱글·서버·로컬 예측은 각각 `step`, 배치 처리, 공개 예측 명령으로 이 경로를 사용한다.
...
- 멀티 조작 판정은 사용자 체감을 최우선으로 한다. 자기 이동·점프·로프·피격 반응은 클라이언트 운동 상태를 기준으로 하며, 일상적인 서버 스냅샷은 소유 플레이어의 위치·속도·로프를 되감지 않는다. 클라이언트가 운동 상태를 서버에 보내면 서버는 tick·속도·이동 거리 봉투를 검증해 공용 월드에 반영한다. HP·사망·개별 체크포인트 부활 같은 결과는 서버가 최종 확정한다. 동료는 지연된 두 스냅샷 사이 보간 시간축을 사용한다.
...
- 모든 게임플레이 이벤트의 최초 트리거 주체는 클라이언트다. 이동·로프·공격·피격·절단·획득·체크포인트·사망·부활처럼 플레이어가 인식하는 사건은 담당 클라이언트가 즉시 로컬 적용하고 claim을 보낸다. 서버는 새 사건을 체감 경로에서 먼저 시작하지 않고 claim의 소유권·tick·위치·중복을 검증해 공용 결과를 확정·배포한다. 서버가 먼저 보낸 결과를 기다린 뒤 모바일 반응을 시작하는 구현은 금지한다.
