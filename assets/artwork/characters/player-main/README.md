...
| `jump` | 8 | 기존 renderer 회전에 맞춘 타원형 몸 말림. 흰 눈·청록 포인트·붉은 스카프가 45도씩 이동하는 8단계 회전 |
...
- `preview/death-radial-burst-8-review.png`: 죽음 모션 8프레임 확대 검토 보드
...
- 회전 생성 원본: `source/release-spin-v2-8phase-imagegen.png` (1672×941 RGB, OpenAI built-in ImageGen). 4×2 배열의 8단계 회전 시트이며, edge-connected checkerboard를 제거한 RGBA 중간 결과는 `source/release-spin-v2-8phase-transparent.png`에 보존한다.
- 죽음 모션 구도 참고 원본: `source/death-radial-burst-8-imagegen.png` (OpenAI built-in ImageGen)
- 투명화 중간 결과: `source/imagegen-master-sheet-transparent.png`
- 정규화 도구: `source/normalize_generated_sheet.py`, `source/normalize_idle_run_v2.py`, `source/normalize_run_v3.py`, `source/normalize_run_v4.py`, `source/normalize_rope_v2.py`, `source/normalize_rope_v3.py`, `source/normalize_rope_v4.py`, `source/normalize_rope_v5.py`, `source/normalize_release_spin.py`, `source/build_death_radial_burst.py`, Pillow nearest-neighbor
...
캐릭터 회전은 기존 renderer의 물리 각도 적용을 그대로 사용한다. 별도 `release-spin` 상태는 없으며, 기존 상승 `jump` animation이 8개 타원형 프레임을 65ms 간격으로 반복한다. 흰 눈·청록 포인트·붉은 스카프의 위치가 타원 둘레를 45도씩 이동해 48×48 출력에서도 회전 방향을 읽게 한다.
