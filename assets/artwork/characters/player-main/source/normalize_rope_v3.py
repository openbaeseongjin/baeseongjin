...
from normalize_rope_v2 import crop_generated_cells
...
def normalize_rope_frames(cells: list[Image.Image]) -> list[Image.Image]:
    ...
    frames = []
    for cell in cells:
        width = min(22, max(1, round(cell.width * scale * 1.4)))
...
def main() -> None:
    if len(sys.argv) != 4:
        raise SystemExit("usage: normalize_rope_v3.py GENERATED_ROPE_SHEET CURRENT_MASTER PLAYER_MAIN_DIRECTORY")
    ...
    transparent.save(player_directory / "source" / "rope-v3-one-hand-transparent.png")
    ...
    print(f"Normalized one-handed rope v3 into {export_directory}: {len(frames)} total frames")
