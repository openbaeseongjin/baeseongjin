...
- Sector 02 fixed plate의 하단은 Sector 01 far 상단으로, 상단은 Sector 03 fixed plate 하단으로 수렴한다.
- Sector 02의 좌·우 island는 하단과 상단 `512px`에서 alpha가 0으로 수렴한다.
- Sector 03의 좌·우 island는 하단 `512px`에서 alpha가 0으로 수렴한다.
...
- `source/references/`: Sector 01 far, 새 Sector 02 fixed/islands, Sector 03 fixed/islands의 입력 보존본
- `source/build_runtime_seam_layers.py`: 양쪽 접점의 fixed 수렴과 island alpha band를 재현하는 빌더
- `export/sector-02-worker-district/`: Sector 01·03 양쪽 접점이 반영된 Sector 02 Runtime PNG
...
- Sector 02 fixed 상단 행 = Sector 03 fixed 하단 행
- Sector 02 좌·우 island의 상단·하단 경계 alpha max = `0`
