...
- Runtime center: authored `shuttlePosition` `(4460, -1330)`
- Authoring anchor: `center` (`shuttlePosition`이 Runtime 오브젝트 중심을 소유)
...
- 셔틀의 불투명 영역은 캔버스 안에 있고 캔버스 중심이 Runtime `shuttlePosition`에 대응해야 한다.
...
- `boarding` 상태는 sprite를 center anchor로 그리며 `hidden` 상태는 계속 렌더하지 않는다.
- 준비 전 또는 로드 실패 시 기존 `MaintenanceShuttleRenderer` Canvas 표현으로 독립 복구한다.

## Non-scope / integration needs

- 이 Runtime 연결은 Shuttle 위치·크기·탑승 영역·게이트/카메라 승리 타임라인·충돌·물리·네트워크 권한을 변경하지 않는다.
