...
1. 그래픽 담당자는 이 폴더의 `frame-map.png`, `locomotion.png`, `actions.png`를 배치 참고로 사용하고 결과를 `assets/artwork/characters/player-main/`에 납품합니다.
2. 담당 개발자는 이 폴더를 `assets/runtime/characters/player-main/`으로 복사하고 납품된 PNG를 배치합니다.
3. 각 24×24 cell 사이에 padding, gap 또는 bleed를 넣지 않고 투명 배경을 유지합니다.
4. 프레임 수, 순서, atlas 크기를 바꾼 경우 `sprite-manifest.json`의 atlas `size`, animation `cell`, `durationMs`도 함께 고칩니다.
5. 기본 원본 방향은 오른쪽입니다. 왼쪽 방향은 renderer가 `flipX`로 표시하므로 별도 좌향 atlas가 필수는 아닙니다.
6. 개발 연결 후 다음 명령을 통과시킵니다.

```powershell
npm run validate:sprite-assets -- assets/runtime/characters/player-main
```

`sprite-manifest.json`의 `$schema`는 `../sprite-manifest.schema.json`이므로 `player-main/`으로 복사한 뒤에도 유효합니다. collider, hitbox, 물리 값 또는 network 상태는 manifest에 넣지 않습니다. 그런 값은 스프라이트와 독립된 게임 런타임 조립이 소유합니다.
...
- [`../player-mock/player-action-mock.svg`](../player-mock/player-action-mock.svg)는 현재 런타임이 직접 읽는 개발용 mock이며, 각 동작 의미와 pose의 원본입니다.
...
- [`../fixtures/player-multi-atlas`](../fixtures/player-multi-atlas)는 schema와 validator 회귀를 위한 결정적 fixture일 뿐 그래픽 품질이나 최종 pose의 참고 자료가 아닙니다.
