...
Sector 01~06의 `story-display`는 같은 정적 정보 패널 외형을 사용한다. 기존 authored position과 `coordinateAnchor`가 배치를 소유하며 이 package는 collision, interaction, 문구, cue 또는 network 상태를 소유하지 않는다.
...
- sprite와 fallback은 같은 `world-object:story-display` bounds와 authored anchor를 사용한다.
