...
- `CompositeBossEncounterRuntime`은 Sector 진행과 분리된 중립 월드 Has-A 상태다. attempt·공통 HP·Player별 active/spectating과 직접 Boss 처치당 최대 체력 이내 100 회복을 소유하고 top-level `bossRuntime` snapshot으로 복제한다. Boss별 Runtime은 별도 회복 규칙을 만들지 않으며 한 번 완료한 Boss ID만 `SectorProgressState`에 영속한다. `GameSimulation`은 활성 Stage 선택, 공개 명령과 event 연결만 조정한다.
...
- `WorldSnapshot` protocol v20은 v19 상태에 active combat interaction snapshot을 추가한다. Boss03 capture의 definition·source·target·phase·시작/목표 위치만 복제하며 정상 owner 결과를 Player 전체 상태로 반복 전송하지 않는다.
...
- 투사체와 같은 예측 가능한 객체는 위치를 계속 전송하지 않고 권위 `spawn` 이벤트의 시작 틱·초기 상태로 각 실행 환경에서 진행한다. 서버 `CombatSystems`와 클라이언트 `PredictableProjectileStore`는 단일 `projectile-motion` capability를 실행하고, 공통 `PhysicsMixin`이 Player·Enemy와 같은 적분을 소유하며 Projectile은 lifetime mixin과 선택적 Homing steering만 추가한다. `objectType`은 진영, `motionKind`는 ballistic/homing, `visualPresetId`는 표현을 독립 선택한다. Boss06의 5발 미사일은 enemy+homing 구체 조합이며 초기 fan velocity와 제한 turn rate를 spawn 사건에 담아 서버·클라이언트가 같은 곡선을 재생한다. 저장소·서버·구체 class에 별도 궤적 공식을 두지 않는다. 서로 다른 projectile ID의 swept contact는 frame 전체 quota로 제한하지 않고 각 객체가 한 번씩 소비하므로 지연 catch-up에서 한 미사일이 다른 미사일의 접촉 segment를 소실시키지 않는다. 충돌·claim 확정·수명 만료만 `resolve` 이벤트로 확정하고 원래 spawn 이벤트는 활성 객체에 보존해 중간 입장 welcome에서만 같은 ID로 재전송한다.
...
- `SpellRuntimeState`는 네 커맨드 슬롯·독립 쿨다운·charge를 소유하고 `SpellEffectState`와 `AugmentPassiveState`가 지속 버프·추진·Rope 회복·2단 점프를 조합한다. 각 `CombatSpellDefinition` 구체 클래스가 projectile·area·self effect·dash cast를 override한다. `SpellProjectileState`와 `SpellAreaState`는 Player·Enemy·Boss를 같은 impact target 계약으로 처리하고 시전자는 기본 제외한다. `CombatStatusEffectPool`은 기본 감전·고전압 감전·점화·냉동 lifecycle과 pulse를 소유한다. 경험치 요구량·상한·사망 손실은 `ExperienceProgressionDefinition`이 등비 곡선에서 계산한다.
