...
브라우저 Canvas에서 실행되는 2D 로프 액션 프로토타입이다. 고정 길이 로프 물리, Map Editor가 저작한 48개 Stage와 개별 Player portal, 적 전투·투사체, 최근 도달 Stage 체크포인트 복귀와 generic Augment 런 성장 상태를 공용 시뮬레이션에서 처리한다. PC와 모바일은 입력 방식만 다르고 게임 규칙은 공유한다.
...
- `AuthoredSectorCatalog`는 build/startup source이고, `AuthoredSeamlessSectorRuntime`은 Sector 01~~06 generated Stage를 서로 보이지 않고 충돌하지 않는 격리 위치에 배치한다. landmark bounds는 각 `AREA-SPEC.v2.json`의 authored bounds와 같고, surface·world object도 authored 항목만 좌표 이동해 사용한다. compiler가 city wing·seam·boundary·transit collision을 만들지 않는다. 3-8·4-8·5-8·6-8은 다음 Stage portal이 없는 content boundary다.
...
- `BossEncounterRuntime`은 Sector 진행과 분리된 중립 월드 Has-A 상태다. attempt·phase HP·약점 노출·Player별 active/spectating을 top-level `bossRuntime` snapshot으로 복제하고 `GameSimulation`은 활성 Stage 선택, 공개 명령과 event 연결만 조정한다.
- 0.59.0 default `GameSimulationFactory`는 `authored-stage-portal-runtime-v14-six-boss-stage`만 현재 authored world revision으로 해석한다. Sector 01~06의 48개 canonical v2 Stage와 Boss01~06 generated catalog를 production world에 포함한다. 일반 Stage surface·wall·world object는 Map Editor source만 사용하고 Runtime derived geometry는 0개다. 일반 Gate를 통과한 Player 한 명만 다음 authored Entry로 이동하며 다른 Player·Enemy·공용 진행을 함께 이동시키지 않는다.
- 여섯 Sector의 8번째 Stage는 일반 Stage portal 대신 각 Boss Stage entry를 연다. Boss01~05는 승리 후 명시된 다음 landmark로 portal transition하고, Boss06은 target landmark가 없는 terminal 계약으로 Gate/Bridge/Shuttle과 Player별 Boarding readiness를 진행해 모든 연결 참가자가 직접 boarding zone에 도달했을 때만 run completion을 연다. 첫 Boarding Player의 party teleport와 regular `6-9`는 없다.
- `WorldSnapshot` protocol v12는 공용 Sector 진행, 활성 Boss Stage와 terminal readiness, Player별 `respawnAnchorId`, content boundary 이력, Hardpoint Jammer target/phase, non-null Action state와 Enemy 동적 상태를 전송한다. 현재 Stage ID는 추가하지 않고 Player 위치가 현재 authored Stage를 나타낸다. Access 조건은 portal 사용 가능 여부만 바꾸며 collision·barrier geometry를 생성하거나 제거하지 않는다. Timer/Purge는 HOLD다.
...
- `createAuthoredSeamlessSectorRuntimeWorld()`가 Sector 01~~06의 generated Stage 48개를 authored bounds 그대로 격리된 48 landmark와 개별 Player portal 44개로 조립한다. content boundary 뒤에는 portal을 만들지 않는다.
...
- 현재 48개 Runtime Stage 중 `3-8`·`4-8`·`5-8`·`6-8`은 각각 stable boundary ID와 required objective metadata를 가진 독립 content boundary다. 어느 경계도 다음 Sector connector·route lock·transition이나 `completed` 전체 게임 종료로 판정하지 않으며, 마지막 authored Area는 `sector-06-08`이다.
