...
- 0.29.1 default `GameSimulationFactory`는 `seamless-sector-runtime-v2`를 사용한다. `WorldSnapshot` protocol v8은 기존 Sector 진행 계약과 Player별 증강·Action/knockback state를 함께 보낸다. legacy Area revision은 explicit compatibility factory로만 생성한다.
