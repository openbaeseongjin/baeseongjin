...
- 사망 표현은 시뮬레이션의 즉시 부활을 지연시키지 않는다. `player-respawned` 사건이 사망 직전 위치와 부활 위치를 함께 전달하고, renderer는 manifest의 `death` frame duration 합계 동안 사망 위치를 그린 뒤 부활 위치의 `respawn`으로 전환한다. 로컬 카메라는 같은 시간 동안 사망 순간 transform을 유지하고 종료 시 부활 위치로 즉시 컷한다. 낙사 위치가 viewport 밖이면 캐릭터 출력 크기만큼 안쪽의 마지막 화면 가장자리로 clamp하며, 원격 플레이어 사망은 로컬 카메라를 고정하지 않는다.
...
- 현재 authored Runtime의 배경·지형 테마는 공용 진행 상태가 아니라 scene의 로컬 플레이어 물리 위치가 포함되거나 수직으로 가장 가까운 `landmark.bounds`를 찾아 그 landmark의 `sectorId`로 선택한다. `SectorProgressState.currentSectorId`는 진행 권위일 뿐 배경 선택 입력이 아니므로, 진행이 전진한 뒤 플레이어가 이전 섹터 높이로 낙하해도 각 클라이언트 화면은 자기 위치의 섹터 표현으로 즉시 돌아간다.
...
- 기본 Runtime의 저작 진행 권위는 `currentSectorId/currentLandmarkId`다. objective 완료는 `requiredObjectiveIds`를 만족한 route lock의 physical connector collision을 활성화하고, 플레이어 원형 collider가 다음 landmark의 `respawnAnchor.triggerBounds`와 겹칠 때 진행과 저장을 함께 확정한다. `requiredRouteId` surface는 `WorldGateGeometry.isSurfaceEnabledForProgress()` 하나로 collision과 polygon/pixel terrain renderer가 함께 필터해 잠긴 통로를 밟을 수 있는 지형으로 그리지 않는다. 세로 landmark 층의 실제 collision surface는 유지한다. `areaId`, 임의의 entry square, Gate aperture, `gate-crossed`, `gate-portal-entered`, `activeCheckpointId`와 legacy Stage boundary wall은 explicit Area revision의 compatibility 계약에만 남는다.
...
- 현재 저작 시나리오는 objective·route unlock·물리적 landmark entry로 진행하고 `sector-03:landmark:08`에서 content boundary에 머문다. 과거 Area/Gate와 절차 월드 summit claim은 호환 코드로만 남아 있으며 최종 시나리오의 엔딩 진입 조건이 확정되기 전까지 기본 제품 테스트 계약으로 사용하지 않는다.
...
- `EnemyArchetypeCatalog`는 영어 Runtime ID와 한글 표시 이름, Has-A behavior 조립 및 각 archetype의 공개 behavior kind/state 목록을 소유한다. `EnemyStateCatalog`는 공통 사격과 Pursuit·Shield·Artillery·Support·Swarm의 상태·허용 전이를 한 번 정의하고, 시간 기반 축은 순수 `StateMachine`을 내부 소유하는 `TimedStateController`를 재사용한다. `GameSimulation`은 capability 단계와 outcome 연결만 조정하며 기존 `attackState/attackStateRemaining/behaviorState` snapshot 계약을 유지한다. 포격 strike는 기존 `BallisticProjectileObject` 중립 spawn 경계를 사용하고, 방패는 `EnemyObject.blocksImpactFrom()` 계약을 통해 #611 Rope 충돌 prediction·claim 양쪽에서 같은 방향 판정을 사용한다.
- `EnemyPresentationState`는 그래픽 담당자가 소비하는 단일 적 상태 인계 경계다. 타입별 지원 상태를 공개하고 `knockback → active behavior → attack → persistent stance/patrol → idle` 순서로 `primaryState`를 결정하며 원래의 `attackState`·`behaviorState` 축도 함께 보존한다. Sprite renderer는 presentation resolver와 sprite resolver를 주입받아 현재 정적 mock에서 향후 타입·상태별 clip으로 교체할 수 있다. 표현 상태나 frame index를 게임·네트워크 권위 snapshot에 추가하지 않는다.
...
- `SectorProgressState`는 `currentSectorId/currentLandmarkId`, objective·route·encounter·`collectedAccessModuleIds` 같은 공용 진행만 소유한다. Player별 `respawnAnchorId`는 Player runtime의 별도 체크포인트 상태가 소유하며 다른 Player 접촉이나 공용 landmark 진행으로 덮어쓰지 않는다. Timer·Purge가 미정인 현재 사망 인원 수에서 공용 Sector reset을 추론하지 않는다.
- 0.32.0 default `GameSimulationFactory`는 `seamless-sector-runtime-v4`를 사용한다. Sector 01은 Stage-local 1-3·1-6·1-7 Carrier encounter가 주는 공용 Access Module 중 아무 2개와 1-8 objective를 Sector transition 조건으로 사용한다. Sector 01~03 enemy slot은 Stage-local 위치·activation·수량을 보존하고 `enemySelection.allowedEnemyTypes`가 있을 때 type만 `slotId + runSeed + worldRevision`으로 결정한다. `WorldSnapshot` protocol v10은 공용 Sector 진행과 Player별 `respawnAnchorId`를 함께 보내되 authored enemy 정적 정의는 `worldRevision + objectId`로 복원하고 동적 상태만 20Hz로 전송한다. legacy Area revision은 explicit compatibility factory로만 생성한다.
- intra-Sector route는 objective 완료로 connector collision surface를 활성화하고 플레이어가 다음 Stage 세이브 구조물에 물리적으로 닿으면 진행한다. 수평 connector는 양쪽 authored deck 사이 실제 빈 구간만 같은 deck 높이로 채우며, deck이 이미 닿거나 겹치면 surface를 만들지 않는다. anchor 중심 전체를 잇는 큰 platform은 금지한다. Stage별 Gate·exit panel·포탈·문 visual은 만들지 않으며 위치를 순간이동하거나 Gate portal event를 만들지 않는다.
...
- `npm test`의 current authored world 검증은 Sector compiler·objective route·Stage 세이브 포인트 respawn·party-wipe Sector-entry baseline·마지막 content boundary를 확인한다. legacy Area/Gate tests는 이전 revision compatibility를 별도로 검증한다.
