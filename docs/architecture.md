...
- authored 환경 package 선택은 `AuthoredAreaEnvironmentCatalog`의 stable Area ID가 소유한다. 현재 Sector 01→02, 02→03, 03→04 package 경계 전환은 `PixelBackdropRenderer`가 Player world Y에서 파생한 단일 비율로만 두 backdrop package 전체와 sky를 교차 합성하며 simulation, camera, collision, gameplay connector 또는 network state를 추가하지 않는다. Boss Stage의 환경 Area override는 해당 Stage status가 `active`일 때만 현재 Area보다 우선한다.
...
- `validateProductionMapParity.mjs`는 이 권위를 `npm run check`에서 재현한다. 48개 generated Runtime Stage와 0개 scenario-only Stage, Stage별 authored/derived/hidden/progress-gated surface와 generated 의미 동등성, 중복·퇴화 geometry, seam source/target과 content-boundary 무연결, Access source·Jammer·proof 소유권, Sector 06 기존 시스템 recall, 금지 legacy/provenance key와 REV design Runtime import를 drift gate로 검사한다.
...
- `AuthoredSectorCatalog`는 build/startup source이고, `AuthoredSeamlessSectorRuntime`은 Sector 01~~06 generated Stage를 각 Sector local 좌표에서 `1 → 8` 세로 stack으로 compile한다. 각 landmark는 4,800px width와 실제 lateral city wing을 가지며 compiler는 local origin/bounds와 Sector world origin을 분리한다. 3-8·4-8·5-8·6-8은 다음 stack과 직접 잇지 않는 content boundary라 future Boss room과 Sector transition은 별도 계약으로만 추가한다.
...
- Boss는 일반 Stage와 분리된 `BossStageSpec v2 → generated BossStageDefinition → BossEncounterRuntime` 저작·실행 계약을 사용한다. World는 여러 Boss Stage definition을 catalog로 보유하지만 진행상 하나만 활성화하고 snapshot의 Stage ID로 concrete mechanism을 선택·복원한다. Definition은 Arena·Phase Zone·mechanic·HUD·transition, Phase HP scaling, 닫힌 몸체 `0.25`와 약점 고정 `0.25`를 소유한다. `GateLockingCarriageRuntime`은 Boss01의 Rail/Beam/Ram을, `ResidentialSecurityPursuitRuntime`은 Boss02의 추격·방향 고정·구조물 충돌·약점 노출을 각각 구현한다. 두 구현 모두 Player 충돌·Renderer·hazard가 같은 kinematic body와 presentation snapshot을 사용하며, 약점은 일반 피해 100%와 scaled Phase HP 25%를 한 causal impact에서 받는다. 구조물 충돌은 geometry를 바꾸지 않고 Boss02 vulnerability만 연다. Timer·collapse와 최종 asset은 범위 밖이다.
...
- 0.57.0 default `GameSimulationFactory`는 `seamless-sector-runtime-v12-multi-boss-stage`만 현재 authored world revision으로 해석한다. Sector 01~06의 48개 canonical v2 Stage가 generated catalog를 통해 production world에 포함되고 과거 flat Area catalog revision은 지원하지 않는다. Sector 01~03은 기존 enemy-defeat 3-of-3 Access source를 사용하며 1-8→Boss01→2-1과 2-8→Boss02→3-1을 요구한다. Sector 04는 세 `objective-completion` Resident Security Override 중 두 개를 요구하지만 4-8 뒤 transition을 만들지 않으며, Sector 05~06은 Access Module을 두지 않는다. 3-8·4-8·5-8·6-8은 manifest·landmark·progress snapshot이 같은 boundary ID와 필수 objective를 소유하고 다음 Sector stack은 기본 Rope reach 밖에 물리적으로 격리한다. 선택 Stage Preview는 앞 boundary 이력만 완료하고 현재 boundary 상태는 초기화한다. Sector 05 Hardpoint Jammer target/phase는 서버 snapshot 권위이며 owner prediction은 이를 소비할 뿐 로컬 Player attachment 집합으로 새 target을 선택하지 않는다. Sector 06은 Wind·Scanner·Standard Sentry·Patrol·Cutter의 기존 시스템만 재조합한다. Boss Map Editor Preview는 production authored world에 전용 Boss arena definition을 조합한다. `WorldSnapshot` protocol v12는 공용 Sector 진행, 활성 Boss Stage, Player별 `respawnAnchorId`, content boundary 이력, Hardpoint Jammer target/phase, non-null Action state와 Enemy 동적 상태를 20Hz로 전송하며 authored Enemy 정적 정의는 `worldRevision + objectId`로 복원한다. Timer/Purge는 mapping HOLD라 현재 snapshot과 gameplay state에 추가하지 않는다.
...
- `createAuthoredSeamlessSectorRuntimeWorld()`가 Sector 01~~06의 generated Stage 48개를 여섯 4,800px Sector의 48 landmark로 조립한다. Sector 03 이후의 각 content boundary는 독립 stack 끝으로 유지해 미확정 Boss·Sector 전환을 암묵적으로 만들지 않는다.
