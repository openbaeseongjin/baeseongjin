...
- `npm test`의 current authored world 검증은 정적 Sector surface, 독립 objective, Player별 savepoint respawn, 전원 사망 진행 보존과 마지막 content boundary를 확인한다.
...
- Access 위치 안내는 authored 문자열을 사용하지 않는다. `CanvasRenderer`가 local Player의 현재 Sector에서 첫 미수집 module world position을 screen-space로 투영하고, viewport 밖일 때만 safe-area 경계와 만나는 무문자 화살표를 그린다. 좌측 화살표는 고정 HUD 아래로, 상단 화살표는 설정/HUD 버튼 오른쪽으로 밀고 모바일은 하단 조작 영역을 safe inset으로 제외한다. 대상이 viewport 안에 들어오면 edge arrow를 끄고 world-space `AccessModuleSignalRenderer`의 무문자 다이아몬드 표식만 사용한다. guide는 고정 HUD와 같은 로컬 표시 토글을 따른다.
