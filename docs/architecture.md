...
- Boss는 `BossStageSpec v2 → generated BossStageDefinition → CompositeBossEncounterRuntime` 계약을 사용하며 product catalog가 Boss03·06을 보유한다. `BossStatePool`은 고정 definition catalog와 결정적 가중 선택을 공통 소유하고 각 Boss의 concrete State class가 predicate·enter·advance를 구현한다. Boss03의 `CombatInteractionController`는 `ActorCaptureInteractionState`를 동적 interaction ID별로 보관해 Player·Boss가 내부 필드를 직접 소유하지 않게 한다.
