...
- 오디오와 스프라이트 runtime package는 안정적인 asset ID와 주입 가능한 definition 선택 경계를 통해 조립한다. 향후 디버그 모드가 package를 교체해 비교할 수 있어야 하며 기본 catalog나 scene/audio host가 특정 mock ID에 영구 결합하지 않는다. 첫 기반은 선택 UI보다 서로 다른 가짜 package를 주입하는 회귀 테스트로 교체 가능성을 증명한다. package 선택은 표현 계층에만 영향을 주고 물리·충돌·전투·네트워크 상태를 바꾸지 않는다.
- 오디오 runtime manifest는 브라우저 파일과 load·loop 계약을 가진 `clips`, 볼륨 그룹·변형·동시발음 같은 표현 정책을 가진 `cues`까지만 소유한다. 게임 사건·읽기 전용 상태를 cue ID에 연결하는 `AudioEventBindings`는 package 밖의 클라이언트 조립 경계이며, gameplay·simulation 객체와 asset manifest는 서로의 구체 이름이나 파일 경로를 import하지 않는다.
...
- audio lifecycle host는 `document.hidden`과 `pagehide`에서 AudioContext·media stream을 일시정지하고 one-shot voice를 폐기한다. window `blur`만으로는 오디오를 정지하지 않는다. 문서 복귀는 누락된 과거 사건을 replay하지 않고 최신 읽기 전용 scene state에서 필요한 BGM·ambience·loop lifecycle을 재조정한다. `resume()`이 사용자 활성화 제약으로 실패하면 pending 상태를 보존해 다음 사용자 동작에서 재시도하고 UI에는 작은 재개 안내만 노출한다.
...
- audio readiness는 `loading|ready|degraded|suspended|failed`의 읽기 전용 진단 상태를 제공한다. 일반 시작 UI는 진행률과 필수 실패의 원인·재시도·메뉴 복귀만, 오디오 설정 탭은 선택 실패가 있을 때 `일부 음원 사용 불가` 요약만 표시한다. `?metrics=1` 및 진단 복사는 pack/package ID, AudioContext 상태, clip별 선택 source·MIME·실패 코드, required/optional 준비 수, 그룹별 활성 voice 수와 cooldown drop·voice stealing 누계를 소비한다. 진단 값은 mixer·gameplay 정책을 자동 변경하지 않는다.
