...
- 로프 스윙처럼 입력 capability가 플레이어의 지속 전투 상태도 바꾸면 클라이언트 예측과 서버 확정이 같은 `GameSimulation` 도메인 메서드를 사용한다. 소유 클라이언트는 즉시 강화하고, 서버는 별도 swing claim 검증 뒤 강화 시간과 무기 파라미터를 스냅샷에 포함한다. 같은 틱 자동 발사는 swing claim 뒤에 확정해 양쪽 피해량 계산의 원인이 일치한다.
- 예측 객체의 생성과 그 입력이 바꾸는 소유자 상태는 하나의 prediction ID 수명주기로 관리한다. 자동 발사는 탄환과 무기 쿨다운을 함께 예측하며, 거부 receipt는 표시 객체만 지우지 않고 발사 이전 쿨다운도 복구한다. claim 대기 상태는 이전 권위 스냅샷이 덮지 않고 receipt 이후 최신 스냅샷에 수렴한다.
- 피해 예측은 projectile ID를 수명주기 키로 사용한다. `OwnerPredictionRuntime`은 피격 직전 `GameSimulation.playerState()`와 tick·발생 순서를 예측 journal에 보존하고, 거부 receipt에서 그 상태를 공개 복원 명령으로 되돌린 뒤 입력 이력과 시간상 뒤에 남은 impact를 원래 tick 순서로 재실행한다. 재적용 직전에 각 후속 impact의 복구 기준도 갱신한다. 따라서 몸체 넉백·무적과 로프 절단·비활성 시간이 중첩돼도 같은 원인 단위로 복구되며, 전송 계층이 물리 역연산을 따로 구현하지 않는다.
