...
- 기본 Runtime은 authored Exit Panel의 `W/↑` 상호작용과 Gate trigger를 개별 Player forward portal의 유일한 Stage 전환 입력으로 사용한다. 모든 Stage는 Entry·Exit와 Exit Panel을 각각 하나만 소유하며 출구용 도달 objective를 만들지 않는다. Gate는 공간 이동과 Rope 분리만 수행하며 무기 cooldown·Rope disable·Augment runtime을 respawn처럼 초기화하지 않는다. Player collider와 `respawnAnchor.triggerBounds`의 겹침은 개인 checkpoint만 갱신한다. 현재 Stage·Sector는 Player 위치에서 파생하며 별도 상태를 저장하지 않는다. 모든 Area surface는 Run 시작부터 collision과 renderer가 공유하는 authored 정적 집합이고 route 진행으로 생성·제거하지 않는다.
...
- 표시 시간이 있는 objective는 `SectorProgressState.activeObjectiveSequences`의 공용 진행으로 시작·진행·완료한다. 단, 출구 objective는 표시 시간을 소유하지 않는다. 1-1 Exit Panel은 W 입력 tick에 objective와 outbound route를 확정하고, Direction이 완료 사건에서 0/0.9/1.8초 Terminal 문구를 별도 재생한다. Story 문구 중에도 이동과 출구 진행을 허용한다.
...
- `PlayerCommand.interact`는 augment Node·terminal·authored Exit Panel 같은 근접 문맥 상호작용 의도다. `InputSampler`는 PC `W/↑`와 모바일 점프 버튼을 점프 축과 `interact`에 함께 매핑하며, 진행 시스템은 명시적 source의 반경 안에서 새 `interactSequence`를 한 번만 소비한다. Key/Access가 준비되면 Exit Panel objective가 열리고 실제 trigger를 통과한 Player만 portal 이동한다. Key가 부족하면 objective를 완료하지 않고 수량 경고만 표시한다.
