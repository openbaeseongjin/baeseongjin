...
- Boss는 일반 Stage와 분리된 `BossStageSpec v2 → generated BossStageDefinition → CompositeBossEncounterRuntime` 저작·실행 계약을 사용한다. World는 terminal Boss06 definition 하나만 catalog로 보유하고 snapshot의 Stage ID로 concrete Runtime을 선택·복원한다. 공용 Runtime은 참가 상태 enum 검증·hazard contact 멱등 조회·ID 순서 respawn·승리 spectator 복귀를 소유한다. Boss06 Continuity Warden은 body ImpactTarget 하나와 96×150 Polygon body를 공유하며 Guard/Counter를 공격자 충돌 위치와 facing으로 분류한다. V3 점프는 authored Main/Ledge landing point를 소비하는 `ContinuityWardenJumpMotion` Has-A 컴포넌트가 공용 Physics acceleration 적분으로 진행하고, gameplay attack state와 `takeoff/jump/fall/landing` 표현 상태를 분리한다. 소환 패턴은 authored 좌우 spawn point에서 이동형 공격 Enemy 2개를 서버 Enemy manager에 등록하며 15초 cooldown과 live 6개 skip을 Boss snapshot으로 복원한다. 표현 telegraph와 실제 판정은 같은 hazard bounds를 사용하고 움직이는 body 접촉은 Polygon collider로 판정한다. 지원되지 않는 mechanic은 generic fallback으로 실행하지 않고 factory에서 거부한다.
...
- `WorldSnapshot` protocol v17은 Boss 완료 이력을 포함한 공용 Sector 진행, 활성 Boss Stage와 Rope attachment actor transform, terminal readiness, Player별 `respawnAnchorId`·상태이상 Pool·Spell 슬롯/쿨다운/charge/area/Passive·경험치, content boundary 이력, Hardpoint Jammer target/phase/sequence와 Enemy 동적 상태를 전송하며 피격 후 무적 타이머는 포함하지 않는다. `owner-motion` v11은 Rope 표면과 Player 상태를 소유자 결과로 공유한다. 현재 Stage ID는 추가하지 않고 Player 위치가 현재 authored Stage를 나타낸다.
