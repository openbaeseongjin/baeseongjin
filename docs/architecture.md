...
- `EnemyEncounterSelection`은 topology 독립 stable slot의 고정 선택 또는 허용 pool을 검증한다. pool 결과는 `slotId + run seed + world revision`으로 결정하고 position·activation·slot ID를 다시 저작하지 않는다. Map Editor는 Enemy 공통 필드와 분리된 definition 기반 `추가 정보` 패널에서 `swarmMemberCount` 2~20을 저작한다. Runtime은 Swarm이 선택됐을 때만 authored 중심·activation을 유지한 stable member ID들로 확장한다.
