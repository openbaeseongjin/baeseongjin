...
- Boss는 일반 Stage와 분리된 `BossStageSpec v2 → generated BossStageDefinition → CompositeBossEncounterRuntime` 저작·실행 계약을 사용한다. World는 Boss03·06 definition만 catalog로 보유하고 진행상 하나만 활성화하며 snapshot의 Stage ID로 concrete Runtime을 선택·복원한다. Boss03 Scanner/Arm은 신규 이동형 보스몹 기획 전까지의 임시 Runtime이고, Boss06 Continuity Warden은 body ImpactTarget 하나와 96×150 Polygon body를 공유하며 Guard/Counter를 공격자 충돌 위치와 facing으로 분류한다. 지원되지 않는 mechanic은 generic fallback으로 실행하지 않고 factory에서 거부한다.
