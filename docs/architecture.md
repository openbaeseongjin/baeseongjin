...
- `validateProductionMapParity.mjs`는 이 권위를 `npm run check`에서 재현한다. 24개 generated Runtime Stage와 24개 scenario-only Stage 분리, Stage별 authored/derived/hidden/progress-gated surface와 mismatch ID, 중복·퇴화 geometry, seam source/target 소유권, Sector 03 stable ID·생성 의미 동등성, 금지 legacy/provenance key와 REV design Runtime import를 drift gate로 검사한다.
...
- `AuthoredSectorCatalog`는 build/startup source이고, `AuthoredSeamlessSectorRuntime`은 Sector 01~~03 generated Stage를 각 Sector local 좌표에서 `1 → 8` 세로 stack으로 compile한다. 각 landmark는 4,800px width와 실제 lateral city wing을 가지며, compiler는 local origin/bounds와 Sector world origin을 분리한다. future Boss room은 transition slot의 rise를 바꾸어 끼우므로 downstream landmark local origin·ID·콘텐츠는 유지한다. Sector 04~~06은 scenario-only identity이며 Runtime output에 넣지 않는다.
...
- 0.56.0 default `GameSimulationFactory`는 `seamless-sector-runtime-v11-multi-boss-stage`만 현재 authored world revision으로 해석한다. 과거 flat Area catalog revision은 production resolution에서 제거했다. Sector 높이 예산은 Sector 01·02가 각각 9,600px, Sector 03이 승인된 12,416px이며 현재 Sector 03 compiled 높이 12,214px를 수용하되 추가 성장을 제한한다. Access topology는 Sector별 공용 Module source 3개와 다음 Sector transition의 `3-of-3`을 사용하며 Boss Stage가 있는 경계는 Access barrier 해제 뒤 Boss 승리를 추가로 요구한다. `WorldSnapshot` protocol v12는 공용 Sector 진행, 활성 Boss Stage snapshot, Player별 `respawnAnchorId`, non-null Action state와 Enemy 동적 상태를 20Hz로 전송한다.
...
- `createAuthoredSeamlessSectorRuntimeWorld()`가 Sector 01·02·03의 generated Stage 24개를 3개 4,800px Sector의 24 landmark로 조립한다. Sector 04~~06 canonical v2는 scenario-only 상태이며 production world에 포함되지 않는다.
