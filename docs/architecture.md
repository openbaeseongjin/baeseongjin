...
- 모든 Enemy는 공통 `enemy-physics` capability로 surface 적분을 끝낸다. Projectile 공격을 사용하는 Enemy만 `EnemyWeaponState` Has-A 컴포넌트와 `enemy-weapon` capability를 추가 조합하며, 무기 없는 Artillery·Support·Swarm은 Runtime type 분기 없이 weapon capability 자체가 없다. `EnemyWeaponState`는 locked target·aim·fire cooldown·기존 attack snapshot을 소유하고 Idle·Acquire·Track·Lock·Fire·Cooldown 구체 상태 클래스가 각 `advance()`와 다음 전이를 override한다. `EnemyArchetypeCatalog`는 영어 Runtime ID와 한글 표시 이름, weapon capability 여부와 Has-A behavior 조립 및 각 archetype의 공개 behavior kind/state 목록을 소유한다. `EnemyStateCatalog`는 공통 사격과 Pursuit·Shield·Artillery·Support·Swarm의 허용 전이를 한 번 정의하고 시간 기반 축은 `TimedStateController`를 재사용한다. `GameSimulation`은 behavior event 종류의 고정 lookup으로 피해 대상을 연결할 뿐 구체 몹 타입을 분기하지 않는다. Support는 자기 HP를 1:3 치료 자원으로 쓰고 초당 2를 재생하며, 포격 strike는 투사체를 만들지 않는 1회 영역 판정이고 Swarm contact는 개별 body 충돌 뒤 한 번 확정된다. 방패는 `EnemyObject.blocksImpactFrom()` 계약을 통해 #611 Rope 충돌 prediction·claim 양쪽에서 같은 방향 판정을 사용한다.
