...
- 피해 예측은 projectile ID를 수명주기 키로 사용한다. `OwnerPredictionRuntime`은 피격 직전 `GameSimulation.playerState()`와 tick을 보존하고, 거부 receipt에서 그 상태를 공개 복원 명령으로 되돌린 뒤 입력 이력을 재실행한다. 따라서 몸체 넉백·무적과 로프 절단·비활성 시간이 같은 원인 단위로 복구되며, 전송 계층이 물리 역연산을 따로 구현하지 않는다.
