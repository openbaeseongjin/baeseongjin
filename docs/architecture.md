...
- `AuthoredSectorCatalog`는 build/startup source이고, `AuthoredSeamlessSectorRuntime`은 Sector 01~~06 generated Stage를 `target.bounds.bottom === source.bounds.top`인 하나의 연속 수직 월드로 배치한다. landmark bounds는 각 `AREA-SPEC.v2.json`의 authored bounds와 같고, surface·world object도 authored 항목만 좌표 이동해 사용한다. compiler가 city wing·seam·boundary·transit collision을 만들지 않는다. 1-8·2-8·4-8·5-8의 outbound route는 다음 Sector Entry로 직접 이어지고, 3-8은 Boss03 entry·완료 뒤 4-1 직행을 공유하며 6-8은 terminal Boss06만 연다.
...
- Boss는 일반 Stage와 분리된 `BossStageSpec v2 → generated BossStageDefinition → CompositeBossEncounterRuntime` 저작·실행 계약을 사용한다. World는 Boss03·06 definition만 catalog로 보유하고 진행상 하나만 활성화하며 snapshot의 Stage ID로 concrete Runtime을 선택·복원한다. 공용 Runtime은 참가 상태 enum 검증·hazard contact 멱등 조회·ID 순서 respawn·승리 spectator 복귀를 소유한다. Boss03 Scanner/Arm은 신규 이동형 보스몹 기획 전까지의 임시 Runtime이고, Boss06 Continuity Warden은 body ImpactTarget 하나와 96×150 Polygon body를 공유하며 Guard/Counter를 공격자 충돌 위치와 facing으로 분류한다. 표현 telegraph와 실제 판정은 같은 hazard bounds를 사용하고 움직이는 body 접촉은 Polygon collider로 판정한다. 지원되지 않는 mechanic은 generic fallback으로 실행하지 않고 factory에서 거부한다.
...
- 0.66.0 default `GameSimulationFactory`는 `authored-continuous-stage-runtime-v17-two-boss-dynamic-jammer-stage`만 현재 authored world revision으로 해석한다. Sector 01~06의 48개 canonical v2 Stage와 Boss03·06 generated catalog를 production world에 포함한다. 일반 Stage surface·wall·world object는 Map Editor source만 사용하고 Runtime derived geometry는 0개다. 일반 Gate를 통과한 Player 한 명만 다음 authored Entry로 이동하며 다른 Player·Enemy·공용 진행을 함께 이동시키지 않는다.
- 3-8 Gate는 Boss03 entry를 Player별로 열고 완료 뒤 4-1로 직행한다. 먼저 들어간 Player만 participant가 되고 뒤늦게 Gate에 들어온 Player만 추가되며 다른 Stage의 Player를 강제 이동시키지 않는다. Boss06은 target landmark가 없는 terminal 계약으로 Gate/Bridge/Shuttle과 Player별 Boarding readiness를 진행해 모든 연결 참가자가 직접 boarding zone에 도달했을 때만 run completion을 연다.
...
- `createAuthoredSeamlessSectorRuntimeWorld()`가 Sector 01~~06의 generated Stage 48개를 authored bounds 그대로 맞닿는 48 landmark와 개별 Player portal 47개로 조립한다. 1-8·2-8·4-8·5-8 portal은 다음 Sector Entry로 직접 이동하고, 3-8 portal만 Boss03 완료 전 진입을 보류하며 6-8은 terminal Boss06 전환을 소유한다.
...
- 현재 48개 Runtime Stage 중 `3-8`·`4-8`·`5-8`·`6-8`은 각각 stable boundary ID와 required objective metadata를 가진 독립 content boundary다. 3-8은 다음 Sector route lock과 Boss03 완료 후 전환을 소유하고, 4-8·5-8은 objective 완료 뒤 직접 다음 Sector로 이동하며 6-8은 regular 다음 landmark가 없는 terminal Boss06 경계다. 마지막 authored Area는 `sector-06-08`이다.
