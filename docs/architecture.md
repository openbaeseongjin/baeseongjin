...
- 멀티 snapshot의 축약 Enemy state는 `world revision + objectId`가 가리키는 authored 정적 정의를 예측 월드에서 한 번 인덱싱하고 동적 state와 plain DTO로 합성한다. hydration은 Enemy runtime을 만들지 않으며 `preparePrediction()`이 snapshot당 Enemy 하나에 runtime 하나만 만든다. immutable authored world만 필요한 앱 경로는 `worldSnapshot()`을 사용해 전체 render snapshot을 만들지 않으며, claim/event/progress를 숨길 수 있는 tick cache는 두지 않는다.
