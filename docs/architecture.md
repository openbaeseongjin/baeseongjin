...
- 기본 Runtime의 저작 진행 권위는 `currentSectorId/currentLandmarkId`다. objective 완료는 `requiredObjectiveIds`를 만족한 route lock의 physical connector collision을 활성화하고, 플레이어가 다음 landmark entry bounds에 직접 들어올 때 진행한다. `requiredRouteId` surface는 `WorldGateGeometry.isSurfaceEnabledForProgress()` 하나로 collision과 polygon/pixel terrain renderer가 함께 필터해 잠긴 통로를 밟을 수 있는 지형으로 그리지 않는다. 세로 landmark 층의 실제 collision surface는 유지한다. `areaId`, Gate aperture, `gate-crossed`, `gate-portal-entered`, `activeCheckpointId`와 legacy Stage boundary wall은 explicit Area revision의 compatibility 계약에만 남는다.
...
- `SectorProgressState`가 `currentSectorId/currentLandmarkId`, objective·route·encounter·`collectedAccessModuleIds`와 `sectorBaselineRevision`을 단독 소유한다. individual respawn은 이 상태를 바꾸지 않고, party wipe만 current Sector의 resettable set을 초기화하며 prior Sector 진행을 보존한다.
- 0.32.0 default `GameSimulationFactory`는 `seamless-sector-runtime-v4`를 사용한다. Sector 01은 Stage-local 1-3·1-6·1-7 Carrier encounter가 주는 공용 Access Module 중 아무 2개와 1-8 objective를 Sector transition 조건으로 사용한다. Sector 01~03 enemy slot은 Stage-local 위치·activation·수량을 보존하고 `enemySelection.allowedEnemyTypes`가 있을 때 type만 `slotId + runSeed + worldRevision`으로 결정한다. `WorldSnapshot` protocol v9는 Sector 진행과 Player 상태를 함께 보내되 authored enemy 정적 정의는 `worldRevision + objectId`로 복원하고 동적 상태만 20Hz로 전송한다. legacy Area revision은 explicit compatibility factory로만 생성한다.
...
- 사망 재개는 월드와 공용 진행을 유지한 채 최근 도달 Stage의 `activeRespawnAnchor`로 복귀한다. 각 landmark entry의 anchor는 `level/radius/label` 표현 metadata를 포함하고 polygon·pixel renderer가 같은 `STAGE SAVE` 구조물로 직접 그린다. `landmark-entered`는 체크포인트 지표·cue와 `stage-saved` 화면 안내를 한 번 만들며, party wipe만 current Sector baseline을 초기화해 Sector entry anchor로 되돌린다.
