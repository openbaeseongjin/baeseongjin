...
- Boss는 일반 Stage와 분리된 `BossStageSpec v2 → generated BossStageDefinition → BossEncounterRuntime` 저작·실행 계약을 사용한다. Definition은 Arena·Phase·mechanic·HUD·transition, Phase HP scaling, 닫힌 몸체 `0.25`, 약점 고정 `0.25`, P3 `failureProgress`를 소유한다. `GateLockingCarriageRuntime`은 Carriage kinematic body와 Has-A Beam body를 단일 위치·속도·collider 권위로 진행하며 끝점 대기→예고→실제 Sweep→약점→방향 반전, P3 50% Beam 파손→Ram loop를 소유한다. 닫힌 몸체는 일반 피해 25%, 열린 약점은 일반 피해 100%와 scaled Phase HP 25%를 유효 causal impact마다 받으며 floor overflow를 버린다. Player 충돌·Renderer·hazard는 같은 body snapshot을 사용한다. Timer·collapse와 최종 asset은 범위 밖이다.
- `BossEncounterRuntime`은 Sector 진행과 분리된 중립 월드 Has-A 상태다. 일반 Stage와 분리된 `BossStageSpec v2 → generated BossStageDefinition → BossEncounterRuntime` 계약이 Arena·Phase·mechanic·HUD·transition과 피해 수치를 소유하고, `GateLockingCarriageRuntime`의 Carriage와 Has-A Beam body가 위치·속도·collider의 단일 권위다. attempt·phase HP·Breaker·Core 노출 시간·Player별 active/spectating은 top-level `bossRuntime` snapshot으로 복제하며 `GameSimulation`은 공개 명령과 event 연결만 조정한다. Timer·시간 만료 Arena collapse·그에 따른 관전 탈락과 최종 asset은 후속 범위다.
- 0.56.0 default `GameSimulationFactory`는 `seamless-sector-runtime-v10`만 현재 authored world revision으로 해석한다. 과거 flat Area catalog revision은 production resolution에서 제거했다. Sector 높이 예산은 Sector 01·02가 각각 9,600px, Sector 03이 승인된 12,416px이며 현재 Sector 03 compiled 높이 12,214px를 수용하되 추가 성장을 제한한다. Access topology는 Sector별 공용 Module source 3개와 다음 Sector transition의 `3-of-3`을 사용한다. source는 Sector 01의 1-3·1-6·1-7, Sector 02의 2-2·2-5·2-7, Sector 03의 3-2·3-5·3-7이며, target Runtime Sector가 없으면 존재하지 않는 transition을 만들지 않는다. Sector 경계의 `routeLockId` transit device는 같은 `barrierSegments`에서 T자형 force-field collision과 visual을 파생하고 `3/3` 뒤 `blockedByRouteId` blocker만 비활성화한다. `WorldSnapshot` protocol v12는 공용 Sector 진행, Player별 `respawnAnchorId`, non-null Action state와 Enemy 동적 상태를 20Hz로 전송하며 authored Enemy 정적 정의는 `worldRevision + objectId`로 복원한다.
