...
- 환경 asset 실패는 scene 전체가 아니라 component별로 격리한다. backdrop 실패는 polygon backdrop, terrain 실패는 polygon world geometry로 각각 대체하고 decoration 실패는 장식만 생략한다. `SpriteSceneResourceBundle.prepare()`가 정상 gameplay 전에 모든 선택 atlas의 load·decode를 terminal 상태로 만들므로 pending fallback은 첫 gameplay frame에 노출하지 않는다. 실제 실패 전이에만 한 번 경고하고 디버그 수치에 component와 atlas ID를 노출한다.
...
- bootstrap 기본 프로필은 `sprite`이며 query의 `renderer=polygon`이 기존 표현을 명시적으로 선택한다. 알 수 없는 값은 경고 후 기본 프로필을 사용한다. sprite 프로필은 manifest 로드 뒤 `SpriteSceneResourceBundle` 하나를 만들고 Player·Enemy·Environment atlas의 load와 decode 완료를 startup loading 안에서 기다린다. 싱글·멀티·디버그 재시작은 같은 준비된 bundle을 재사용한다. Player asset 실패는 Player renderer만 polygon body로 복구하고 Enemy package와 Environment component fallback은 서로 독립적이며 scene 전체 profile을 내리지 않는다.
