...
- 0.58.0 default `GameSimulationFactory`는 `seamless-sector-runtime-v13-terminal-boss-stage`를 현재 authored world revision으로 해석한다. Sector 01~06의 48개 canonical v2 Stage와 Boss01~06 generated catalog를 production world에 포함한다. 여섯 일반 content boundary는 직접 Sector seam을 만들지 않고 각각의 Boss Stage entry가 연결을 소유한다. Boss01~05는 승리 후 명시된 다음 landmark로 portal transition하고, Boss06은 target landmark가 없는 terminal 계약으로 Gate/Bridge/Shuttle과 Player별 Boarding readiness를 진행해 모든 연결 참가자가 직접 boarding zone에 도달했을 때만 run completion을 연다. 첫 Boarding Player의 party teleport와 regular `6-9`는 없다. `WorldSnapshot` protocol v12는 공용 Sector 진행, 활성 Boss Stage와 terminal readiness, Player별 `respawnAnchorId`, content boundary 이력, Hardpoint Jammer target/phase, non-null Action state와 Enemy 동적 상태를 20Hz로 전송하며 authored 정적 정의는 `worldRevision + objectId`로 복원한다. Timer/Purge는 mapping HOLD라 현재 snapshot과 gameplay state에 추가하지 않는다.
