...
브라우저 Canvas에서 실행되는 2D 로프 액션 프로토타입이다. 고정 길이 로프 물리, Map Editor가 저작한 48개 Stage의 연속 수직 월드와 개별 Player portal, 적 전투·투사체, 최근 도달 Stage 체크포인트 복귀와 generic Augment 런 성장 상태를 공용 시뮬레이션에서 처리한다. PC와 모바일은 입력 방식만 다르고 게임 규칙은 공유한다.
...
- `AuthoredSectorCatalog`는 build/startup source이고, `AuthoredSeamlessSectorRuntime`은 Sector 01~~06 generated Stage를 하나의 연속 수직 월드로 배치한다. 3-8 outbound route는 Boss03 catalog가 활성일 때 독립 Arena로 가로채고 완료 뒤 4-1로 복귀하며, catalog 항목이 없으면 같은 authored route가 direct portal로 동작한다. 6-8은 terminal Boss06을 연다.
...
- 0.72.0 default `GameSimulationFactory`는 `authored-continuous-stage-runtime-v21-boss03-commander`를 현재 authored world revision으로 해석한다. 48개 canonical Stage와 Boss03·06 generated catalog를 포함하고 일반 Stage derived geometry는 0개다.
