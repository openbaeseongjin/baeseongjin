...
- 환경 asset 실패는 scene 전체가 아니라 현재 Area의 component별로 격리한다. backdrop 실패는 polygon backdrop, terrain 실패는 polygon world geometry로 각각 대체하고 decoration 실패는 장식만 생략한다. `SpriteSceneResourceBundle.prepareArea()`가 정상 gameplay 전에 Player와 시작 Area의 Enemy·Environment atlas만 terminal 상태로 만들고, 다른 Sector package는 독립 background promise로 준비한다. component readiness는 current Area definition의 frozen atlas ID lookup만 평가하므로 무관한 package의 idle·pending·failed가 현재 플랫폼을 fallback시키지 않는다. 실제 실패 전이에만 한 번 경고하고 디버그 수치에 component와 atlas ID를 노출한다.
