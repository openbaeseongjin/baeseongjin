...
- 사망·낙사는 공용 player reset 경로를 사용하되 default Sector Runtime에서는 사망 Player의 체크포인트 상태가 가리키는 anchor로 복귀한다. `fallRecoveryY(playerId)`는 해당 anchor의 landmark Bounds 하단+780px을 사용해 Player가 Gate 분리 공백에서 이전 Stage에 닿기 전에 복구한다. 활성 Boss 참가자는 이 임계값을 적용하지 않고 Boss Arena의 독립 recovery가 우선한다. 동료와 공용 진행은 유지하고 same-tick 전원 respawn도 `sector-reset`을 발생시키지 않는다.
