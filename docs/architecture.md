...
- `WorldSnapshot` protocol v13은 Boss 완료 이력을 포함한 공용 Sector 진행, 활성 Boss Stage와 terminal readiness, Player별 `respawnAnchorId`, content boundary 이력, Hardpoint Jammer target/phase, non-null Action state와 Enemy 동적 상태를 전송한다. 현재 Stage ID는 추가하지 않고 Player 위치가 현재 authored Stage를 나타낸다. Access 조건은 portal 사용 가능 여부만 바꾸며 collision·barrier geometry를 생성하거나 제거하지 않는다. Timer/Purge는 HOLD다.
