...
- 0.30.0 default `GameSimulationFactory`는 `seamless-sector-runtime-v3`를 사용한다. Sector 01은 Stage-local 1-3·1-6·1-7 Carrier encounter가 주는 공용 Access Module 중 아무 2개와 1-8 objective를 Sector transition 조건으로 사용한다. `WorldSnapshot` protocol v8은 이 Sector 진행 JSON과 Player별 증강·Action/knockback state를 함께 보낸다. legacy Area revision은 explicit compatibility factory로만 생성한다.
