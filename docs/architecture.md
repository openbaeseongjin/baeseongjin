...
- 기본 Runtime의 저작 진행 권위는 `currentSectorId/currentLandmarkId`다. objective 완료는 `requiredObjectiveIds`를 만족한 route lock의 physical connector collision을 활성화하고, 플레이어가 다음 landmark entry bounds에 직접 들어올 때 진행한다. `areaId`, Gate aperture, `gate-crossed`, `gate-portal-entered`, `activeCheckpointId`와 Stage boundary wall은 explicit legacy Area revision의 compatibility 계약에만 남는다.
