...
- 기본 Runtime은 authored Gate trigger를 개별 Player forward portal의 유일한 Stage 전환 입력으로 사용한다. Gate는 공간 이동과 Rope 분리만 수행하며 무기 cooldown·Rope disable·Augment runtime을 respawn처럼 초기화하지 않는다. Player collider와 `respawnAnchor.triggerBounds`의 겹침은 개인 checkpoint만 갱신하고 objective는 자기 bounds/source와 `requiredObjectiveIds`만 판정한다. 현재 Stage·Sector는 Player 위치에서 파생하며 별도 상태를 저장하지 않는다. 모든 Area surface는 Run 시작부터 collision과 renderer가 공유하는 authored 정적 집합이고 route 진행으로 생성·제거하지 않는다.
...
- `SectorProgressState`는 objective·논리 prerequisite·encounter·`collectedAccessModuleIds`와 `completedBossStageIds`를 Run 공용 진행으로 소유한다. `currentSectorId/currentLandmarkId/visitedLandmarkIds`는 snapshot과 gameplay authority에 없다. Player별 `respawnAnchorId`는 Player runtime이 소유하며 Timer·Purge가 미정인 현재 사망 인원 수에서 공용 reset을 추론하지 않는다.
...
- `PlayerCommand.interact`는 augment Node·terminal·authored exit panel 같은 근접 문맥 상호작용 의도다. `InputSampler`는 PC `W/↑`와 모바일 점프 버튼을 점프 축과 `interact`에 함께 매핑하며, 진행 시스템은 명시적 source의 반경 안에서만 이를 소비한다. Gate objective가 열린 뒤 실제 trigger를 통과해야 portal이 발생한다.
