...
- 피해 결과는 일반화한 impact ID를 수명주기 키로 사용하며, 기존 투사체 소비자는 같은 값을 `projectileId` 호환 별칭으로 받을 수 있다. `OwnerPredictionRuntime`은 `GameSimulation`의 공용 피해 전이로 넉백·HP·치명 Stage 세이브 포인트 부활·착지 피해 또는 로프 절단을 즉시 적용하고, pending 기록은 중복 억제와 승인 resolve 대응에 사용한다. Player에는 피격 후 시간 기반 무적 상태가 없으므로 서로 다른 유효 impact는 간격과 관계없이 각각 피해를 적용한다. 동일 impact의 재전송은 impact ID 수명주기에서 멱등 처리하고, 비활성 생명 상태와 Boss 회복 보호는 별도 수명주기 guard로 유지한다. 승인 receipt는 resolve 사건까지 기록을 유지하고, 거부 receipt는 pending 기록만 정리하며 이미 인식한 피해 상태를 복원하지 않는다. 전송 계층은 HP나 물리 역연산을 구현하지 않는다.
...
- 저작 landmark는 canonical AREA-SPEC v2의 local camera zone과 desktop/mobile zoom, 선택적 player screen ratio를 사용한다. 싱글·멀티의 `AuthoredCameraDirector`는 공용 진행이 아니라 각 로컬 플레이어의 물리 좌표로 현재 landmark와 Shot을 고른다. renderer는 계산된 카메라만 받고 Camera Zone 규칙을 다시 해석하지 않는다.
...
- `AuthoredSectorCatalog`는 build/startup source이고, `AuthoredSeamlessSectorRuntime`은 Sector 01~~06 generated Stage를 `target.bounds.bottom === source.bounds.top`인 하나의 연속 수직 월드로 배치한다. landmark bounds는 각 `AREA-SPEC.v2.json`의 authored bounds와 같고, surface·world object도 authored 항목만 좌표 이동해 사용한다. compiler가 city wing·seam·boundary·transit collision을 만들지 않는다. 1-8·2-8·3-8·4-8·5-8의 outbound route는 다음 Sector Entry로 직접 이어지고, 6-8만 terminal Boss06을 연다.
...
- Boss는 일반 Stage와 분리된 `BossStageSpec v2 → generated BossStageDefinition → CompositeBossEncounterRuntime` 저작·실행 계약을 사용한다. World는 terminal Boss06 definition 하나만 catalog로 보유하고 snapshot의 Stage ID로 concrete Runtime을 선택·복원한다. 공용 Runtime은 참가 상태 enum 검증·hazard contact 멱등 조회·ID 순서 respawn·승리 spectator 복귀를 소유한다. Boss06은 3200px Main과 단방향 Ledge 3개의 발 좌표 support query를 사용하며 플랫폼 이벤트나 저장 boolean을 만들지 않는다. 걷기·점프·내려가기·낙하·착지와 모든 공격은 단일 concrete StateDefinition catalog의 locomotion/attack lane으로 등록한다. 공격 lane은 `worldSeed+attempt+selectionSequence` 결정적 가중 선택, 선택 0.25·나머지 +0.25 회복·1회 chain 2배를 사용하고 이를 Boss snapshot v2로 복원한다. 소환은 2개·15초 cooldown·live 6 skip을 유지하며 표현 telegraph와 실제 판정은 같은 hazard bounds를 사용한다.
...
- 0.71.0 default `GameSimulationFactory`는 `authored-continuous-stage-runtime-v19-boss06-open-edge-state-pool`만 현재 authored world revision으로 해석한다. Sector 01~06의 48개 canonical v2 Stage와 Boss06 generated catalog를 production world에 포함한다. 일반 Stage surface·wall·world object는 Map Editor source만 사용하고 Runtime derived geometry는 0개다. 일반 Gate를 통과한 Player 한 명만 다음 authored Entry로 이동하며 다른 Player·Enemy·공용 진행을 함께 이동시키지 않는다.
...
- `WorldSnapshot` protocol v18은 Boss 상태 선택 sequence·가중치와 완료 이력을 포함한 공용 Sector 진행, 활성 Boss Stage와 Rope attachment actor transform, terminal readiness, Player별 `respawnAnchorId`·상태이상 Pool·Spell 슬롯/쿨다운/charge/area/Passive·경험치, content boundary 이력, Hardpoint Jammer target/phase/sequence와 Enemy 동적 상태를 전송하며 피격 후 무적 타이머는 포함하지 않는다. `owner-motion` v11은 Rope 표면과 Player 상태를 소유자 결과로 공유한다. 현재 Stage ID는 추가하지 않고 Player 위치가 현재 authored Stage를 나타낸다.
