...
- 고정 `sentry` Turret만 이동 intent가 없다. Patrol은 authored 경로 또는 짧은 기본 왕복 경로를, Pursuit·Swarm은 직접 추격을 사용한다. Shield·Artillery·Support·Hardpoint Jammer는 공용 `EnemyRoamingMotion` Has-A 상태를 각 구체 Behavior가 자기 상태에 맞춰 조합한다. 모든 이동 intent와 넉백 displacement는 공용 physics step에 적재하고 서버 중립 Enemy step이 한 번만 적분해 `Collider.resolveSurfaces`로 벽·지형을 해결한다. owner Player는 로컬 Enemy snapshot과 즉시 몸체 충돌하고 서버 Enemy도 최신 복제 Player body와 충돌해 어느 한쪽만 통과 허용 predicate를 만들지 않는다. 위치 snapshot 복원과 명시적 spawn/reset은 이동 적분이 아니므로 별도 상태 복원 경계에 남는다.
...
- `WorldSnapshot` protocol v17은 Boss 완료 이력을 포함한 공용 Sector 진행, 활성 Boss Stage와 Rope attachment actor transform, terminal readiness, Player별 `respawnAnchorId`·상태이상 Pool·마법 슬롯/쿨다운·경험치, content boundary 이력, Hardpoint Jammer target/phase/sequence와 Enemy 동적 상태를 전송한다. 피격 후 무적 타이머는 Player 상태가 아니므로 전송하지 않는다. `owner-motion` v10은 Rope 표면과 Player 상태를 소유자 결과로 공유한다. 현재 Stage ID는 추가하지 않고 Player 위치가 현재 authored Stage를 나타낸다.
...
- enemy knockback은 behavior 내부를 직접 수정하지 않는 public transient state다. `EnemyMobility`가 플레이어를 직접 추격·돌진하는 Pursuit/Swarm만 외부 displacement 대상으로 분류한다. 고정 Sentry/Turret과 자기 Patrol/Roaming 경로를 소유한 Patrol·Shield·Artillery·Support·Jammer는 스스로 이동하지만 피해 넉백으로 경로가 이탈하지 않도록 `impactDisplacementEnabled = false`를 유지한다. 명시적 false는 Pursuit/Swarm도 추가로 고정할 수 있지만 고정 계열을 true로 opt-in할 수 없다.
