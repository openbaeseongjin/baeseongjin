...
> 상태: **REFERENCE-ONLY / 본체 시각 레퍼런스 채택**
>
> 범위: 본체 시각 레퍼런스 1종의 외형 기준을 소유한다. 양팔 장비의 분리 이미지와 Runtime 스프라이트 전환은 후속 승인 범위다. 이 문서는 Boss03을 제품에 재도입하거나, Runtime 자산·충돌·공격 판정·물리·네트워크 규칙을 만드는 근거가 아니다.
...
![LOWER SECTOR COMMANDER 본체 시각 레퍼런스](./images/commander-body-base-reference-v1.png)

- 이 이미지는 완전 기계형 중량 2족 보행기, 우향, 낮고 넓은 황색 센서 시야, 앞쪽 유압 그리퍼와 뒤쪽 파일드라이버 해머, 먹색 강철·녹슨 철색·제한적 황색 경고등·중고 산업 장비 표면을 전달하는 **시각 레퍼런스**다.
- 파일은 `1024×1536px` RGB 생성 시안이며, 표시된 체크무늬는 이미지에 포함된 배경이다. 투명 alpha, `128×192px` 캔버스, 기준점, Runtime 합성 가능 여부는 이 파일에서 주장하지 않는다.
- 사용자는 현재 목적에 필요한 구체적 외형 전달을 우선했다. 따라서 이 파일은 방향·실루엣·장비 역할·재질을 확인하는 기준으로만 사용하며, 게임 이미지·지형·Collision·스프라이트 export로 사용하지 않는다.

## 3. 향후 Runtime 자산 전환 기준 — 현재 범위 아님

향후 별도 승인을 받아 Runtime 자산으로 전환할 때만 모든 결과물은 `lower-sector-commander` 참고 자산 패키지에 속한다. 이 패키지는 그 시점에도 Runtime package가 아니며, 세 이미지는 아래 파일명으로만 구분한다.
...
- 현재 범위에는 분리 팔 이미지, 행동 포즈, 스프라이트 시트, 애니메이션, 조준선·경고 원, 끌어오기·해머 VFX, UI가 없다.
...
## 5. 향후 픽셀 Runtime 자산 규칙 — 현재 레퍼런스에는 적용하지 않음
...
## 6. 향후 Runtime 자산 생성 프롬프트 기준 — 현재 범위 아님
...
## 7. 향후 Runtime 자산 판정 기준 — 현재 범위 아님
...
- 현재 본체 레퍼런스는 [`./images/commander-body-base-reference-v1.png`](./images/commander-body-base-reference-v1.png)에 보관한다. 생성 도구는 현재 대화의 내장 이미지 생성이며, API/CLI 비용을 사용하지 않았다.
- 향후 Runtime 자산 전환이 승인된 뒤 생성한 파일만 `assets/artwork/characters/lower-sector-commander/` 아래에 아래 구조로 보관한다.
...
- 향후 README에는 상태(`REFERENCE-ONLY`), 각 export 파일명, 제작 캔버스, 실제 불투명 영역, 기준점, `32×32` 기준 격자, 기본 우향과 **좌우 반전 미결정**, 생성 도구·버전·원본 형식, 프롬프트 출처, 라이선스 정보와 이 기획서 링크를 기록한다.
- 실제 게임 출력 크기·좌우 반전 정책은 Boss Runtime 공개 계약이 승인될 때 정의한다. Runtime 연결 뒤에는 데스크톱과 모바일 실제 게임 화면에서 Player, Rope, Anchor, Telegraph, 충돌 발판보다 과도하게 복잡하거나 가려지지 않는지 확인한다.
- 현재 레퍼런스와 향후 생성물은 별도 Boss Runtime 공개 계약과 loader·manifest·renderer 연결이 승인되기 전까지 `production-ready` 또는 `Runtime PASS`로 표시하지 않는다.
- 그랩 자세, 해머 내려찍기 자세, 조준선·경고 원, 끌어오기·충돌 VFX는 사용자가 별도 승인할 때만 기획·생성한다.
...
| 현재 납품 | 양팔 장비를 갖춘 본체 전체 시각 레퍼런스 1종 |
| 현재 우선순위 | Runtime용 투명 PNG 규격보다 구체적인 외형 전달 |
