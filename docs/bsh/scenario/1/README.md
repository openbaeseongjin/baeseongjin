...
Stage별 Scenario Art는 생성 직전에 해당 Stage Runtime과 Camera Zone을 다시 확인한다. 현재 `1-2`~`1-4` 이미지는 환경 분위기와 일부 오브젝트 위계만 참고할 수 있는 `TEMPORARY / PENDING REGENERATION` 상태이며, Player 크기와 Rope 연결 의미의 새 기준으로 사용하지 않는다.
