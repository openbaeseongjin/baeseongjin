...
Stage별 Scenario Art는 생성 직전에 해당 Stage Runtime과 Camera Zone을 다시 확인하고 보이는 발판·장애물의 구조 가이드를 먼저 만든다. 현재 1-1 C04 `05`와 1-2 C02 `06` 이미지는 구조 관계까지 검수한 `APPROVED ART REFERENCE`이며, `1-3`~`1-4` 이미지는 환경 분위기와 일부 오브젝트 위계만 참고할 수 있는 `TEMPORARY / PENDING REGENERATION` 상태다.
