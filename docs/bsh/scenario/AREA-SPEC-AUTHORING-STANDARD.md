...
7. catalog ID·revision·Stage 순서·source/output path는 `AREA-CATALOG*.json` manifest가 소유하고 모든 선택 Stage의 `source`는 `generated`여야 한다.
8. `npm run validate:area-specs`는 Sector 01~06의 canonical v2 파일이 정확히 48개인지와 각 파일의 schema·Runtime 의미·경로 정체성을 검증한다.
9. `node scripts/area-authoring-v2/generateAreaCatalogs.mjs --check`는 manifest와 generated Runtime module의 byte-level freshness를 검증한다.
10. 신규 또는 변경 Stage는 validator와 generator check를 통과한 뒤 production compiler·renderer 경로에서 확인하며 v1·수기 executable fallback을 추가하지 않는다.
