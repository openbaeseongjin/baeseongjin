...
- ACK 이하 입력을 제거하고 권위 틱부터 현재 predicted tick까지 입력을 재실행한다.
- 30px 이하 위치 오차는 별도 표시 offset으로 100ms 안에 감쇠한다.
- 160px 초과 오차, 로프 부착 불일치, 다운·체크포인트 전이는 즉시 권위 상태로 맞춘다.

### C. 실제 원격 스냅샷 보간

- 최소 3개 스냅샷을 보관하고 초기 100ms 지연된 렌더 시각의 앞뒤 표본을 보간한다.
- 정상 수신 중에는 외삽하지 않고, 표본이 없을 때만 최대 120ms 외삽한다.
- HP·다운·로프 절단 같은 비위치 판정은 최신 권위 값을 즉시 사용한다.

### D. 자기 공격 예측

- 자기 자동 발사에 `predictionId`, 입력 sequence와 발생 tick을 붙여 탄환과 예상 타격 VFX를 즉시 표시한다.
- 서버 `spawn/resolve`가 같은 ID를 확정하면 중복 생성·VFX 없이 승격한다.
- 불일치하면 예측 탄환을 제거하거나 권위 탄환으로 교체하며, 적 HP와 사망은 서버 값을 따른다.

### E. 계측과 실제 기기 검증

- 중단한 Issue #181의 RTT·스냅샷 간격·대기 명령·거부율을 새 구조 위에 재적용한다.
- 보정 거리 p50/p95, 하드 스냅 수, 외삽 시간과 예측 취소 수를 추가한다.
