...
현재 자기 이동·로프·자기 공격 충돌·자기 피격·로프 절단은 클라이언트 선행 경로가 구현돼 있다. 반면 적 발사 생성, 체크포인트 도달, 다운·부활 완료, 아티팩트 획득·손실, 팀 패배·정상 도달 중 서버 시뮬레이션이 먼저 생성하는 경로는 이 계약으로 전환해야 한다. 해당 코드를 수정하거나 기능을 확장할 때 서버 시작형 경로를 그대로 복제하지 말고 담당 클라이언트 claim과 멱등 event key를 먼저 설계한다.
