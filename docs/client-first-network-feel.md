...
모든 플레이어 인지형 게임플레이 이벤트의 최초 트리거는 클라이언트다. 클라이언트는 서버 왕복 전에 해당 사건의 운동·UI·연출 반응을 시작하고 claim을 전송한다. 서버는 claim을 검증하고 중복을 제거해 최종 상태를 확정·배포하지만, 모바일 체감 경로에서 사건을 먼저 시작하지 않는다.

- 자기 행동: 소유 클라이언트가 트리거한다.
- 피격·충돌·로프 절단: 피해 클라이언트가 트리거한다.
- 대상 없는 공용 사건: 안정적인 플레이어 ID 순서의 대표 클라이언트가 트리거한다.
- 여러 클라이언트가 감지할 수 있는 사건: 같은 결정적 event key를 사용하고 서버가 한 번만 승인한다.
- 세션 연결·퇴장·빈 방 제거·서버 장애는 서버 시작을 허용하는 예외다.

## 기존 구현 전환 상태

현재 자기 이동·로프·자기 공격 충돌·자기 피격·로프 절단은 클라이언트 선행 경로가 구현돼 있다. 반면 적 발사 생성, 체크포인트 도달, 사망·체크포인트 부활 완료, 아티팩트 획득·손실, 정상 도달 중 서버 시뮬레이션이 먼저 생성하는 경로는 이 계약으로 전환해야 한다. 해당 코드를 수정하거나 기능을 확장할 때 서버 시작형 경로를 그대로 복제하지 말고 담당 클라이언트 claim과 멱등 event key를 먼저 설계한다.
