...
5. **실행 권한과 도메인 로직을 분리한다.** 플레이어 당사자 사건은 소유자 또는 피해 클라이언트가 먼저 적용하고, 중립 월드는 서버가 진행한다. 양쪽은 같은 객체·컴포넌트·capability의 공용 규칙을 사용하며 서버·클라이언트별 복제 구현을 만들지 않는다. 서버는 검증·중복 제거·최종 확정과 공유를 담당한다.
