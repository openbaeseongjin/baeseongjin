...
- 서버의 정상 승인 receipt와 snapshot은 소유 `InputDrivenObject`를 복구하는 명령이 아니다. 소유자의 HP·피격 무적·생명·로프·무기 쿨다운·시간 제한 강화는 로컬 공용 시뮬레이션이 계속 작성하며, 서버 복제본과 동료가 승인된 상태·사건을 따라간다. 소유자 전체 상태 복원은 최초 입장·재접속과 `owner-motion`처럼 복구 가능한 claim의 명시적 거부로 제한한다. impact 거부는 이미 적용한 HP·부활·아티팩트·로프를 되감지 않는다.
- 피해 claim의 receipt는 로컬에서 이미 인식한 HP·부활·아티팩트·로프 전이를 되감는 명령이 아니다. 거부되어도 피해 클라이언트 상태를 복원하거나 소비한 적 탄환을 되살리지 않는다. 서버는 인증된 피해자와 서버 소유 탄환의 ID·대상·중복을 검증해 공유하며, 지연된 서버 위치나 궤적 오차로 피해자의 로컬 충돌 결과를 거절하지 않는다.
- 적 탄환 본체 피격은 복제 객체의 대미지로 로컬 HP와 치명 시 체크포인트 부활·결정적 아티팩트 손실까지 같은 `GameSimulation` 명령에서 즉시 적용한다. snapshot과 receipt는 이 HP·부활·아티팩트·로프 상태를 덮지 않는다. 멀티 HUD는 소유 클라이언트 상태를 표시하고 서버 복제본과 동료가 피해 클라이언트의 확정 결과로 수렴한다.
...
- 새 게임플레이 시스템의 테스트에는 최소한 `로컬 트리거가 서버 receipt보다 먼저 발생함`, `중복 claim은 한 번만 확정됨`, `각 claim 계약에 맞는 거부 처리`, `승인 receipt와 정상 snapshot이 소유 클라이언트 상태·연출을 되감지 않음`, `서버 복제본과 동료가 소유자의 상태로 수렴함`, `중립 객체는 모든 클라이언트가 서버 상태로 수렴함`을 포함한다. impact 거부 테스트는 HP·부활·로프와 소비한 탄환이 복구되지 않는지 수치로 검증한다.
