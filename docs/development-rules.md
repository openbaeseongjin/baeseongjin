...
- PASS는 실제로 통과한 경계만 뜻한다. 실행한 명령·사용한 모듈·통과시킨 사건과 확인하지 않은 경계를 구분하고, 좁은 smoke나 순수 시뮬레이션을 전체 사용자 동작 검증으로 표현하지 않는다.
- 사용자가 특정 사용자 경로를 검증하라고 요구하면 더 좁은 검사로 조용히 대체하지 않는다. 대체 검증만 가능하면 그 사실과 한계를 배포·완료 보고 전에 먼저 밝힌다.
- 멀티플레이 gameplay 사건은 production client/server 모듈, 실제 transport, wire 직렬화, live snapshot과 두 클라이언트를 통해 `입력 → 객체 생성 → snapshot 전송 → 상대 판정·claim → 상태 수렴`을 끝까지 통과시킨다. 사건 뒤 서버 `/health`와 후속 snapshot도 확인해 프로세스 생존을 증명한다.
- 인프로세스 `GameSimulation`, 접속·퇴장 smoke, 만료 뒤 snapshot처럼 실제 사건 중간의 wire 상태를 지나지 않는 검사는 해당 부분 증거일 뿐 실제 멀티 gameplay 검증을 대체하지 않는다.
- 실패·미실행·관측 누락을 숨기지 않는다. 새 증거가 기존 PASS 주장을 깨면 즉시 이전 주장을 철회하고 실제 영향, 현재 서비스 상태와 재검증 필요 범위를 보고한다.
- 멀티 기본 회귀 예산은 일반 사용자 흐름인 `접속 → 이동·동료 수렴 → 전투 → 사망·부활 → 방 수명`과 서버 소유 중립 월드에 먼저 쓴다. malformed payload·구버전 protocol·극단 backlog 같은 희귀 방어 시나리오는 실제 장애가 있었거나 해당 계약을 변경하는 작업에서만 추가한다.
...
- 구현 인터뷰에서는 확정된 계약에서 파생되는 선택을 다시 묻지 않고, 결과가 실제로 7:3 이상으로 갈릴 수 있는 결정만 질문한다.
...
- 서버의 정상 승인 receipt와 snapshot은 소유 `InputDrivenObject`를 복구하는 명령이 아니다. 소유자의 HP·생명·로프·무기 쿨다운·시간 제한 강화는 로컬 공용 시뮬레이션이 계속 작성하며, 서버 복제본과 동료가 승인된 상태·사건을 따라간다. Player는 피격 후 시간 기반 무적 상태를 만들지 않고 서로 다른 유효 impact를 각각 즉시 적용한다. 동일 사건 중복은 impact ID 수명주기에서 제거하며 비활성 생명 상태와 부활·회복 보호는 별도 guard로 유지한다. impact 지문 불일치 때는 서버가 피해자 상태를 요청해 흡수하며 소유자를 서버 상태로 되감지 않는다.
...
- 모든 Runtime `GameObject`는 `GameObjectManager`가 조합한 종류별 manager에 등록한다. 모든 종류 manager는 `BaseGameObjectManager`를 상속하고 ID index·순서 보존 collection·등록·조회·교체·제거·render snapshot을 부모 구현 그대로 사용한다. Player manager는 Player·Rope·Weapon aggregate transaction, Enemy manager는 active lifecycle처럼 실제로 다른 동작만 override한다. 차이가 없는 종류에 한 줄 wrapper subclass를 만들거나 공통 메서드를 종류별 manager에 반복하지 않는다.
- `GameObjectManager`는 종류별 manager·공간 index·ID allocator를 Has-A로 조합하고 게임 종류를 `if`·`switch`로 해석하지 않는다. `GameSimulation`은 tick 단계와 도메인 사건만 조정하며 객체 배열을 직접 교체·추가·제거하거나 별도 ID lookup을 만들지 않는다. authored world DTO는 Runtime 객체로 위장하지 않고 immutable `WorldObjectManager` index로 관리한다.
...
- 고정 HUD의 표시 토글은 각 클라이언트의 표현 상태로만 두고 게임 명령·시뮬레이션·네트워크 snapshot에 넣지 않는다. 모바일·짧은 가로 viewport의 고정 HUD는 별도 gameplay 정보를 줄이지 않고 축소 레이아웃을 사용하며, 토글은 고정 상태·objective 패널과 하단 조작 안내를 함께 숨긴다. 토글 중에도 Player·Enemy HP 표현은 유지한다.
...
- 환경 renderer는 backdrop·terrain으로 분리하고 고도 zone, atlas frame, Canvas 호출과 component 상태를 각 하위 renderer가 소유한다. 싱글·멀티 앱에 같은 환경 분기나 진단 전달 코드를 복제하지 않는다.
- 도트 terrain은 기존 collision surface polygon을 그대로 clip·stroke하고 one-way edge도 동일 vertex chain을 사용한다. Runtime은 world seed·surface 목록·hash로 비충돌 배경 장식을 자동 생성하거나 배치하지 않는다. 배경 세부는 authored backdrop에 포함하고 독립된 장식 객체는 Map Editor의 명시적 world object로 저작한다.
- 공통 Canvas host는 CSS viewport와 camera로 visible world bounds를 한 번 계산해 불변 viewport로 전달한다. composer나 앱은 객체 종류를 분기해 컬링하지 않으며 terrain·actor·projectile 하위 renderer가 자기 bounds와 안전 margin을 적용해 실제 draw 여부를 완결한다. sprite·polygon 또는 싱글·멀티별 별도 컬링 경로를 만들지 않는다.
- collision surface bounds·edge 정보처럼 정적인 계산은 소유 renderer가 world·zone 변화에 맞춰 무효화하는 캐시로 보관한다. camera 이동이나 매 draw마다 전체 월드 geometry를 다시 만들지 않으며 캐시가 gameplay collision·권위 상태를 소유하게 하지 않는다.
...
- 여러 환경 atlas는 캐릭터와 분리된 `environment-asset-format.md` 계약으로 로드한다. Runtime atlas 실패는 backdrop·terrain 단위로만 fallback하고 pending을 실패로 고정하지 않으며, loader·schema·example·validator 중 하나를 바꾸면 나머지 계약과 component별 실패 진단을 함께 갱신한다.
...
- Quadtree를 추가했다는 사실만으로 최적화 완료로 보지 않는다. fixed step·render·snapshot의 호출자가 원본 전체 collection을 직접 순회하면 broad-phase 계약 위반이며, 공간 후보·활성 membership·snapshot 재사용·정적 cache의 단일 기준은 [`performance-architecture.md`](./performance-architecture.md)를 따른다.
...
- 원격 위치 표본의 tick은 그 상태를 실제로 만든 시계를 사용한다. 적·공용 객체는 `serverTick`, 플레이어는 서버 시간축으로 투영된 `ownerMotionTick`으로 보간한다. `ownerMotionTick`이 이미 `authorityTick`인 현재 계약에서는 플레이어 목표 시각에 `inputLeadTicks`를 다시 더하지 않는다. 반복된 owner motion을 새 server tick의 새 위치처럼 취급하지 않는다.
- 원격 Player 표현은 [`multiplayer-synchronization.md`의 연결과 복구](./multiplayer-synchronization.md#연결과-복구)가 소유하는 Player별 playback clock·arrival-jitter buffer·2차 smoothing 계약을 사용한다. snapshot 수신 시각으로 playback tick을 직접 재설정하거나, 끊김을 숨기려고 snapshot Hz·외삽 상한을 늘리거나, `ownerMotionTick`을 prediction timeline으로 다시 해석하지 않는다. 변경 시 일정 간격뿐 아니라 실제 WebSocket의 jitter·owner tick 반복·포탈 snap·정지 frame 비율을 함께 검증한다.
...
- 멀티 서버 fixed tick은 플레이어 체력 0 스캔으로 사망·부활을 시작하거나 보상 선택 명령으로 Augment를 확정하지 않는다. 피해·사망은 피해자 impact claim, 증강 선택은 `augment-selection` claim의 검증 경계에서만 서버 복제 상태와 공유 사건에 반영한다.
...
- `game-kit`은 플레이어, 로프, 적, 증강, 월드 진행, HUD를 알지 못한다.
...
- Boss·Enemy 애니메이션 검증은 AI의 랜덤 상태 선택을 기다리는 절차로 대체하지 않는다. 로컬 Preview가 검증할 presentation 상태와 부착 VFX를 결정적으로 반복하고, 활성 중 gameplay 진행을 정지하며, 사용자가 같은 Canvas에서 직접 시작·중간·종료를 확인할 수 있어야 한다. 이 강제 상태는 저작 Spec·전투 판정·일반 게임·멀티 snapshot에 저장하지 않는다.
...
- 작업 공지는 [`.codex/skills/work-announcement/SKILL.md`](../.codex/skills/work-announcement/SKILL.md)에 따라 Git에 반영된 이력만 근거로 작성한다.
- 기간·작업 분야·담당자를 독립 필터로 받고, 둘 이상을 지정하면 교집합만 공지한다. 담당자 판정은 merge 작성자가 아니라 실제 feature commit 작성자와 변경 경로를 사용한다.
- 전체 연속 작업 시간을 붙이지 않고 의미가 같은 작업 묶음마다 Git 반영 시각 기준의 대략적인 KST 시간대를 표시한다. 사용자 체감 버그 수정과 아직 남은 검증을 누락하지 않는다.
- Discord 게시 전 전체 초안을 검토받고, 최종 문구와 대상 채널의 명시적 승인 뒤 승인된 텍스트만 전송한다. 초안의 기간·필터·내용이 바뀌면 다시 승인받는다.
