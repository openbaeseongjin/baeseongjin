...
- 플레이어가 인식하는 모든 게임플레이 이벤트는 클라이언트가 최초 트리거한다. 서버 응답을 받은 뒤 이동, 넉백, 로프 해제, UI 전환 또는 VFX를 처음 시작하면 안 된다.
- 자기 행동은 소유 클라이언트, 충돌·피격은 피해 클라이언트, 대상 없는 공용 사건은 안정적인 플레이어 ID 순서로 정한 대표 클라이언트가 claim을 만든다.
- 클라이언트는 트리거 프레임에 로컬 상태와 피드백을 적용하고, `eventKey`, `clientTick`, 관련 객체 ID와 최소 판정 자료만 서버에 보낸다. HP나 보상 수치처럼 서버가 소유한 최종 값은 claim에 넣지 않는다.
- 서버 역할은 claim 검증, 중복 제거, 최종 상태 확정과 다른 클라이언트로의 배포다. 서버 시뮬레이션이 동일 사건을 먼저 발생시켜 모바일 클라이언트 반응을 왕복 지연시키면 안 된다.
...
- 기존 서버 시작형 게임플레이 이벤트를 건드리는 작업은 같은 범위에서 클라이언트 트리거형으로 전환하거나, 미전환 사유와 후속 범위를 명시한다.
