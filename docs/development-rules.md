...
- 피해 claim의 receipt는 로컬에서 이미 인식한 HP·부활·아티팩트·로프 전이를 되감는 명령이 아니다. 서버는 정상 claim에 같은 전이를 적용해 지문을 비교하고, 일치하지 않을 때만 `state-diverged` 복구 요청을 보낸다. 피해 클라이언트는 그때의 최신 소유자 상태를 한 번 전송하며 서버와 동료가 이를 따른다. 소비한 적 탄환을 되살리지 않는다.
...
- `InputDrivenObject`는 서버가 승인한 최신 소유자 상태가 서버와 다른 클라이언트의 수렴 원점이다. 정상 승인 중 소유자를 지연된 서버 위치로 되감지 않는다. `owner-motion`처럼 서버 허용 운동을 따르는 계약은 명시적 거부 때 공유 상태에서 미확정 입력을 재실행하지만, impact 불일치는 반대로 서버가 피해 클라이언트의 최신 결과 상태를 흡수한다.
...
- 새 게임플레이 시스템의 테스트에는 최소한 `로컬 트리거가 서버 receipt보다 먼저 발생함`, `정상 claim은 최소 사건 자료만 보냄`, `상태 불일치 때만 복구 상태를 보냄`, `중복 claim은 한 번만 확정됨`, `각 claim 계약에 맞는 거부 처리`, `승인 receipt와 정상 snapshot이 소유 클라이언트 상태·연출을 되감지 않음`, `서버 복제본과 동료가 소유자의 상태로 수렴함`, `중립 객체는 모든 클라이언트가 서버 상태로 수렴함`을 포함한다. impact 테스트는 HP·부활·로프와 소비한 탄환이 복구되지 않고, 복구 요청 뒤 서버 상태가 피해자의 최신 수치로 수렴하는지 검증한다.
