...
- 피해 claim의 receipt는 로컬에서 이미 인식한 HP·부활·로프 전이를 되감는 명령이 아니다. 서버는 정상 claim에 같은 전이를 적용해 지문을 비교하고, 일치하지 않을 때만 gameplay 거부와 분리된 `impact-recovery-request`를 보낸다. 피해 클라이언트는 그때의 최신 소유자 상태를 한 번 전송하며 서버와 동료가 이를 따른다. 소비한 적 탄환을 되살리지 않는다.
