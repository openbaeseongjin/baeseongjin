...
- 서버의 정상 승인 receipt와 snapshot은 소유 `InputDrivenObject`를 복구하는 명령이 아니다. 소유자의 HP·피격 무적·생명·로프·무기 쿨다운·시간 제한 강화는 로컬 공용 시뮬레이션이 계속 작성하며, 서버 복제본과 동료가 승인된 상태·사건을 따라간다. 소유자 전체 상태 복원은 최초 입장·재접속과 명시적으로 거부된 claim 재실행 경계로 제한한다.
...
- 적 탄환 본체 피격은 복제 객체의 대미지로 로컬 HP와 치명 시 체크포인트 부활·결정적 아티팩트 손실까지 같은 `GameSimulation` 명령에서 즉시 적용한다. 승인 전 snapshot은 pending impact가 만든 HP·부활·아티팩트·로프 상태를 덮지 않는다. 승인 receipt 뒤에도 해당 projectile의 resolve 사건까지 복구 정보를 유지하고, resolve 뒤 정상 snapshot도 소유자의 HP·부활·로프를 다시 쓰지 않는다. 멀티 HUD는 소유 클라이언트 상태를 표시한다. 서버는 claim의 대미지를 신뢰하지 않고 서버 소유 탄환 데이터로 검증해 서버 복제본과 동료에게 공유하며, 거부 receipt만 피격 전 상태 복원과 후속 입력·impact 재실행을 시작한다.
...
- 클라이언트는 트리거 프레임에 로컬 상태와 피드백을 적용하고, `eventKey`, `clientTick`, 관련 객체 ID와 최소 판정 자료만 서버에 보낸다. HP·대미지·보상 수치처럼 서버가 자체 중립 객체·규칙으로 검증할 수 있는 결과값은 claim에 넣어 신뢰하게 하지 않는다. 검증 뒤 서버 복제본과 동료가 소유 클라이언트의 전이를 따라가며 소유자에게 결과 상태를 역적용하지 않는다.
...
- 플레이어 당사자 사건에서 서버 역할은 claim 검증, 중복 제거, 서버 복제 상태 갱신과 다른 클라이언트로의 배포다. 서버는 별도로 중립 객체의 생성·궤적·수명주기를 진행하되, 지연된 플레이어 복제 위치로 피격이나 절단을 먼저 발생시켜 모바일 클라이언트 반응을 왕복 지연시키거나 정상 스냅샷으로 소유자 상태를 다시 쓰면 안 된다.
...
- 멀티 서버 fixed tick은 플레이어 체력 0 스캔으로 사망·부활을 시작하거나 보상 선택 명령으로 아티팩트를 확정하지 않는다. 피해·사망은 피해자 impact claim, 보상 획득은 artifact-selection claim의 검증 경계에서만 서버 복제 상태와 공유 사건에 반영한다.
...
- 새 게임플레이 시스템의 테스트에는 최소한 `로컬 트리거가 서버 receipt보다 먼저 발생함`, `중복 claim은 한 번만 확정됨`, `거부 receipt가 로컬 상태를 복구함`, `승인 receipt와 정상 snapshot이 소유 클라이언트 상태·연출을 되감지 않음`, `서버 복제본과 동료가 소유자의 승인 상태로 수렴함`, `중립 객체는 모든 클라이언트가 서버 상태로 수렴함`을 포함한다.
