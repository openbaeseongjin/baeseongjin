...
- TO-BE 군집: Enemy slot 하나는 기본 10기의 작은 member를 조립하고 각 기체가 HP 10·독립 collider·피격·처치를 소유한다. 조밀한 무리로 함께 추격하며 공용 사격 없이 접촉 1회 피해 후 반동·회복·재합류한다. Map Editor의 Enemy `추가 정보` 패널은 군집 수를 2~20으로 저작한다.
- TO-BE 방패: 사격·방어 인식은 1440이며 방패 방향은 즉시 회전하지 않고 느린 angular acceleration·angular velocity를 적분해 Player를 따라간다.
