...
현재 Sector 01 package는 추격 드론 5상태 × 4 frame의 `128x160` atlas, Sentry의 고정 베이스·회전 머리를 담은 `64x32` atlas, Shield의 4 frame upright body loop를 담은 `128x32` atlas와 방패 8방향을 담은 `256x32` atlas, Artillery의 12 frame 감지 모션을 담은 `384x32` atlas, Patrol의 이동·공격을 담은 `128x192` atlas, Support의 idle·link를 담은 `128x64` atlas와 Swarm의 chase·contact/recoil을 담은 `128x96` atlas를 함께 사용한다. Sentry는 상태 clip에서 베이스를 선택하고 `aimLayer`로 머리·센서·단일 포신을 합성한다. Artillery는 별도 회전 layer 없이 `idle` 3 frame, `artillery-telegraph` 5 frame, `artillery-cooldown` 4 frame을 재생한다. 감지 셔터와 하부 범위 투영기는 대칭으로 반응하며 공격 위치는 gameplay 원형 telegraph가 전달한다. Pursuit는 `pursuit-seek`, `pursuit-windup`, `pursuit-dash`, `pursuit-recover`, `knockback`에 각각 네 frame을 사용하며 나머지 projectile attack 상태는 `pursuit-seek`로 fallback한다. `pursuit-windup`과 `pursuit-dash`에서는 새 방향 atlas나 layer를 추가하지 않고 기존 frame 전체를 `behaviorState.dashDirection`에 맞춰 회전하고 왼쪽 반원에서 반전한다. 이 방향은 gameplay가 Player 위치로 확정해 실제 돌진에도 사용하는 값이며 renderer는 추적 대상·궤적·판정을 다시 계산하지 않는다. Shield는 `shield-guard` 몸체 네 frame을 반복하고 `guardLayer`로 gameplay 방어 방향의 방패를 합성하며 idle·knockback·projectile attack 표현 상태가 같은 clip으로 명시적으로 fallback한다. Patrol은 정식 route 목표 방향으로 right-facing frame을 반전하고 사격 FSM의 다섯 상태를 별도 clip으로 표시한다. Support는 `support-idle`과 `support-link`를 직접 재생하며 공용 idle·knockback은 idle clip으로 fallback한다. Swarm은 `swarm-chase` 4 frame을 반복하고 실제 body contact 뒤 기존 `swarm-recoil` 상태 안에서 contact cue 4 frame과 recoil 4 frame을 합계 `450ms`로 재생한다. 싱글 디버그 몬스터 모션 더미도 같은 package 정의를 사용하며, 새 gameplay state나 공격 판정을 만들지 않는다. 이후 pose를 추가할 때 gameplay state 이름을 새로 만들지 말고 해당 presentation state의 clip만 교체한다.
