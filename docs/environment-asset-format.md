...
현재 authored package의 Sector 01→02, Sector 02→03, Sector 03→04, Sector 04→05, Sector 05→06 경계는 `PixelBackdropRenderer`가 Player world Y에서 파생한 단일 smoothstep 비율로 양쪽 package 전체와 sky를 교차 합성한다. 한 package의 모든 backdrop layer는 항상 같은 alpha를 사용해 경계에서 레이어별 잔상이나 건물 분리를 만들지 않는다. Sector 05→06에서는 나가는 Sector 05 package 전체에 동일한 blur를 같은 진행률로 증가시키고, 들어오는 Sector 06 package는 blur 없이 합성한다. Boss Stage 환경 Area는 Stage status가 `active`일 때만 현재 Area를 override한다. 이 표현 전환은 content boundary 사이의 collision, Gate, Camera, gameplay connector 또는 network state를 새로 만들지 않는다.

Authored backdrop은 `imageSmoothingEnabled = false`를 유지하고 layer의 destination X/Y를 정수 좌표로 snap한다. Sector 05→06의 blur는 사용자가 지정한 경계 연출이며 PNG 확대·시차 이동의 기본 sampling 규칙을 바꾸지 않는다.
