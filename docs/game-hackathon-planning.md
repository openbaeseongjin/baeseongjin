...
| 공간 경험 | 하나의 큰 절차 생성 월드를 연속해서 등반·횡단 | 점프킹, 할로우 나이트 |
...
- 부착 후 로프 길이를 플레이어가 직접 감거나 풀지 않는다.
- 로프는 고정 길이가 아니라 매달린 동안 계속 늘고 줄어드는 스프링으로 동작한다.
...
- 한 판의 목표 시간은 얼마인가?
- 월드는 주로 위로 오르는가, 옆으로 전진하는가, 두 방향을 혼합하는가?
