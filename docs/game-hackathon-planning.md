...
붕괴 중인 하나의 도시 월드를 고정 길이 로프로 탈출하며, 영역별 방해요소를 처리하고 매 플레이마다 달라지는 아티팩트 빌드로 돌파하는 2D 횡스크롤 액션 게임이다.
...
장르 중심은 **로프 액션 로그라이크**로 확정한다. 한 런은 하나의 붕괴 도시 월드 안에서 계속되며, 시나리오의 `맵`은 별도 월드가 아니라 명시적 입구·완료 조건·출구를 가진 진행 영역을 뜻한다. 실패해 체크포인트로 복귀하면 해당 도전에서 획득한 아티팩트 일부를 잃는다.
...
4. 영역을 넘어가도 월드·런·플레이어·아티팩트·체크포인트 상태를 새로 만들지 않는다.
5. 아티팩트를 수집해 공격 방식과 능력치를 강화한다.
6. 더 높은 구역 또는 최종 목표를 향해 전진한다.
7. 실패하면 최근 체크포인트로 복귀하며 이번 도전의 아티팩트 일부를 잃는다.
8. 체크포인트는 시나리오 진행과 섹터 구조에 맞춰 배치하며, 한번 활성화한 진행 지점은 아래로 내려가도 후퇴하지 않는다.
9. 복귀 손실은 2개 이상 보유 시 최근 획득 아티팩트부터 약 1/3이며 최소 1개는 유지한다.
10. 첫 체크포인트에서는 공격력·연사·로프 연계형 3종 중 하나를 좌우 이동과 점프로 선택한다.
11. 획득·손실은 공통 토스트로 알리고, 데스크톱만 보유 목록을 상시 HUD에 표시한다.
12. 시작점을 제외한 각 신규 체크포인트가 한 번씩 보상을 주고, 중복 선택은 해당 효과를 곱연산으로 강화한다.
13. 배포 전 `npm run validate:world`로 연결된 진행 영역의 로프 사거리·상승 경로·출구 연결을 검증한다.
14. 활성 플레이 시간·처치·피해·로프 절단·영역 완료·첫 보상 시간은 `RunMetrics`에서 수집해 난이도 조정 근거로 사용한다.
15. 발견된 문제 월드·영역은 회귀 목록에 이유와 함께 추가해 모든 월드 검증에서 우선 재현한다.
16. 원격 플레이테스트는 배포 URL의 `?metrics=1` 패널로 현재 런 지표를 확인한다.
17. 체력이 0이 되거나 낙사하면 동료 입력이나 팀 전멸을 기다리지 않고 해당 플레이어만 활성 체크포인트에서 최대 체력으로 즉시 부활한다. 부활할 때 자신의 최근 아티팩트 약 1/3을 잃되 최소 1개는 유지한다.
...
- 각 섹터의 일반 구간은 개별 맵마다 시간을 다시 주지 않고 섹터 전체가 공유하는 **일반 구간 총 타이머**를 사용한다. 타이머는 계속 감소하고 명시적 Gate를 통과하면 시간을 보충하며, 다음 섹터에 진입하면 새 일반 타이머를 시작한다.
- 일반 타이머가 0초가 되면 즉시 실패하지 않고 하층부터 붕괴가 상승한다. 붕괴에 잡힌 플레이어는 최소 관전 상태가 되어 생존 동료를 자동 추적하고, 생존자가 다음 Gate를 통과하면 자동 합류한다. 전원이 잡히면 해당 섹터 일반 구간을 재시작한다.
- 각 섹터에는 보스가 1개씩 있으며, 기획자가 정한 보스 진입 지점에서 일반 타이머와 상승 붕괴를 끝내고 잔여 시간을 폐기한 뒤 별도의 **보스 전투 타이머**를 시작한다.
- 보스 타이머가 0초가 되면 Arena가 점차 붕괴한다. 전원이 잡히면 섹터 전체가 아니라 보스 시도만 재시작하고, 보스 처치 뒤 다음 섹터 진입에서 새 일반 타이머를 시작한다.
...
- 섹터별 일반 구간 총 시간과 보스 전투 시간은 얼마인가?
