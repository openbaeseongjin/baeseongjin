...
고정 길이 로프 액션과 로그라이크·증강 성장 구조를 사용한다. 붕괴 중인 하나의 도시 월드를 올라가며 영역별 방해요소를 처리하고 매 플레이 성장 방향을 만든다.
...
- 정상 짧은 낙하는 허용하되 충돌 속도가 커질수록 착지 피해도 사망까지 비례 증가하게 해 고도 관리와 안전한 Recovery 선택에 의미를 준다.
...
- 사거리 안에 있는 모든 정적 collision 플랫폼·벽에 부착할 수 있다. one-way 여부나 표면의 구조적 역할은 부착 가능 여부를 바꾸지 않는다.
...
- 성공한 접선 스윙 뒤 로프가 부착 중이거나 정의된 해제 carry 안에 `620px/s` 이상으로 적과 새로 몸체 충돌하면, 반발 전 전체 속력에 비례한 피해를 준다. 기준은 `1000px/s → 100 피해`이며 `피해 = 속력 × 0.1`이다. 겹친 채 머무르는 동안에는 반복 피해를 주지 않고 분리 후 재진입해야 하며, 속도 부족·carry 만료·Shield 차단은 서로 구분해 알린다.
...
수평 발판은 아래에서 위로 통과하고 위에서 떨어질 때만 착지한다. 충돌 방향과 무관하게 로프는 모든 정적 collision 발판·벽 표면에 부착할 수 있다. 초기 월드는 상승 경로를 막는 수직 벽과 천장을 생성하지 않는다.

플랫폼 충돌 피해는 authored collision surface와 새 충돌이 시작된 tick의 보정 전 2D 속력으로 한 번 계산한다. `1200px/s` 미만은 안전하고 `1200px/s`에서 최대 체력의 10%(현재 10 피해)로 시작해 `2400px/s`의 100%(현재 100 피해)까지 선형 증가한다. `2400px/s` 이상은 최대 체력 100% 피해로 사망하며, 피해가 중간값에 머무는 별도 cap은 두지 않는다.
...
- Boss는 Sector03 경계의 `LOWER SECTOR COMMANDER`와 Sector06 마지막의 `CONTINUITY WARDEN`을 둔다. Boss03은 개인 입장·즉시 복귀·개별 출구, Boss06은 terminal Boarding을 사용한다.
