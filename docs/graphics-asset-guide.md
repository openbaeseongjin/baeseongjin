...
- Sector 01~06 공용 출구문과 조작 패널은 `world-object:gate`·`world-object:gate-panel` presentation ID 하나씩을 사용한다. 특정 Sector renderer가 별도 sprite 외형을 선택하지 않으며, Gate 해제 전/후의 `closed`·`opened` 형태를 공용 gameplay 상태에서 고른다. sprite 준비 전이나 실패 때만 기존 Sector별 Canvas mock으로 fallback한다.
