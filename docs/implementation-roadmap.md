...
메인 개발자는 [`docs/bsh/scenario/`](./bsh/scenario/)의 전체 48개 맵을 6개 섹터 단위로 아래 순서대로 구현한다. 각 섹터는 8개 맵을 가지며 내부에서는 `n-1 → n-8` 순서를 지킨다. 각 단계는 정식 그래픽·오디오를 기다리지 않고 `맵 흐름 확정 → 등장 오브젝트·상태·표현 cue 목록화 → gameplay 구현 → mock 이미지·사운드 연결 → 플레이 가능한 인계 빌드`까지 완료한다. 그래픽·오디오 담당자는 공개된 목록과 mock 배치를 기준으로 앞 섹터부터 병행 제작하며, validator와 실제 화면·청취 검증을 통과한 결과만 나중에 교체한다.

현재 저장소에 있는 시나리오 문서는 `1-1`, `1-2`, `1-3` 세 개뿐이다. 나머지 45개 맵의 시나리오·레벨 디자인은 아직 없으므로 2026년 8월 19일까지 섹터 순서대로 새로 작성하고 최소 오브젝트 요구를 함께 문서화한다. 정식 표현 리소스는 구현을 막지 않지만, 각 섹터의 8개 맵 명세는 그 섹터 mock 구현의 선행 입력이다.
