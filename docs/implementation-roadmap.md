...
- [0.82.1] Map Editor Boss03 Preview는 AI 공격 선택과 분리된 그랩 발사 반복 검증 모드를 제공한다. 실제 `grab-pull` clip과 사슬·hook tip을 같은 Canvas에서 반복하고 활성 중 gameplay만 정지하며, 다시 누르면 실전 AI 상태로 복귀한다. 실전 SEARCH의 발사 자세도 고정 0.5초가 아니라 실제 `grabHookProgress`에 맞춰 세 프레임 전체를 훅 비행 안에서 재생한다.
