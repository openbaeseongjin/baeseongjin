...
플레이어·로프처럼 특정 소유자가 있는 입력 주도 사건은 소유 클라이언트가, 충돌·피격은 피해 클라이언트가 최초 트리거한다. 서버는 이 사건을 플레이어 체감 경로에서 먼저 시작하지 않으며, claim의 소유권·tick·중립 객체 궤적·중복을 검증한 뒤 검증된 복제 상태와 공유 사건을 배포한다. 서버 왕복을 기다린 뒤 모바일 반응을 시작하는 구현은 허용하지 않는다.

서버의 주역할은 소유 클라이언트가 만든 상태와 사건을 검증해 다른 클라이언트에 공유하고 전체 복제본을 수렴시키는 것이다. 정상 승인 중인 소유 클라이언트의 직접 체감 상태를 서버 스냅샷으로 다시 작성하지 않는다. 검증된 아티팩트·무기 파라미터 같은 협동 진행 정보만 별도 공유 진행 경계에서 흡수한다. 소유자 전체 상태 복원은 최초 입장·재접속과 명시적으로 거부된 claim 복구에만 사용한다. 서버가 스스로 진행하고 최종 상태를 작성하는 범위는 몹·중립 투사체·공용 월드와 세션 수명주기다.
...
- `OwnerPredictionRuntime`은 소유 `InputDrivenObject` 집합 전체의 입력 이력, 예측 tick, claim 수명, 명시적 거부 복구와 표시 보정만 담당한다. 플레이어·로프 종류별 게임 로직은 capability 믹스인에 둔다.
...
아티팩트는 `PlayerRuntimeFactory`가 만드는 플레이어별 인벤토리로 두어 각자의 빌드 선택을 보존한다. 선택 UI는 행동 클라이언트가 즉시 진행하고 서버가 선택 claim을 검증한 뒤 빌드 진행을 공유한다. 사망·낙사를 감지한 소유 클라이언트는 `ArtifactInventory.applyCheckpointLoss()`를 부활과 함께 즉시 적용하고, 서버는 같은 결정적 정책과 중립 충돌 자료로 impact claim을 검증해 서버 복제본·동료와 검증된 빌드 진행을 공유한다. impact pending 동안 이전 스냅샷은 로컬 손실을 되돌리지 않으며, resolve가 포함된 공유 진행에서 같은 결정적 결과를 확인한다. 동료 인벤토리는 수정하지 않는다.
...
자신과 교차한 적 탄환은 피해 클라이언트가 `PlayerImpactPrediction` capability로 로컬 몸체·로프에 먼저 충돌시켜 즉시 피드백과 이동 반응을 적용한다. 본체 피격은 복제 탄환 대미지로 로컬 HP와 치명 시 체크포인트 부활·결정적 아티팩트 손실까지 공용 `GameSimulation`에서 실행하고, 멀티 HUD는 이 소유 클라이언트 상태를 표시한다. 클라이언트는 `projectileId`, clientTick, `player-hit` 또는 `rope-cut`, 충돌 위치만 impact claim으로 보내며 대미지를 신뢰 입력으로 보내지 않는다. 피격 직전 `owner-motion`을 캡처하고 로컬 반응을 적용한 뒤 전송선에서는 캡처한 motion 다음 impact claim 순서를 지킨다.

피해 클라이언트는 projectile ID별 피격 직전 전체 소유자 상태와 tick·발생 순서를 보존한다. 서버는 claim의 소유권·탄환·tick·궤적·위치·중복을 검사하고 서버 탄환의 대미지로 서버 복제본을 갱신한 뒤 resolve 사건과 상태를 동료에게 공유한다. 승인 receipt는 로컬 결과가 검증됐다는 표시이며 journal을 즉시 제거하지 않는다. 해당 projectile의 resolve 사건이 도착할 때 journal만 정리하고 소유자의 HP·부활·로프 상태는 서버 값으로 교체하지 않는다. pending impact가 있는 동안 이전 snapshot이나 명시적 motion 재기준화가 필요하면 impact를 원래 tick에 재실행해 로컬 결과를 보존한다. 거부 receipt만 적 탄환을 다시 표시하고 피격 직전 상태를 복원한 뒤 이후 실제 입력과 아직 대기 중인 후속 impact를 원래 tick·발생 순서대로 공용 1/120초 시뮬레이션에 재실행한다. 탄환은 같은 겹침에서 impact를 반복하지 않고 대상과 한 번 분리된 뒤 다시 진입해야 재무장한다. 재적용한 후속 impact의 직전 상태도 수정된 시간축으로 교체한다. 중간 입장과 재연결 welcome에는 아직 살아 있는 예측 객체의 원래 생성 이벤트를 같은 이벤트 ID로 다시 제공해 생성 tick부터 복원한다.
...
| HP·생명·부활·로프 절단 | 피해·행동 클라이언트의 로컬 결과와 승인된 claim | 피해 HP·치명 부활·절단을 즉시 적용하고 정상 snapshot으로 다시 쓰지 않음 | 서버가 검증·중복 제거한 공유값 즉시 적용 | impact 거절은 피격 전 전체 상태에서 이후 입력·후속 impact 재실행 |
...
- `owner-motion` 거부 receipt가 오면 마지막 공유 스냅샷의 소유자 물리·로프·제어 상태를 해당 플레이어의 `ownerMotionTick`에 복원하고, 그 다음 tick의 미확정 입력부터 현재 예측 tick까지 순서대로 다시 실행한다. 이미 owner-motion에 포함된 입력을 `serverTick` 기준으로 두 번 실행하지 않는다. 이 명시적 거부 복구와 최초 입장·재접속 외에는 서버 상태로 소유자를 통째로 복원하지 않는다.
...
HP 감소, 로프 절단, 사망·체크포인트 부활, 체크포인트 활성화, 아티팩트 변경과 런 완료는 시각적 보간 대상이 아니다. 당사자 클라이언트는 로컬 결과를 즉시 확정해 계속 사용하고, 서버는 claim을 검증·중복 제거해 다른 클라이언트에 공유한다. 승인 receipt와 resolve 사건은 소유자의 로컬 결과를 서버 값으로 교체하는 신호가 아니라 복제본이 같은 사건을 받아들였다는 확인이다.
