...
서버는 claim의 연결 소유권, 서버 탄환 ID, 인증된 피해 대상과 결정적 event key 중복을 검사하고 서버 탄환의 대미지로 서버 복제본을 갱신한 뒤 resolve 사건과 상태를 동료에게 공유한다. 피해 클라이언트의 위치·궤적은 지연된 서버 복제 위치와 다를 수 있으므로 그 오차를 impact 거절 근거로 사용하지 않는다. receipt의 승인 여부와 무관하게 이미 인식한 HP·부활·아티팩트·로프 전이는 되감지 않고, 소비한 적 탄환도 다시 표시하거나 같은 충돌을 반복하지 않는다. 정상 경로에서는 서버 복제본과 동료가 피해 클라이언트의 결과로 수렴한다. 중간 입장과 재연결 welcome에는 아직 살아 있는 예측 객체의 원래 생성 이벤트를 같은 이벤트 ID로 다시 제공해 생성 tick부터 복원한다.
...
| HP·생명·부활·로프 절단 | 피해·행동 클라이언트의 로컬 결과와 claim | 피해 HP·치명 부활·절단을 즉시 적용하고 snapshot·receipt로 다시 쓰지 않음 | 서버가 인증·중복 제거한 공유값 즉시 적용 | impact 거절도 로컬 체감 상태를 되감지 않음 |
