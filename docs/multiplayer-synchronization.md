...
현재 전환이 끝난 범위는 자기 이동·로프·자기 공격 충돌·자기 피격·로프 절단이다. 적 발사 생성, 체크포인트, 다운·부활 완료, 아티팩트 획득·손실, 팀 패배·정상 도달의 서버 시작형 경로는 전환 대상이며 완료로 간주하지 않는다. 신규 구현은 이 미전환 구조를 본보기로 사용하지 않는다.
...
체크포인트 보상 선택은 공용 서버 시계를 정지시키지 않는다. 선택 중인 플레이어의 좌우·확정·포인터 입력은 해당 tick의 보상 UI가 소비하고 중립 게임 명령으로 바꿔 이동·점프·로프에 중복 적용하지 않는다. 물리·적·투사체·자동 공격·다운 판정과 이미 선택을 마친 동료의 입력은 계속 진행한다.
