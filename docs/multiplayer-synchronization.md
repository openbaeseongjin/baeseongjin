...
`PredictableProjectileStore`는 최근 이벤트를 명시적으로 drain해 `spawn`에서 구체 `HomingProjectileObject` 또는 `BallisticProjectileObject`를 만들고 `resolve`에서 제거한다. 늦게 받은 생성 이벤트는 현재 serverTick과 생성 tick의 차이만큼 먼저 진행한다. 적 투사체는 공유된 초기 속도로 직선 이동하고, 플레이어 투사체는 공유된 targetId와 speed로 최신 적 위치를 추적한다. 두 궤적은 `SimulationDispatcher`가 선택한 motion capability를 거쳐 서버와 같은 `ProjectileMotion` 함수를 호출하며 동일 초기 상태·tick의 위치와 속도가 정확히 일치하는 테스트로 잠근다. 피해 claim 대기 객체는 계속 진행하되 렌더·추가 충돌에서 제외하고, 거부 receipt를 받으면 복원하며 승인 뒤 resolve가 오면 로컬 피드백 중복 없이 제거한다. 투사체 매 틱 좌표 배열은 스냅샷에 추가하지 않는다.
