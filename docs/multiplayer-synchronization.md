...
현재 권위 복귀 구현은 사망한 플레이어 한 명의 물리·로프·입력·체력·무기 상태만 활성 체크포인트에서 초기화한다. 해당 플레이어의 `lastCheckpointLoss`, `artifact-loss`, `player-respawned` 사건을 남기며 다른 플레이어와 공용 적·투사체 상태는 유지한다. 첫 플레이어의 손실 토스트 계약은 호환 별칭으로 유지한다.
