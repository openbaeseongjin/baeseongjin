...
| 체크포인트 도달·보상 시작 | 도달한 소유 클라이언트 선행, 서버 최종 확정 | 로컬 진행도·보상·로프 전이 후 위치 claim 전송 |
...
투사체처럼 시작 틱, 초기 위치·속도와 결정적 파라미터로 이후 움직임을 계산할 수 있는 객체는 전체 상태를 스냅샷마다 반복하지 않는다. 서버가 고유 이벤트 ID, 객체 ID·종류, 생성 틱, 초기 위치·속도와 반경·피해·소유자 같은 파라미터를 `spawn` 이벤트로 한 번 보낸다. 각 클라이언트는 같은 고정 틱으로 객체를 진행시킨다. 서버 권위 궤적과 클라이언트 예측 궤적은 모두 공용 `ProjectileMotion` 적분식을 사용하며, 전송 계층에 별도 이동 공식을 복제하지 않는다. 파편, 낙하물 등 같은 성질의 객체도 이 계약을 재사용한다.

플레이어 자동 무기 발사는 소유 클라이언트가 같은 `GameSimulation`에서 먼저 생성하고 즉시 표시한다. 클라이언트는 소유자와 clientTick으로 만든 `predictionId`, 대상 ID와 발사 위치만 `projectile-spawn-claim`으로 보낸다. 서버는 연결 소유권, tick 범위, 무기 쿨다운, 현재 사거리 안의 최근접 대상과 발사 위치 오차를 검증해 탄환과 고유 `spawn` 사건을 한 번 확정한다. 같은 claim은 같은 projectile ID가 든 receipt를 반환하고 다시 생성하지 않는다. 클라이언트는 prediction ID에 발사 직전·직후 무기 쿨다운과 tick을 보존해 승인 시 기록을 정리한다. 여러 발사가 pending이면 앞 거절은 후속 발사의 현재 쿨다운을 지우지 않고 후속 rollback 기준만 앞 발사가 없었던 시간축으로 갱신하며, 마지막 거절은 경과 tick을 뺀 최초 준비 상태로 복구한다. claim 대기 중인 이전 권위 스냅샷은 즉시 시작한 로컬 쿨다운을 지우지 않으며 receipt 뒤 최신 권위 쿨다운으로 수렴한다. 멀티 서버 fixed tick은 플레이어 무기 쿨다운과 이미 확정된 탄환의 검증용 궤적·수명만 진행하고 플레이어 발사를 독립적으로 시작하지 않는다. 싱글은 네트워크 claim 없이 같은 자동 무기 메서드를 직접 사용한다.

로프 공명 아티팩트의 스윙 강화는 입력 capability가 만든 지속 전투 상태다. 소유 클라이언트가 스윙 임펄스와 강화 타이머를 같은 프레임에 먼저 적용하고 소유자·clientTick으로 만든 `predictionId`, 위치와 로프 anchor만 `rope-swing-claim`으로 보낸다. 전송 순서는 `owner-motion → rope-swing-claim → projectile-spawn-claim`이다. 서버는 연결 소유권, tick 범위, 최신 위치·부착 anchor 오차와 보유 아티팩트를 검증하고 강화 타이머·무기 피해량과 고유 `rope-swing` 사건을 한 번 확정한다. 중복 claim은 기존 receipt만 반환한다. 클라이언트는 스윙별 강화 직전·직후 값과 tick을 기록한다. 앞 스윙 거절 뒤 후속 스윙이 pending이면 현재 후속 강화는 유지하고 후속 항목의 직전 값만 앞 스윙이 없었던 시간축으로 교정하며, 마지막 거절에서 최초 강화 값으로 돌아간다. 권위 스냅샷은 강화 남은 시간과 그 효과가 반영된 무기 상태를 계속 포함해 소유자·서버·동료를 같은 값으로 수렴시킨다.

자기 탄환 충돌은 공격 클라이언트가 먼저 예측해 타격 VFX를 재생한다. 클라이언트는 `predictionId`, 대상 ID, clientTick과 적중 위치만 hit claim으로 보내며 적 HP나 대미지를 결정하지 않는다. 멀티 서버 fixed tick은 플레이어 투사체의 검증용 궤적·대상 소실·8초 수명만 진행하고 적 충돌이나 HP 감소를 자동 시작하지 않는다. 공격 클라이언트는 claim 대기 총알을 화면과 추가 충돌에서 숨기되 궤적 진행은 유지하고, 거부 receipt가 오면 현재 위치에서 다시 활성화하며 승인 뒤에는 서버 적 HP 스냅샷과 resolve 사건으로 수렴한다. 자신과 교차한 적 탄환도 피해 클라이언트가 로컬 예측 몸체·로프에 먼저 충돌시켜 즉시 피드백과 이동 반응을 적용한다. 본체 피격은 복제 탄환 대미지로 로컬 HP와 치명 시 체크포인트 부활·결정적 아티팩트 손실까지 공용 `GameSimulation`에서 먼저 실행하고, 멀티 HUD는 이 예측 소유자 상태를 표시한다. 클라이언트는 `projectileId`, clientTick, `player-hit` 또는 `rope-cut`, 충돌 위치만 impact claim으로 보내며 대미지를 서버 최종값으로 제시하지 않는다. 이때 피격 직전 `owner-motion`을 캡처하고 로컬 반응을 즉시 적용한 뒤, 전송 순서는 캡처한 motion 다음 impact claim으로 고정한다. 피해 클라이언트는 projectile ID별 피격 직전 전체 소유자 상태와 tick·발생 순서를 보존한다. 서버는 먼저 충돌 직전 소유자 상태를 받은 뒤 claim의 소유권·탄환·tick·궤적·위치·중복을 검사하고 서버 탄환의 대미지로 넉백·HP·사망 시 체크포인트 부활·최종 로프 상태를 마지막 전이로 한 번 확정한다. claim 뒤에 피격 직후 motion을 보내 부활 위치를 덮어쓰면 안 된다. pending impact가 있는 동안 오래된 snapshot과 owner-motion 재기준화는 로컬 HP·부활·아티팩트·로프 결과를 덮지 않고 해당 impact를 원래 tick에 재실행한다. 승인 receipt는 복구 상태를 정리하고 서버 결과로 수렴한다. 거부 receipt는 적 탄환을 현재 궤도에서 다시 활성화하는 동시에 피격 직전 상태를 복원하고 이후 실제 입력과 아직 receipt를 기다리는 후속 impact를 원래 tick·발생 순서대로 공용 1/120초 시뮬레이션에 재실행한다. 이때 후속 impact의 직전 상태도 수정된 시간축으로 교체해, 앞 claim의 거절이 후속 피격을 지우거나 후속 거절이 앞선 잘못된 상태를 되살리지 않게 한다. 중간 입장과 재연결 welcome에는 아직 살아 있는 예측 객체의 원래 생성 이벤트를 같은 이벤트 ID로 다시 제공해 생성 tick부터 복원한다.
...
| HP·생명·부활·아티팩트·보상 | 서버 claim 확정 snapshot | 피해 HP·치명 부활·손실은 즉시 예측하고 pending 동안 보존, 이후 권위 결과 흡수 | 최신 권위값 즉시 적용 | impact 거절은 피격 전 전체 상태에서 이후 입력·후속 impact 재실행, 그 외 claim은 예측 피드백 해제 또는 권위 상태 적용 |
| 무기 파라미터·스윙 강화 시간 | 서버 claim 확정 snapshot | 스윙 즉시 예측, pending claim 동안 이전 snapshot 덮어쓰기 차단 | 최신 권위값 즉시 적용 | 후속 pending 강화 유지·baseline 교정, 마지막 거절은 최초 값 복구 |
| 자동 무기 쿨다운 | 승인된 발사 claim 뒤 서버 snapshot | 발사와 동시에 예측, pending claim 동안 이전 snapshot 덮어쓰기 차단 | 최신 권위값 즉시 적용 | 후속 pending 쿨다운 유지·baseline 교정, 마지막 거절은 최초 준비값 복구 |
...
HP 감소, 로프 절단, 사망·체크포인트 부활, 체크포인트 활성화, 아티팩트 변경과 런 완료는 시각적 보간 대상이 아니다. 당사자 클라이언트는 로컬 예측 결과를 즉시 표시하고 다른 클라이언트는 최신 서버 확정 상태를 적용하며, receipt와 snapshot으로 같은 지속 상태에 수렴한다.
...
`PredictableProjectileStore`는 최근 이벤트를 명시적으로 drain해 `spawn`에서 클라이언트 투사체를 만들고 `resolve`에서 제거한다. 늦게 받은 생성 이벤트는 현재 serverTick과 생성 tick의 차이만큼 먼저 진행한다. 적 투사체는 공유된 초기 속도로 직선 이동하고, 플레이어 투사체는 공유된 targetId와 speed로 최신 적 위치를 추적한다. 두 궤적은 서버와 같은 `ProjectileMotion` 함수를 호출하며 동일 초기 상태·tick의 위치와 속도가 정확히 일치하는 테스트로 잠근다. 피해 claim 대기 객체는 계속 진행하되 렌더·추가 충돌에서 제외하고, 거부 receipt를 받으면 복원하며 승인 뒤 resolve가 오면 로컬 피드백 중복 없이 제거한다. 투사체 매 틱 좌표 배열은 스냅샷에 추가하지 않는다.
