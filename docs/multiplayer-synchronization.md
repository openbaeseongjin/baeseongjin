...
| generic Augment 선택·효과           | 행동 클라이언트                    | 개인 chooser와 효과를 즉시 적용하고 서버가 호환 `augment-selection` claim으로 검증·공유 |
| 파티클·화면 흔들림·경고             | 각 클라이언트                      | 서버는 의미 사건만 공유하고 각 화면이 audience에 맞춰 재생                              |

핵심 원칙은 다음과 같다.

- 20Hz 반복 동기화는 `socket.send()` 직전 최종 WebSocket payload를 기준으로 클라이언트당 평균 24KB/s 이하를 기본 전송 예산으로 한다. welcome·late join·명시적 resync의 일회성 baseline만 제외하며, 전체 상태를 주기적으로 반복 전송해서는 안 된다.
...
| 상태 snapshot은 필요에 따라 unreliable/delta/relevancy를 사용한다. 브라우저 HTML5는 raw UDP를 사용할 수 없고 WebSocket 또는 WebRTC 경계를 사용한다. ([Godot High-level multiplayer](https://docs.godotengine.org/en/stable/tutorials/networking/high_level_multiplayer.html))                                                                                | v23은 20Hz reliable WebSocket을 유지하되 welcome·late join·resync baseline 뒤 ACK 기준 field delta와 Enemy 관심 영역을 사용한다. authored enemy 정적 정의는 `worldRevision + objectId`로 복원한다.                                                      | 현재 규모에서는 핵심 누락 아님        |
...
서버의 주역할은 소유 클라이언트가 만든 상태와 사건을 검증해 다른 클라이언트에 공유하고 전체 복제본을 수렴시키는 것이다. 소유 클라이언트의 직접 체감 상태를 서버 스냅샷으로 다시 작성하지 않는다. `owner-motion`은 인증·프로토콜 형식·유한값을 통과한 최신 tick을 서버 복제본과 동료의 수렴 원점으로 적용하며 속도·각속도·이동 거리의 크기로 거부하지 않는다. 다만 중립 Boss actor Rope는 `ownerId + localAnchor`가 현재 ropeable surface 위인지와 최초 reach/LOS를 검증하고 서버 Boss transform으로 world anchor를 재구성한다. 불가능한 actor anchor만 `ropeReleased` receipt/event로 해제하며 Player 전체 상태를 복원하지 않는다.
...
- 로컬 플레이어는 즉시 시뮬레이션하며 일반 서버 스냅샷으로 위치·속도·로프를 보정하지 않는다. actor Rope도 즉시 붙되 서버가 해당 owner/local anchor를 현재 Boss capability에서 재구성할 수 없을 때만 causal detach를 적용한다. 사건 claim은 각 계약의 허용 범위를 검증하지만 `owner-motion` 연속 상태는 물리량 봉투로 거부하지 않는다.
...
- `OwnerPredictionRuntime`은 소유 `InputDrivenObject` 집합 전체의 입력 이력, 예측 tick, claim 수명, 별도 복구 계약이 있는 사건 전이와 표시 보정만 담당한다. 정상 `owner-motion` receipt와 impact receipt는 로컬 상태를 복구하지 않으며, actor Rope의 `ropeReleased` receipt/event만 같은 owner/local anchor에 멱등 detach한다. 플레이어·로프 종류별 게임 로직은 capability와 공용 target resolver에 둔다.
...
멀티 디버그 Stage 이동도 요청 Player의 개별 이동을 유지한다. 다만 활성 또는 완료 전환 중인 Boss에서 요청이 들어오면 서버가 기존 Boss의 소환몹·투사체·hazard·capture를 먼저 제거하고, 요청자를 제외한 기존 참가자를 해당 Boss source Stage의 respawn anchor로 active 복귀시킨다. 이후 선택 Stage 기준 공용 진행과 다음 미완료 Boss Runtime을 inactive 상태로 원자적으로 다시 만들고 요청자만 선택 Stage Entry로 이동한다. 각 relocation은 고유 `debug-teleported` 사건을 가지며, 서버는 해당 owner-motion v12의 `debugTransitionId`가 사건 ID와 일치하기 전 운동 표본을 `ignored-debug-transition` no-op으로 처리해 이전 위치가 전환을 덮지 못하게 한다.
...
Boss06 공격 선택은 `worldSeed + attempt + selectionSequence`로 재현한다. concrete StateDefinition catalog의 고정 순서와 snapshot v2의 selection sequence·공격별 multiplier가 rollback·owner prediction·서버 복원에서 원자적으로 이동하며, 플랫폼 여부는 전송하지 않고 각 실행 환경이 같은 Boss 좌표와 authored surface bounds로 다시 계산한다. 후보 포함 조건에는 snapshot 이후 원격 Player 위치처럼 prediction마다 달라질 수 있는 값을 사용하지 않는다.
...
멀티 서버 fixed tick은 원시 게임 명령에 선택 입력이 포함돼도 보상 선택을 처리하지 않는다. generic Augment 선택의 유일한 서버 전이는 이름을 호환 유지한 `augment-selection` claim이며, 체력 0을 스캔해 사망·부활을 보조 발생시키지도 않는다. 피해와 사망·부활은 피해 클라이언트의 `player-impact` claim 안에서 함께 확정한다. 싱글은 네트워크 claim 왕복이 없으므로 같은 `GameSimulation` 옵션의 기본값으로 로컬 보상 입력과 체력 복구를 직접 수행한다.
...
- `owner-motion` protocol v12는 Rope joint, Spell·Experience·공통 상태이상 Pool과 마지막으로 적용한 디버그 전환 사건 ID를 보낸다.
...
players[]
  ...
  rope(anchor, anchorSurfaceId, attachmentOffset, length, currentLength, tension), weaponCooldown
  control(aimWorld, lastPointer, lastViewport, wasPointerDown, attachBufferRemaining, swingDrag)
  launcher(shot, cooldownRemaining), selectedAugmentIds, augmentRuntimeState, statusEffects
enemies[]
progressKind: sector
...
기본 로프 몸체 공격은 소유 클라이언트가 `RopeImpactState`의 성공 스윙과 부착 또는 해제 carry, `620px/s` 이상 반발 전 속도와 적 겹침의 새 진입을 같은 120Hz 스텝에서 감지해 즉시 `predicted-resolve` 피드백을 만든다. 정상 `rope-impact` claim은 `predictionId`, target ID, client tick, 충돌 위치와 반발 전 플레이어 속도만 보낸다. 서버는 연결 소유권·tick·prediction ID 중복을 확인하고 claim 속력에 `피해 = 속력 × 0.1`을 적용해 공식 피해를 다시 계산한 뒤 대상 생존/tombstone과 서버 소유 적 HP의 resolve 사건을 한 번 확정한다. 로프 부착·적 위치·접촉 집합은 지연된 서버 복제본으로 재판정하지 않는다. 같은 겹침은 `RopeImpactState`의 접촉 집합에서 재무장되지 않으며, 같은 prediction ID 재전송도 서버에서 같은 receipt를 반환한다. `충돌 폭발`의 augment-impact v2 claim은 같은 `impactSpeed`로 직접 100%·주변 50% 피해를 검증한다.
...
Hardpoint Jammer는 서버가 Jam target/phase를 진행하지만 실제 부착은 Rope 소유 클라이언트가 먼저 판정한다. Active target에 Hook이 붙은 tick에는 부착된 `owner-motion`을 먼저 보내고, 로컬 Rope를 즉시 절단해 재부착 제한과 단일 `ElectrifiedStatusEffect`를 시작한 뒤 `jammer-shock` claim 하나만 보낸다. 서버는 field group·cycle sequence·target surface와 owner Rope의 `anchorSurfaceId`·`attachmentId`, 총 피해 25를 검증해 같은 상태를 시작한다. 감전은 0.05초마다 2.5를 0.5초 동안 적용하지만 각 pulse는 네트워크 사건이나 claim이 아니다. owner와 서버 fixed step이 같은 상태 snapshot을 진행하며 재적용은 남은 지속시간만 0.5초로 갱신하고 pulse phase·피해 stack을 추가하지 않는다.
...
v9는 authored Enemy의 정적 필드를 제거하고 `worldRevision + objectId`로 복원했지만 20Hz 전체 동적 roster는 남겼다. 이후 authored slot이 86→189, seed 2 Runtime Enemy가 198개로 증가하면서 이 O(N) 계약이 다시 payload를 키웠다. v23은 welcome·late join·명시적 resync에서만 전체 baseline을 보내고 일반 20Hz envelope에는 정확한 `baseSequence`와 ACK baseline 이후의 field change·생성·제거·의미 사건만 기록한다. Player collection은 `id`, Enemy collection은 stable `objectId`가 정체성이다. 관심 영역 밖 Enemy는 기존 복제 상태를 유지한 채 반복 patch하지 않고, 관심 영역 재진입은 최신 동적 상태 전체를 한 번 upsert한다. despawn은 관심 여부와 무관하게 `objectId` removal로 보내며 사망 표현은 기존 Runtime Enemy ID의 의미 사건으로 별도 보존한다.

서버는 소켓마다 ACK된 sequence의 materialized wire state와 관심 membership, 아직 ACK되지 않은 eventId를 소유한다. 여러 snapshot이 밀리면 delta끼리 합치지 않고 `ACK baseline → 최신 canonical state`를 다시 계산하며 사건은 eventId로 합집합한다. 클라이언트도 `baseSequence → wire state`를 보존하고 해당 baseline에서 먼저 materialize한 뒤 authored Enemy 정적 정의를 hydrate한다. baseline이 없으면 delta를 추측 적용하지 않고 `snapshot-resync`를 한 번 요청한다. resync baseline에는 ACK 이후 보류 중이던 의미 사건도 포함한다.
...
`WorldSnapshotEnvelope`가 스냅샷 순서, 서버 틱, 월드 식별자, 플레이어별 승인 번호, `WorldSnapshotReplication`과 권위 이벤트를 하나의 전송 단위로 묶는다. `replication.kind`는 전체 `baseline` 또는 ACK state에 적용하는 `delta`이며 delta는 `baseSequence`, 일반 state field patch, Player·Enemy collection upsert/removal을 명시한다. `serverTick`은 중립 월드 시뮬레이션 시간을 뜻하고 각 플레이어의 `ownerMotionTick`은 그 물리·로프 상태를 만든 마지막 승인 소유자 tick을 뜻한다. `snapshotSequence`는 같은 server tick에 참가·퇴장·claim 확정으로 다시 만들어진 봉투까지 구분한다. 클라이언트는 sequence가 새롭고 serverTick이 같거나 증가한 봉투를 수용해야 한다. 투사체·낙하물처럼 예측 가능한 객체 배열을 state에 넣으면 계약 검증이 실패하며 반드시 생성·종료 이벤트를 사용해야 한다. 이벤트는 틱과 ID 순으로 정규화하고 한 봉투 안의 중복 ID를 거부한다.

Player Bark는 gameplay/network 사건이 아니라 각 클라이언트의 local presentation이다. 1-1/1-2의 Bark는 local `DirectionRuntime`이 같은 Beat의 System Text duration 뒤 message command로 queue에 넣고 local Player 머리 위에서 타이핑하며 WorldSnapshot·owner-motion·claim·동료 UI에 포함하지 않는다. Party Chat은 별도 `party-chat-submit`/`party-chat-message` transport를 사용한다. owner는 client sequence와 최대 80 grapheme text를 보낼 수 있을 때 즉시 local queue에 넣고, 서버는 socket의 인증 Player ID로 speaker와 message ID를 만들어 방 전체에 broadcast한다. sender echo는 causal dedupe하며 late join snapshot·재접속·채팅 기록에는 과거 message를 넣지 않는다. 화면 밖 speaker의 말풍선은 공용 screen-edge layout으로 해당 방향 경계에 표시하고 animation frame은 각 수신 클라이언트가 계산한다.
...
`AuthoritySnapshotBuilder`는 `GameSimulation`의 로컬 렌더 상태를 그대로 복제하지 않는다. 플레이어별 물리·로프·생명·무기·generic Augment, Enemy 동적 상태와 월드 진행으로 canonical baseline sample을 만들고, 정적 지형과 authored Enemy 정적 정의 대신 `worldSeed`, `worldRevision`과 stable `objectId`를 보낸다. `MultiplayerGameServer`는 이 sample을 소켓별 ACK baseline과 비교해 v23 delta를 만든다. 비활성 Boss는 DTO 생성과 wire에서 제외하고 활성·완료 Boss는 `bossStage` 하나만 기록한다. `bossRuntime`·`bossStageRuntime`은 v22 legacy baseline 입력을 `bossStage`로 읽는 deserializer 경계에서만 허용하며 serializer와 모든 client restore caller는 생성하지 않는다. 클라이언트는 world와 Enemy definition을 재생성하며 revision이 다르면 세션 참가를 중단한다.
...
| -------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| 위치·속도·접지·로프                    | 인증·형식 검사를 통과한 최신 `owner-motion` | 즉시 예측하며 정상 snapshot·receipt로 되감지 않음                        | 위치는 보간·제한 외삽, actor Rope는 같은 Boss transform에서 local anchor 복원 | 불가능한 actor anchor만 causal detach; 중복·역순 tick은 성공한 no-op   |
...
- 플레이어·로프 같은 `InputDrivenObject`는 소유 클라이언트의 즉시 시뮬레이션이 원점이다. 클라이언트가 `owner-motion`으로 현재 tick·위치·속도·각도·접지·로프 상태를 보내면 서버는 인증·프로토콜 형식·유한값과 세션 tick 범위를 통과한 최신 상태를 공용 `GameSimulation` 명령에 적용한다. actor Rope는 raw world anchor를 신뢰하지 않고 `ownerId + localAnchor`를 서버 Boss transform으로 복원하며 surface capability와 신규 부착 reach/LOS를 검증한다. 중복·역순·세션 범위 밖 tick과 완료된 런의 후속 상태는 성공한 no-op이다.
- 정상 승인 중인 소유 클라이언트는 20Hz 서버 지연 위치뿐 아니라 HP·생명 상태·로프 절단·무기 쿨다운·시간 제한 강화도 서버 스냅샷 값으로 다시 쓰지 않는다. generic Augment 선택처럼 협동 전체가 알아야 하는 공유 진행 결과만 흡수하며, 소유자의 직접 체감 상태와 authored Gate portal은 로컬 `GameSimulation`이 먼저 작성한다.
- 정상 `owner-motion` receipt는 소유자의 물리·로프·제어 상태를 복원하거나 미확정 입력 재실행을 시작하지 않는다. 예외인 `ropeReleased:true`는 서버가 actor owner 소멸·capability 비활성·off-surface local anchor를 확인한 결과이며 receipt/event가 echo한 `attachmentId`와 현재 세대가 정확히 같을 때만 detach한다. authored Gate portal은 별도 `gate-portal-entered` causal event와 portal tick으로 수렴하고 impact `recovery-required`는 피해 클라이언트의 최신 상태를 서버가 흡수한다.
- 적과 공용 월드 같은 `SimulationDrivenObject`는 서버 스냅샷이 원점이다. 동료와 적은 두 표본 사이를 보간하고 표본 공백만 최대 120ms 외삽한 뒤 다음 스냅샷에서 서버 궤도로 돌아온다.

`OwnerPredictionRuntime`은 별도 간이 물리를 만들지 않는다. 최초 입장 때 최신 공유 스냅샷을 로컬 예측용 `GameSimulation`의 공개 소유자 복원 명령에 전달하고 남은 입력을 고정 1/120초로 재실행한다. 이후 정상 스냅샷과 receipt는 서버 상태를 소유자 복구 명령으로 사용하지 않고, 검증된 공유 진행과 causal Rope detach만 흡수한다. authored Gate를 통과하면 같은 `GameSimulation`이 target Entry portal을 즉시 적용하고 `confirmPortalTransition()`이 같은 gate event만 멱등 확인한다.
...
`RemoteWorldStateBuffer`는 최대 8개 공유 스냅샷을 보관한다. Enemy와 공용 객체는 추정 서버 현재보다 100ms 과거의 `serverTick`을 사용한다. 동료 Player의 `ownerMotionTick`은 client prediction tick이 아니라 서버 시간축으로 투영된 `authorityTick`이므로 공용 목표 tick에 `inputLeadTicks`를 다시 더하지 않는다. Player는 자기 `ownerMotionTick`에서 시작한 안정적 playback clock을 가지며 기본 50ms buffer에 owner tick 간격과 실제 도착 간격의 편차를 12.5% EWMA로 흡수한 jitter 여유를 더해 최대 200ms 안에서 조절한다. buffer 깊이 오차는 playback 속도를 최대 ±20%만 바꿔 수렴시키고, 1차 보간 결과는 30ms exponential smoothing으로 표현한다. 160px을 넘는 전이와 포탈은 smoothing하지 않고 최신 표본으로 snap한다. 이 값은 `MultiplayerTiming`이 소유하며 Player 외 객체의 100ms 시간축을 바꾸지 않는다.

같은 serverTick의 새 sequence는 시간 표본을 중복 추가하지 않고 해당 tick의 최신 표본으로 교체하며 새 사건은 별도 대기열에 추가한다. 추정 서버 시계는 첫 표본에 고정하지 않고 serverTick이 전진한 스냅샷마다 최신 오차의 12.5%를 흡수하되 한 번의 지연 스파이크가 시간축을 크게 꺾지 않도록 보정량을 50ms로 제한한다. 동료와 적 위치는 각자 `ownerMotionTick`·`serverTick` 표본 앞뒤를 보간하고, 패킷 공백에서만 최대 120ms 외삽한다. 이 구성은 Unity Netcode의 buffered interpolation·RTT/jitter 기반 interpolation time 조절과 최신 NetworkTransform의 2단계 smoothing, Unreal의 exponential network smoothing, Source의 update-rate 기반 interpolation buffer 원칙을 현재 20Hz WebSocket 계약에 맞춘 것이다. ([Unity Interpolation](https://docs.unity.cn/Packages/com.unity.netcode%401.5/manual/interpolation.html), [Unity Time synchronization](https://docs.unity.cn/Packages/com.unity.netcode%400.50/manual/time-synchronization.html), [Unity NetworkTransform smoothing](https://docs.unity.cn/Packages/com.unity.netcode.gameobjects%402.7/api/Unity.Netcode.Components.NetworkTransform.InterpolationTypes.html), [Unreal Network Smoothing](https://dev.epicgames.com/documentation/unreal-engine/API/Runtime/Engine/ENetworkSmoothingMode?lang=en-US), [Source Multiplayer Networking](https://developer.valvesoftware.com/wiki/Source_Multiplayer_Networking)) Enemy snapshot은 actor collision 직후 선속도를 함께 보내 외삽과 Player 소유 충돌 응답이 같은 중립 운동 표본을 사용한다. 원격 Player HP·`lifeState`·로프 부착과 중립 Sector 진행·런 상태는 최신 검증 공유값을 사용한다. 자기 소유 객체는 `OwnerPredictionRuntime`이 담당하며 이 버퍼의 일반 위치·HP·생명·로프 값을 적용하지 않는다. 새로 생성되거나 제거된 엔티티는 최신 목록을 따르고 스냅샷 사건은 별도 대기열에서 정확히 한 번만 drain한다.
...
최초 입장·재접속에서 자기 플레이어를 공유 상태로 다시 시작할 때 점프 가능 여부와 로프 제약을 추측하지 않도록 플레이어별 `isGrounded`와 로프의 `length`, `currentLength`를 스냅샷에 포함한다. owner motion v8도 고정 `length`를 보내 서버 복제본이 매 motion마다 현재 거리로 rest length를 다시 만들지 않게 한다. 정상 스냅샷과 receipt는 이 값으로 소유자 물리를 다시 쓰지 않는다.
...
- owner가 이미 시작한 Spell·피해·상태이상 VFX는 정상 receipt와 resolve 사건 때문에 다시 재생하거나 되감지 않는다.
...
7. [완료] 장시간 로컬 수신 시계 드리프트에서도 최신 표본이 계속 오면 원격 시간축이 지속 외삽으로 밀리지 않는지 테스트한다.
8. [완료] Node 권위 서버와 실제 WebSocket 클라이언트 두 개를 같은 오픈월드에 연결한다.
9. [완료] impact 전체 상태는 서버가 먼저 발급한 일회용 challenge 뒤에만 받고, 복구 상태와 `stateTick`을 원자적으로 수용하며 실제 복원 필드 전체의 wire schema를 검증한다.
10. [필요] 서로 다른 실제 기기 두 대에서 로프 절단, 사망·낙사·개별 Stage 세이브 포인트 부활과 전원 사망 공용 진행 보존을 장시간 검증한다.
