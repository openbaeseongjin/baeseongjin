...
모든 게임플레이 이벤트의 최초 트리거는 클라이언트가 담당한다. 서버는 게임플레이 사건을 플레이어 체감 경로에서 먼저 시작하지 않으며, 클라이언트 claim의 소유권·tick·공간 판정·중복을 검증한 뒤 최종 상태와 공유 사건을 확정한다. 서버 왕복을 기다린 뒤 모바일 반응을 시작하는 구현은 허용하지 않는다. 연결·퇴장·빈 방 제거 같은 세션 수명주기만 서버 시작 예외다.

트리거 담당 클라이언트는 자기 행동이면 소유자, 충돌·피격이면 피해자, 대상 없는 공용 사건이면 안정적인 플레이어 ID 순서의 대표자로 정한다. 중복 감지가 가능한 사건은 결정적 event key를 사용한다.

현재 전환이 끝난 범위는 자기 이동·로프·자기 공격 충돌·자기 피격·로프 절단·아티팩트 선택과 획득이다. 적 발사 생성, 체크포인트, 사망·체크포인트 부활 완료, 아티팩트 손실, 정상 도달의 서버 시작형 경로는 전환 대상이며 완료로 간주하지 않는다. 신규 구현은 이 미전환 구조를 본보기로 사용하지 않는다.
...
`LocalPlayerPredictor`는 별도 간이 물리를 만들지 않는다. 최신 스냅샷의 자기 플레이어 물리·로프·제어·생명·무기·아티팩트를 로컬 예측용 `GameSimulation`의 공개 복원 명령에 전달하고, `serverTick` 다음부터 `RemoteCommandStream.pendingBatches()`의 목표 틱까지 공개 예측 진행 명령을 고정 1/120초로 실행한다. 예측기는 `GameSimulation`의 플레이어 객체·배열·tick을 직접 수정하지 않는다. 60Hz 명령 사이의 빈 120Hz 틱은 서버와 예측이 같은 `InputStateSimulator`로 마지막 입력을 최대 30틱(250ms) 유지한다. 새 중립 입력이 오면 즉시 교체하고 제한 시간이 끝나면 이동축을 중립화한다. 월드 seed 또는 generation revision이 다르면 예측을 시작하지 않는다.
...
`RemoteWorldStateBuffer`는 최대 8개 권위 스냅샷을 보관하고 추정 서버 현재보다 100ms 과거의 목표 tick을 계산한다. 동료와 적 위치는 목표 tick 앞뒤의 두 표본 사이에서 보간하고, 패킷 공백으로 목표가 최신 표본을 넘을 때만 최대 120ms 외삽한다. 자기 플레이어는 `LocalPlayerPredictor`가 담당하므로 보간하지 않는다. HP, `lifeState`, 로프 부착, 체크포인트와 런 상태는 최신 권위 값을 즉시 사용하며 새로 생성되거나 제거된 엔티티도 최신 목록을 따른다. 스냅샷 사건은 별도 대기열에서 정확히 한 번만 drain한다.
