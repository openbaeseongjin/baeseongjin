...
`RemoteWorldStateBuffer`는 동료 플레이어를 최신 권위 위치와 속도로 최대 120ms 외삽한다. 새 스냅샷이 도착하면 직전 화면 위치와 새 권위 경로의 오차를 100ms 동안 감쇠해 순간 이동을 줄인다. 자기 플레이어는 `LocalPlayerPredictor`가 담당하므로 외삽하지 않는다. HP, `lifeState`, 로프 부착, 체크포인트와 런 상태는 최신 권위 값을 즉시 사용하며 새로 생성되거나 제거된 엔티티도 최신 목록을 따른다. 스냅샷 사건은 별도 대기열에서 정확히 한 번만 drain한다.
