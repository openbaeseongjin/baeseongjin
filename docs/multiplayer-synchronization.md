...
자기 탄환 충돌은 공격 클라이언트가 먼저 예측해 타격 VFX를 재생한다. 클라이언트는 `predictionId`, 대상 ID, clientTick과 적중 위치만 hit claim으로 보내며 HP나 대미지를 결정하지 않는다. 서버는 WebSocket 연결의 플레이어와 탄환 소유자가 같은지, 탄환과 대상이 존재하는지, tick이 허용 범위인지, 적중 위치가 서버 대상 반경의 오차 범위 안인지, 이미 처리한 주장인지 검사한다. 통과하면 서버가 보유한 탄환 대미지로 HP·사망을 한 번만 확정하고 `resolve` 이벤트를 보낸다. 적 탄환의 플레이어 피격과 로프 절단은 서버 판정을 유지한다. 중간 입장과 재연결 때는 아직 살아 있는 예측 객체의 생성 이벤트를 현재 틱과 함께 다시 제공해 복원한다.
...
원격 클라이언트는 명령 sequence별 전송 시각과 receipt 수신 시각으로 왕복 시간을 측정하고, 연속 스냅샷의 수신 간격과 승인·거부 누계를 함께 보관한다. 소유자 예측기는 최근 256개 비영점 보정 거리의 p50/p95와 하드 스냅 누계를, 원격 상태 버퍼는 현재·최대 외삽 시간을, 예측 투사체 저장소는 권위 해결이 로컬 충돌보다 먼저 도착해 예측을 취소한 횟수를 제공한다. 이 값은 게임 규칙이나 보정값을 자동 변경하지 않으며 `?metrics=1` 진단 패널에서만 확인한다.
