...
- 새 클라이언트는 `snapshotAck=1` 연결 옵션으로 수신 확인을 협상한다. 서버는 클라이언트별 미확인 snapshot을 최대 4개만 전송하고 창이 찬 동안의 지속 상태는 가장 최신 snapshot으로 합친다. 합쳐지는 중간 snapshot의 spawn·resolve·피격 같은 의미 사건은 최신 봉투에 모두 보존한다.
- 누적 ACK가 창을 비우면 서버는 중간 지속 상태를 차례로 재생하지 않고 보존한 사건과 최신 상태를 즉시 전송한다. 합쳐진 대기 봉투가 256KiB를 넘으면 오래된 상태를 계속 재생하는 대신 세션 수렴 불가로 연결을 종료한다. 협상하지 않은 이전 클라이언트에는 배포 전환 호환을 위해 기존 전송을 유지한다.
