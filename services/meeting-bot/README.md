...
Before every start, the service verifies that the requester can view every capture channel and the minutes channel. It also requires all capture/minutes channels to resolve to the same effective `View Channel` overwrite policy — each channel's own overwrites layered on top of its category's overwrites — regardless of whether the channels share a category; syncing capture/minutes channels to the same category (or to categories with matching permissions) is the simplest way to satisfy this. The start fails closed when a channel or audience policy cannot be verified. This prevents a broader minutes audience from receiving private planning/coding text, display names, links, or filenames. The service also refuses to use the minutes channel itself as a reference channel.
