export const BOSS_STATE_LANE = Object.freeze({
...
function requireWorldSeed(value) {
    if ((typeof value !== "string" && !Number.isSafeInteger(value)) || String(value).length === 0) {
        throw new Error("Boss state worldSeed must be a safe integer or non-empty string");
    }
    return value;
}
...
function requireMultiplier(value, stateId) {
    if (!Number.isFinite(value) || value < SELECTED_MULTIPLIER || value > MAXIMUM_MULTIPLIER) {
        throw new Error(`Boss state multiplier for ${stateId} must be between 0.25 and 1`);
    }
    return value;
}
...
function createMultiplierState(definitions, restored = null) {
    ...
    for (const definition of definitions) {
        const weighted = definition.lane === BOSS_STATE_LANE.ATTACK;
    ...
    if (restored !== null) {
        for (const stateId of Object.keys(restored)) {
            if (
                ...
                definitions.find(({ id }) => id === stateId)?.lane !== BOSS_STATE_LANE.ATTACK
            ) {
                throw new Error(`Boss state multiplier references unknown state ${stateId}`);
            }
        }
    }
    return multipliers;
...
function immutableMultiplierSnapshot(definitions, multipliers) {
    return Object.freeze(
        Object.fromEntries(
            definitions
                .filter(({ lane }) => lane === BOSS_STATE_LANE.ATTACK)
                .map((definition) => [definition.id, multipliers[definition.id]])
        )
    );
}

export class BossStateDefinition {
    constructor({
        id,
        lane,
        ...
        this.id = requireNonEmptyString(id, "Boss state id");
        this.lane = requireNonEmptyString(lane, `Boss state ${this.id} lane`);
        this.canEnter = requireHook(canEnter, `Boss state ${this.id} canEnter`);
        this.weight = requireHook(weight, `Boss state ${this.id} weight`);
        this.enter = requireHook(enter, `Boss state ${this.id} enter`);
        this.advance = requireHook(advance, `Boss state ${this.id} advance`);
        this.exit = requireHook(exit, `Boss state ${this.id} exit`);
        Object.freeze(this);
    }
}

export function defineBossStateCatalog(definitions) {
    if (!Array.isArray(definitions) || definitions.length === 0) {
        throw new Error("Boss state catalog requires at least one definition");
    }
    const catalog = Object.create(null);
    for (const definition of definitions) {
        if (!(definition instanceof BossStateDefinition)) {
            throw new Error("Boss state catalog accepts only BossStateDefinition entries");
        }
        if (catalog[definition.id] !== undefined)
            throw new Error(`Boss state catalog contains duplicate id ${definition.id}`);
        catalog[definition.id] = definition;
    }
    return Object.freeze(catalog);
}

export class BossStatePool {
    constructor({ catalog, worldSeed, attempt = 0, selectionSequence = 0, multipliers = null }) {
        if (catalog === null || typeof catalog !== "object" || Array.isArray(catalog) || !Object.isFrozen(catalog)) {
            throw new Error("BossStatePool requires a fixed catalog object");
        }
        this.catalog = catalog;
        this.definitions = Object.freeze(Object.values(catalog));
        if (this.definitions.length === 0) throw new Error("BossStatePool catalog cannot be empty");
        for (const definition of this.definitions) {
            if (!(definition instanceof BossStateDefinition) || catalog[definition.id] !== definition) {
                throw new Error("BossStatePool catalog must be keyed by definition id");
            }
        }
        this.worldSeed = requireWorldSeed(worldSeed);
        this.attempt = requireSafeInteger(attempt, "Boss state attempt");
        this.selectionSequence = requireSafeInteger(selectionSequence, "Boss state selectionSequence");
        this.multipliers = createMultiplierState(this.definitions, multipliers);
    }

    definition(stateId) {
        requireNonEmptyString(stateId, "Boss state id");
        const definition = this.catalog[stateId];
        if (definition === undefined) throw new Error(`Unknown Boss state ${stateId}`);
        return definition;
    }
    ...
    select({ lane, context, chainStateId = null }) {
        requireNonEmptyString(lane, "Boss selection lane");
    ...
    restore(snapshot) {
        if (snapshot === null || typeof snapshot !== "object")
            throw new Error("BossStatePool restore requires a snapshot");
