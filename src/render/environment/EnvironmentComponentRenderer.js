...
export class EnvironmentComponentRenderer {
    constructor({ id, atlasIds, assets, renderer, fallbackRenderer, warn = console.warn } = {}) {
        ...
        this.id = id;
        this.atlasIds = uniqueAtlasIds(atlasIds);
        this.assets = assets;
    ...
    assessStatus() {
        ...
        const nextStatus = failedAtlasIds.length
            ? "failed"
            : this.assets && this.atlasIds.every((atlasId) => this.assets.isReady(atlasId))
              ? "ready"
              : "pending";
    ...
    draw(args) {
        const status = this.assessStatus();
